...
namespace ZeroTier {
    ...
    LFDB::LFDB(const Identity &myId, const char *path, const char *lfOwnerPrivate,
               const char *lfOwnerPublic, const char *lfNodeHost, int lfNodePort,
               bool storeOnlineState) :
            DB(),
            _myId(myId),
                    ...
                    for (auto ns = _state.begin(); ns != _state.end(); ++ns) {
                        if (ns->second.dirty) {
                            ...
                            if (get(ns->first, network)) {
                                ...
                                try {
                                    auto resp = htcli.Post("/makerecord", newrec.dump(),
                                                           "application/json");
                                    if (resp) {
                                        if (resp->status == 200) {
                                            ...
                                            fprintf(stderr,
                                                    "ERROR: LFDB: %d from node (create/update network): %s" ZT_EOL_S,
                                    ...
                                    fprintf(stderr,
                                            "ERROR: LFDB: unexpected exception querying node (create/update network): %s" ZT_EOL_S,
                                            e.what());
                                } catch (...) {
                                    fprintf(stderr,
                                            "ERROR: LFDB: unexpected exception querying node (create/update network): unknown exception" ZT_EOL_S);
                                }
                            }
                        }

                        for (auto ms = ns->second.members.begin();
                             ms != ns->second.members.end(); ++ms) {
                            if ((_storeOnlineState) && (ms->second.lastOnlineDirty) &&
                                (ms->second.lastOnlineAddress)) {
                                nlohmann::json newrec, selector0, selector1, selectors, ip;
                                char tmp[1024], tmp2[128];
                                OSUtils::ztsnprintf(tmp, sizeof(tmp),
                                                    "com.zerotier.controller.lfdb:%s/network/%.16llx/online",
                                                    controllerAddress,
                                                    (unsigned long long) ns->first);
                                ...
                                try {
                                    ...
                                    if (resp) {
                                        if (resp->status == 200) {
                                            ...
                                            fprintf(stderr,
                                                    "ERROR: LFDB: %d from node (create/update member online status): %s" ZT_EOL_S,
                                                    resp->status, resp->body.c_str());
                                        }
                                    } else {
                                        fprintf(stderr, "ERROR: LFDB: node is offline" ZT_EOL_S);
                                    ...
                                    fprintf(stderr,
                                            "ERROR: LFDB: unexpected exception querying node (create/update member online status): %s" ZT_EOL_S,
                                            e.what());
                                } catch (...) {
                                    fprintf(stderr,
                                            "ERROR: LFDB: unexpected exception querying node (create/update member online status): unknown exception" ZT_EOL_S);
                                }
                            }
                                    ...
                                    try {
                                        ...
                                        if (resp) {
                                            if (resp->status == 200) {
                                                ...
                                                fprintf(stderr,
                                                        "ERROR: LFDB: %d from node (create/update member): %s" ZT_EOL_S,
                                                        resp->status, resp->body.c_str());
                                            }
                                        } else {
                                            fprintf(stderr,
                                                    "ERROR: LFDB: node is offline" ZT_EOL_S);
                                        }
                                    } catch (std::exception &e) {
                                        fprintf(stderr,
                                                "ERROR: LFDB: unexpected exception querying node (create/update member): %s" ZT_EOL_S,
                                                e.what());
                                    } catch (...) {
                                        fprintf(stderr,
                                                "ERROR: LFDB: unexpected exception querying node (create/update member): unknown exception" ZT_EOL_S);
                                    }
                                }
                            }
                        }
                ...
                try {
                    ...
                    query <<
                          ...
                          "\"Name\":\"" << networksSelectorName << "\","
                                                                   ...
                                                                   "}],"
                                                                   "\"TimeRange\":["
                          << timeRangeStart << ",9223372036854775807],"
                    ...
                    if (resp) {
                        if (resp->status == 200) {
                                    ...
                                    if ((rset.is_array()) && (!rset.empty())) {
                                        ...
                                        if (result.is_object()) {
                                            ...
                                            if (record.is_object()) {
                                                ...
                                                nlohmann::json network(
                                                        OSUtils::jsonParse(recordValue));
                                                if (network.is_object()) {
                                                    const std::string idstr = network["id"];
                                                    const uint64_t id = Utils::hexStrToU64(
                                                            idstr.c_str());
                                                    if ((id >> 24) ==
                                                        controllerAddressInt) { // sanity check
                                                        ...
                                                        if ((timeRangeStart > 0) &&
                                                            ...
                                                            if (prevRevision < revision) {
                                                                _networkChanged(oldNetwork, network,
                                                                                timeRangeStart > 0);
                                                            }
                                                        } else {
                                                            nlohmann::json nullJson;
                                                            _networkChanged(nullJson, network,
                                                                            timeRangeStart > 0);
                                                        }
                            ...
                            fprintf(stderr,
                                    "ERROR: LFDB: %d from node (check for network updates): %s" ZT_EOL_S,
                                    resp->status, resp->body.c_str());
                        }
                    } else {
                        fprintf(stderr, "ERROR: LFDB: node is offline" ZT_EOL_S);
                    ...
                    fprintf(stderr,
                            "ERROR: LFDB: unexpected exception querying node (check for network updates): %s" ZT_EOL_S,
                            e.what());
                } catch (...) {
                    fprintf(stderr,
                            "ERROR: LFDB: unexpected exception querying node (check for network updates): unknown exception" ZT_EOL_S);
                }
                ...
                try {
                    ...
                    if (resp) {
                        if (resp->status == 200) {
                                        ...
                                        if (result.is_object()) {
                                            ...
                                            if (record.is_object()) {
                                                ...
                                                if (member.is_object()) {
                                                    ...
                                                    const uint64_t nwid = Utils::hexStrToU64(
                                                            nwidstr.c_str());
                                                    const uint64_t id = Utils::hexStrToU64(
                                                            idstr.c_str());
                                                    if ((id) && ((nwid >> 24) ==
                            ...
                            fprintf(stderr,
                                    "ERROR: LFDB: %d from node (check for member updates): %s" ZT_EOL_S,
                                    resp->status, resp->body.c_str());
                        }
                    } else {
                        fprintf(stderr, "ERROR: LFDB: node is offline" ZT_EOL_S);
                    ...
                    fprintf(stderr,
                            "ERROR: LFDB: unexpected exception querying node (check for member updates): %s" ZT_EOL_S,
                            e.what());
                } catch (...) {
                    fprintf(stderr,
                            "ERROR: LFDB: unexpected exception querying node (check for member updates): unknown exception" ZT_EOL_S);
                }

                timeRangeStart = time(nullptr) -
                                 120; // start next query 2m before now to avoid losing updates
                _ready.store(true);

                for (int k = 0; k <
                                4; ++k) { // 2s delay between queries for remotely modified networks or members
                    if (!_running.load())
                        return;
    ...
    void LFDB::nodeIsOnline(const uint64_t networkId, const uint64_t memberId,
