...
namespace ZeroTier {
    ...
    DBMirrorSet::DBMirrorSet(DB::ChangeListener *listener)
            : _listener(listener), _running(true), _syncCheckerThread(), _dbs(), _dbs_l() {
        _syncCheckerThread = std::thread([this]() {
            for (;;) {
                ...
                for (auto db = dbs.begin(); db != dbs.end(); ++db) {
                    (*db)->each([&dbs, &db](uint64_t networkId, const nlohmann::json &network,
                                            uint64_t memberId, const nlohmann::json &member) {
                        try {
                            if (network.is_object()) {
                                if (memberId == 0) {
                                    for (auto db2 = dbs.begin(); db2 != dbs.end(); ++db2) {
                                        if (db->get() != db2->get()) {
                                            nlohmann::json nw2;
                                            if ((!(*db2)->get(networkId, nw2)) ||
                                                ((nw2.is_object()) &&
                                                 (OSUtils::jsonInt(nw2["revision"], 0) <
                                                  OSUtils::jsonInt(network["revision"], 0)))) {
                                                nw2 = network;
                                                (*db2)->save(nw2, false);
                                            }
                                        }
                                    ...
                                    for (auto db2 = dbs.begin(); db2 != dbs.end(); ++db2) {
                                        if (db->get() != db2->get()) {
                                            ...
                                            if ((!(*db2)->get(networkId, nw2, memberId, m2)) ||
                                                ((m2.is_object()) &&
                                                 (OSUtils::jsonInt(m2["revision"], 0) <
                                                  OSUtils::jsonInt(member["revision"], 0)))) {
                                                m2 = member;
                                                (*db2)->save(m2, false);
                                            }
                                        }
    ...
    bool
    DBMirrorSet::get(const uint64_t networkId, nlohmann::json &network, const uint64_t memberId,
    ...
    bool
    DBMirrorSet::get(const uint64_t networkId, nlohmann::json &network, const uint64_t memberId,
                     nlohmann::json &member, DB::NetworkSummaryInfo &info) {
        std::shared_lock<std::shared_mutex> l(_dbs_l);
        for (auto d = _dbs.begin(); d != _dbs.end(); ++d) {
    ...
    AuthInfo
    DBMirrorSet::getSSOAuthInfo(const nlohmann::json &member, const std::string &redirectURL) {
        std::shared_lock<std::shared_mutex> l(_dbs_l);
        for (auto d = _dbs.begin(); d != _dbs.end(); ++d) {
    ...
    void DBMirrorSet::nodeIsOnline(const uint64_t networkId, const uint64_t memberId,
    ...
    void DBMirrorSet::onNetworkUpdate(const void *db, uint64_t networkId,
    ...
    void DBMirrorSet::onNetworkMemberUpdate(const void *db, uint64_t networkId, uint64_t memberId,
    ...
    void
    DBMirrorSet::onNetworkMemberDeauthorize(const void *db, uint64_t networkId, uint64_t memberId) {
