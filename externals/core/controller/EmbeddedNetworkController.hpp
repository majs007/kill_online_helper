...
namespace ZeroTier {
    ...
    public:
         ...
         */
        EmbeddedNetworkController(Node *node, const char *ztPath, const char *dbPath,
                                  int listenPort, RedisConfig *rc);
        ...
        virtual void onNetworkMemberUpdate(const void *db, uint64_t networkId, uint64_t memberId,
    ...
    private:
        void _request(uint64_t nwid, const InetAddress &fromAddr, uint64_t requestPacketId,
                      const Identity &identity,
                      const Dictionary<ZT_NETWORKCONFIG_METADATA_DICT_CAPACITY> &metaData);
        ...
        struct _MemberStatusKey {
            ...
            _MemberStatusKey(const uint64_t nwid, const uint64_t nid) : networkId(nwid),
                                                                        nodeId(nid) {}
            ...
            inline bool operator<(const _MemberStatusKey &k) const {
                return (k.networkId < networkId) ||
                       ((k.networkId == networkId) && (k.nodeId < nodeId));
            }
        };

        struct _MemberStatus {
            _MemberStatus() : lastRequestTime(0), authenticationExpiryTime(-1), vMajor(-1),
                              vMinor(-1), vRev(-1), vProto(-1) {}
