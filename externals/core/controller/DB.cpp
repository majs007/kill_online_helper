...
namespace ZeroTier {
    ...
    void DB::initNetwork(nlohmann::json &network) {
        ...
        if (!network.count("v6AssignMode"))
            network["v6AssignMode"] = {{"rfc4193", false},
                                       {"zt",      false},
                                       {"6plane",  false}};
    ...
    bool DB::get(const uint64_t networkId, nlohmann::json &network, const uint64_t memberId,
    ...
    void
    DB::_memberChanged(nlohmann::json &old, nlohmann::json &memberConfig, bool notifyListeners) {
        ...
        if (memberConfig.is_object()) {
...

            {
                ...
                if (!isAuth) {
                    const int64_t ldt = (int64_t) OSUtils::jsonInt(
                            memberConfig["lastDeauthorizedTime"], 0ULL);
        ...
        if (notifyListeners) {
            ...
            } else if (networkId != 0 && memberId != 0 && !old.is_object() &&
                       memberConfig.is_object()) {
                // new member
                Metrics::member_count++;
            }
    ...
    void
    DB::_networkChanged(nlohmann::json &old, nlohmann::json &networkConfig, bool notifyListeners) {
        Metrics::db_network_change++;
        if (notifyListeners) {
            if (old.is_object() && old.contains("id") && networkConfig.is_object() &&
                networkConfig.contains("id")) {
                Metrics::network_changes++;
            } else if (!old.is_object() && networkConfig.is_object() &&
                       networkConfig.contains("id")) {
