...
namespace ZeroTier {
    ...
    namespace {
            ...
            if (r.empty()) {
                switch (rt) {
                    ...
                    case ZT_NETWORK_RULE_MATCH_MAC_SOURCE:
                        ...
                        OSUtils::ztsnprintf(tmp, sizeof(tmp), "%.2x:%.2x:%.2x:%.2x:%.2x:%.2x",
                    ...
                    case ZT_NETWORK_RULE_MATCH_IPV4_SOURCE:
                        ...
                        r["ip"] = InetAddress(&(rule.v.ipv4.ip), 4,
                                              (unsigned int) rule.v.ipv4.mask).toString(tmp);
                        break;
                    ...
                    case ZT_NETWORK_RULE_MATCH_IPV6_SOURCE:
                        ...
                        r["ip"] = InetAddress(rule.v.ipv6.ip, 16,
                                              (unsigned int) rule.v.ipv6.mask).toString(tmp);
                        break;
                    ...
                    case ZT_NETWORK_RULE_MATCH_INTEGER_RANGE:
                        ...
                        r["start"] = tmp;
                        OSUtils::ztsnprintf(tmp, sizeof(tmp), "%.16llx",
                                            rule.v.intRange.start + (uint64_t) rule.v.intRange.end);
                        r["end"] = tmp;
        ...
        static bool _parseRule(json &r, ZT_VirtualNetworkRule &rule) {
            ...
            if (t == "ACTION_DROP") {
                ...
                rule.v.fwd.address =
                        Utils::hexStrToU64(OSUtils::jsonString(r["address"], "0").c_str()) &
                        0xffffffffffULL;
                ...
                rule.v.zt = Utils::hexStrToU64(OSUtils::jsonString(r["zt"], "0").c_str()) &
                            0xffffffffffULL;
                return true;
                ...
                rule.v.ipv4.mask =
                        Utils::ntoh(reinterpret_cast<struct sockaddr_in *>(&ip)->sin_port) & 0xff;
                ...
                memcpy(rule.v.ipv6.ip,
                       reinterpret_cast<struct sockaddr_in6 *>(&ip)->sin6_addr.s6_addr, 16);
                rule.v.ipv6.mask =
                        Utils::ntoh(reinterpret_cast<struct sockaddr_in6 *>(&ip)->sin6_port) & 0xff;
                ...
                rule.v.port[1] = (uint16_t) (OSUtils::jsonInt(r["end"], (uint64_t) rule.v.port[0]) &
                                             0xffffULL);
                return true;
                ...
                rule.v.frameSize[1] = (uint16_t) (
                        OSUtils::jsonInt(r["end"], (uint64_t) rule.v.frameSize[0]) & 0xffffULL);
                return true;
            } else if (t == "MATCH_RANDOM") {
                rule.t |= ZT_NETWORK_RULE_MATCH_RANDOM;
                rule.v.randomProbability = (uint32_t) (OSUtils::jsonInt(r["probability"], 0ULL) &
                                                       0xffffffffULL);
                return true;
                ...
                if (e.is_string()) {
                    std::string tmp = e;
                    rule.v.intRange.end = (uint32_t) (Utils::hexStrToU64(tmp.c_str()) -
                                                      rule.v.intRange.start);
                } else {
                    rule.v.intRange.end = (uint32_t) (OSUtils::jsonInt(e, 0ULL) -
                                                      rule.v.intRange.start);
                }
    ...
    EmbeddedNetworkController::EmbeddedNetworkController(Node *node, const char *ztPath,
                                                         const char *dbPath, int listenPort,
                                                         RedisConfig *rc)
            : _startTime(OSUtils::now()), _listenPort(listenPort), _node(node), _ztPath(ztPath),
              _path(dbPath), _signingId(), _signingIdAddressString(),
              _sender((NetworkController::Sender *) 0), _db(this), _queue(), _threads(),
              _threads_l(), _memberStatus(), _memberStatus_l(), _expiringSoon(), _expiringSoon_l(),
              _rc(rc), _ssoExpiryRunning(true),
            ...
            if (settings.is_object()) {
                ...
                if (controllerDb.is_object()) {
                    ...
                    if (type == "lf") {
                        ...
                        if ((lfOwner.length()) && (lfHost.length()) && (lfPort > 0) &&
                            (lfPort < 65536)) {
                            ...
                            if ((pubHdrLoc > 0) && ((pubHdrLoc + 8) < lfOwner.length())) {
                                ...
                                if (pubHdrEnd != std::string::npos) {
                                    ...
                                    _db.addDB(std::shared_ptr<DB>(
                                            new LFDB(_signingId, _path.c_str(), lfOwner.c_str(),
                                                     lfOwnerPublic.c_str(), lfHost.c_str(), lfPort,
                                                     storeOnlineState)));
                                }
                            }
                        }
    ...
    void EmbeddedNetworkController::request(
        ...
        if (((!_signingId) || (!_signingId.hasPrivate())) ||
            (_signingId.address().toInt() != (nwid >> 24)) || (!_sender))
            return;
        _startThreads();
    ...
    std::string EmbeddedNetworkController::networkUpdateFromPostData(uint64_t networkID,
        ...
        if (b.count("mtu"))
            network["mtu"] = std::max(
                    std::min((unsigned int) OSUtils::jsonInt(b["mtu"], ZT_DEFAULT_MTU),
                             (unsigned int) ZT_MAX_MTU), (unsigned int) ZT_MIN_MTU);
        ...
        if (b.count("v6AssignMode")) {
            ...
            if (v6m.is_string()) { // backward compatibility
                std::vector<std::string> v6ms(
                        OSUtils::split(OSUtils::jsonString(v6m, "").c_str(), ",", "", ""));
    ...
    void EmbeddedNetworkController::configureHTTPControlPlane(
        ...
        auto networkListGet = [&, setContent](const httplib::Request &req, httplib::Response &res) {
            ...
            auto out = json::array();
            for (std::set<uint64_t>::const_iterator i(networkIds.begin());
                 i != networkIds.end(); ++i) {
        ...
        auto createNewNetwork = [&, setContent](const httplib::Request &req,
                                                httplib::Response &res) {
            fprintf(stderr, "creating new network (new style)\n");
            uint64_t nwid = 0;
            uint64_t nwidPrefix = (Utils::hexStrToU64(_signingIdAddressString.c_str()) << 24) &
        ...
        auto createNewNetworkOldAndBusted = [&, setContent](const httplib::Request &req,
                                                            httplib::Response &res) {
        ...
        auto memberPost = [&, setContent](const httplib::Request &req, httplib::Response &res) {
            ...
            if (b.count("authenticationExpiryTime"))
                member["authenticationExpiryTime"] = (uint64_t) OSUtils::jsonInt(
                        b["authenticationExpiryTime"], 0ULL);
            ...
            if (b.count("authorized")) {
                ...
                if (newAuth != OSUtils::jsonBool(member["authorized"], false)) {
                    ...
                    member[((newAuth) ? "lastAuthorizedTime"
                                      : "lastDeauthorizedTime")] = OSUtils::now();
                    if (newAuth) {
            ...
            if (b.count("tags")) {
                ...
                if (tags.is_array()) {
                    ...
                    for (std::map<uint64_t, uint64_t>::iterator t(mtags.begin());
                         t != mtags.end(); ++t) {
                        json ta = json::array();
    ...
    void EmbeddedNetworkController::handleRemoteTrace(const ZT_RemoteTrace &rt) {
        ...
        try {
            ...
            const int64_t now = OSUtils::now();
            OSUtils::ztsnprintf(id, sizeof(id), "%.10llx-%.16llx-%.10llx-%.4x",
                                _signingId.address().toInt(), now, rt.origin,
    ...
    void EmbeddedNetworkController::onNetworkUpdate(const void *db, uint64_t networkId,
        ...
        for (auto i = _memberStatus.begin(); i != _memberStatus.end(); ++i) {
            if ((i->first.networkId == networkId) && (i->second.online(now)) &&
                (i->second.lastRequestMetaData))
                request(networkId, InetAddress(), 0, i->second.identity,
                        i->second.lastRequestMetaData);
        }
    }

    void EmbeddedNetworkController::onNetworkMemberUpdate(const void *db, uint64_t networkId,
                                                          uint64_t memberId,
    ...
    void EmbeddedNetworkController::onNetworkMemberDeauthorize(const void *db, uint64_t networkId,
                                                               uint64_t memberId) {
        const int64_t now = OSUtils::now();
        Revocation rev((uint32_t) _node->prng(), networkId, 0, now,
                       ZT_REVOCATION_FLAG_FAST_PROPAGATE, Address(memberId),
    ...
    void EmbeddedNetworkController::_request(
        ...
        if ((!network.is_object()) || (network.empty())) {
            _sender->ncSendError(nwid, requestPacketId, identity.address(),
                                 NetworkController::NC_ERROR_OBJECT_NOT_FOUND, nullptr, 0);
#ifdef CENTRAL_CONTROLLER_REQUEST_BENCHMARK
            b3.stop();
#endif
            return;
            ...
            if (haveIdStr.length() > 0) {
                ...
                try {
                    if (Identity(haveIdStr.c_str()) != identity) {
                        _sender->ncSendError(nwid, requestPacketId, identity.address(),
                                             NetworkController::NC_ERROR_ACCESS_DENIED, nullptr, 0);
#ifdef CENTRAL_CONTROLLER_REQUEST_BENCHMARK
                        b4.stop();
#endif
                        return;
        ...
        if (OSUtils::jsonBool(member["authorized"], false)) {
            ...
            if (metaData.get(ZT_NETWORKCONFIG_REQUEST_METADATA_KEY_AUTH, presentedAuth,
                             sizeof(presentedAuth)) > 0) {
        ...
        if (networkSSOEnabled && !memberSSOExempt) {
            authenticationExpiryTime = (int64_t) OSUtils::jsonInt(
                    member["authenticationExpiryTime"], 0);
            ...
            if (authenticationExpiryTime <= now) {
                if (info.version == 0) {
                    ...
                    authInfo.add(ZT_AUTHINFO_DICT_KEY_AUTHENTICATION_URL,
                                 info.authenticationURL.c_str());
                    _sender->ncSendError(nwid, requestPacketId, identity.address(),
                                         NetworkController::NC_ERROR_AUTHENTICATION_REQUIRED,
                    ...
                    authInfo.add(ZT_AUTHINFO_DICT_KEY_CENTRAL_ENDPOINT_URL,
        ...
        if (authorized) {
            ...
            if (requestPacketId) {
                const uint64_t vMajor = metaData.getUI(
                        ZT_NETWORKCONFIG_REQUEST_METADATA_KEY_NODE_MAJOR_VERSION, 0);
                const uint64_t vMinor = metaData.getUI(
                        ZT_NETWORKCONFIG_REQUEST_METADATA_KEY_NODE_MINOR_VERSION, 0);
                const uint64_t vRev = metaData.getUI(
                        ZT_NETWORKCONFIG_REQUEST_METADATA_KEY_NODE_REVISION, 0);
                const uint64_t vProto = metaData.getUI(
                        ZT_NETWORKCONFIG_REQUEST_METADATA_KEY_PROTOCOL_VERSION, 0);
                ...
                if (authenticationExpiryTime > 0) {
                    ...
                    _expiringSoon.insert(
                            std::pair<int64_t, _MemberStatusKey>(authenticationExpiryTime, msk));
                }
            }
        } else {
        ...
        nc->networkId = nwid;
        nc->type = OSUtils::jsonBool(network["private"], true) ? ZT_NETWORK_TYPE_PRIVATE
        ...
        nc->mtu = std::max(std::min((unsigned int) OSUtils::jsonInt(network["mtu"], ZT_DEFAULT_MTU),
                                    (unsigned int) ZT_MAX_MTU), (unsigned int) ZT_MIN_MTU);
        ...
        if (info.version == 0) {
            nc->authenticationExpiryTime = OSUtils::jsonInt(member["authenticationExpiryTime"],
                                                            0LL);
            if (!info.authenticationURL.empty()) {
                Utils::scopy(nc->authenticationURL, sizeof(nc->authenticationURL),
                             info.authenticationURL.c_str());
            }
        } else if (info.version == 1) {
            nc->authenticationExpiryTime = OSUtils::jsonInt(member["authenticationExpiryTime"],
            ...
            if (!info.centralAuthURL.empty()) {
                Utils::scopy(nc->centralAuthURL, sizeof(nc->centralAuthURL),
                             info.centralAuthURL.c_str());
            }
        ...
        if (rtt.length() == 10) {
            ...
            nc->remoteTraceLevel = (Trace::Level) OSUtils::jsonInt(member["remoteTraceLevel"],
                                                                   0ULL);
        } else {
            ...
            nc->remoteTraceLevel = (Trace::Level) OSUtils::jsonInt(network["remoteTraceLevel"],
                                                                   0ULL);
        }

        for (std::vector<Address>::const_iterator ab(ns.activeBridges.begin());
             ab != ns.activeBridges.end(); ++ab) {
        ...
        if (metaData.getUI(ZT_NETWORKCONFIG_REQUEST_METADATA_KEY_RULES_ENGINE_REV, 0) <= 0) {
                    ...
                    if (cap.is_object()) {
                        ...
                        if ((newMember) && (OSUtils::jsonBool(cap["default"], false))) {
                            ...
                            for (unsigned long i = 0; i < memberCapabilities.size(); ++i) {
                                if (id == (OSUtils::jsonInt(memberCapabilities[i], 0ULL) &
                                           0xffffffffULL)) {
                                    have = true;
                                    break;
                                }
            ...
            for (unsigned long i = 0; i < memberCapabilities.size(); ++i) {
                const uint64_t capId =
                        OSUtils::jsonInt(memberCapabilities[i], 0ULL) & 0xffffffffULL;
                ...
                if (ctmp != capsById.end()) {
                    ...
                    if ((cap) && (cap->is_object()) && (!cap->empty())) {
                        ...
                        nc->capabilities[nc->capabilityCount] = Capability((uint32_t) capId, nwid,
                                                                           now, 1, capr, caprc);
                        if (nc->capabilities[nc->capabilityCount].sign(_signingId,
                                                                       identity.address()))
                            ++nc->capabilityCount;
            ...
            if (memberTags.is_array()) {
                for (unsigned long i = 0; i < memberTags.size(); ++i) {
                    ...
                    if ((t.is_array()) && (t.size() == 2))
                        memberTagsById[(uint32_t) (OSUtils::jsonInt(t[0], 0ULL) &
                                                   0xffffffffULL)] = (uint32_t) (
                                OSUtils::jsonInt(t[1], 0ULL) & 0xffffffffULL);
                }
            }
            if (tags.is_array()) { // check network tags array for defaults that are not present in member tags
                for (unsigned long i = 0; i < tags.size(); ++i) {
                    json &t = tags[i];
                    if (t.is_object()) {
                        const uint32_t id = (uint32_t) (OSUtils::jsonInt(t["id"], 0) &
                                                        0xffffffffULL);
                        json &dfl = t["default"];
                        if ((dfl.is_number()) &&
                            (memberTagsById.find(id) == memberTagsById.end())) {
                            memberTagsById[id] = (uint32_t) (OSUtils::jsonInt(dfl, 0) &
                                                             0xffffffffULL);
                            json mt = json::array();
                            mt.push_back(id);
                            mt.push_back(dfl);
            ...
            for (std::map<uint32_t, uint32_t>::const_iterator t(memberTagsById.begin());
                 t != memberTagsById.end(); ++t) {
        ...
        if ((v6AssignMode.is_object()) && (!noAutoAssignIps)) {
            if ((OSUtils::jsonBool(v6AssignMode["rfc4193"], false)) &&
                (nc->staticIpCount < ZT_MAX_ZT_ASSIGNED_ADDRESSES)) {
                nc->staticIps[nc->staticIpCount++] = InetAddress::makeIpv6rfc4193(nwid,
                                                                                  identity.address().toInt());
                nc->flags |= ZT_NETWORKCONFIG_FLAG_ENABLE_IPV6_NDP_EMULATION;
            }
            if ((OSUtils::jsonBool(v6AssignMode["6plane"], false)) &&
                (nc->staticIpCount < ZT_MAX_ZT_ASSIGNED_ADDRESSES)) {
                nc->staticIps[nc->staticIpCount++] = InetAddress::makeIpv66plane(nwid,
                                                                                 identity.address().toInt());
                nc->flags |= ZT_NETWORKCONFIG_FLAG_ENABLE_IPV6_NDP_EMULATION;
            }
        }
            ...
            for (unsigned long i = 0; i < ipAssignments.size(); ++i) {
                if (ipAssignments[i].is_string()) {
                    ...
                    for (unsigned int rk = 0; rk < nc->routeCount; ++rk) {
                        if (reinterpret_cast<const InetAddress *>(&(nc->routes[rk].target))->containsAddress(
                                ip)) {
        ...
        if ((ipAssignmentPools.is_array()) &&
            ((v6AssignMode.is_object()) && (OSUtils::jsonBool(v6AssignMode["zt"], false))) &&
            (!haveManagedIpv6AutoAssignment) && (!noAutoAssignIps)) {
            for (unsigned long p = 0; ((p < ipAssignmentPools.size()) &&
                                       (!haveManagedIpv6AutoAssignment)); ++p) {
                ...
                if (pool.is_object()) {
                    ...
                    if ((ipRangeStart.ss_family == AF_INET6) &&
                        (ipRangeEnd.ss_family == AF_INET6)) {
                        ...
                        for (unsigned int trialCount = 0; trialCount < 1000; ++trialCount) {
                            if ((trialCount == 0) && (e[1] > s[1]) &&
                                ((e[1] - s[1]) >= 0xffffffffffULL)) {
                            ...
                            for (unsigned int rk = 0; rk < nc->routeCount; ++rk) {
                                if ((!nc->routes[rk].via.ss_family) &&
                                    (nc->routes[rk].target.ss_family == AF_INET6) &&
                                    (reinterpret_cast<const InetAddress *>(&(nc->routes[rk].target))->containsAddress(
                                            ip6)))
                            ...
                            if ((routedNetmaskBits > 0) &&
                                (!std::binary_search(ns.allocatedIps.begin(), ns.allocatedIps.end(),
                                                     ip6))) {
                                char tmpip[64];
                                const std::string ipStr(ip6.toIpString(tmpip));
                                if (std::find(ipAssignments.begin(), ipAssignments.end(), ipStr) ==
                                    ipAssignments.end()) {
        ...
        if ((ipAssignmentPools.is_array()) &&
            ((v4AssignMode.is_object()) && (OSUtils::jsonBool(v4AssignMode["zt"], false))) &&
            (!haveManagedIpv4AutoAssignment) && (!noAutoAssignIps)) {
            for (unsigned long p = 0; ((p < ipAssignmentPools.size()) &&
                                       (!haveManagedIpv4AutoAssignment)); ++p) {
                json &pool = ipAssignmentPools[p];
                if (pool.is_object()) {
                    InetAddress ipRangeStartIA(
                            OSUtils::jsonString(pool["ipRangeStart"], "").c_str());
                    InetAddress ipRangeEndIA(OSUtils::jsonString(pool["ipRangeEnd"], "").c_str());
                    if ((ipRangeStartIA.ss_family == AF_INET) &&
                        (ipRangeEndIA.ss_family == AF_INET)) {
                        uint32_t ipRangeStart = Utils::ntoh(
                                (uint32_t) (reinterpret_cast<struct sockaddr_in *>(&ipRangeStartIA)->sin_addr.s_addr));
                        uint32_t ipRangeEnd = Utils::ntoh(
                                (uint32_t) (reinterpret_cast<struct sockaddr_in *>(&ipRangeEndIA)->sin_addr.s_addr));
                        ...
                        uint32_t ipTrialCounter = (uint32_t) (identity.address().toInt() &
                                                              0xffffffff);

                        for (uint32_t k = ipRangeStart, trialCount = 0; ((k <= ipRangeEnd) &&
                                                                         (trialCount <
                                                                          1000)); ++k, ++trialCount) {
                            uint32_t ip = (ipRangeLen > 0) ? (ipRangeStart +
                                                              (ipTrialCounter % ipRangeLen))
                                                           : ipRangeStart;
                            ++ipTrialCounter;
                            ...
                            for (unsigned int rk = 0; rk < nc->routeCount; ++rk) {
                                if (nc->routes[rk].target.ss_family == AF_INET) {
                                    uint32_t targetIp = Utils::ntoh(
                                            (uint32_t) (reinterpret_cast<const struct sockaddr_in *>(&(nc->routes[rk].target))->sin_addr.s_addr));
                                    int targetBits = Utils::ntoh(
                                            (uint16_t) (reinterpret_cast<const struct sockaddr_in *>(&(nc->routes[rk].target))->sin_port));
...
#endif
        if (nc->staticIpCount) {
            nc->certificatesOfOwnership[0] = CertificateOfOwnership(nwid, now, identity.address(),
                                                                    1);
        ...
        if (com.sign(_signingId)) {
            ...
            _sender->ncSendError(nwid, requestPacketId, identity.address(),
                                 NetworkController::NC_ERROR_INTERNAL_SERVER_ERROR, nullptr, 0);
#ifdef CENTRAL_CONTROLLER_REQUEST_BENCHMARK
            b9.stop();
#endif
            return;
        ...
        b11.start();
#endif
        _sender->ncSendConfig(nwid, requestPacketId, identity.address(), *(nc.get()),
            ...
            _threads.emplace_back([this]() {
                ...
                for (;;) {
                    ...
                    if (timedWaitResult == BlockingQueue<_RQEntry *>::STOP) {
                        ...
                        if (qe) {
                            try {
                                _request(qe->nwid, qe->fromAddr, qe->requestPacketId, qe->identity,
                                         qe->metaData);
                            } catch (std::exception &e) {
                                fprintf(stderr,
                                        "ERROR: exception in controller request handling thread: %s" ZT_EOL_S,
                                        e.what());
                            } catch (...) {
                                fprintf(stderr,
                ...
                for (auto s = _expiringSoon.begin(); s != _expiringSoon.end();) {
                    ...
                    if (when <= now) {
                        ...
                        if (_db.get(s->second.networkId, network, s->second.nodeId, member)) {
                            int64_t authenticationExpiryTime = (int64_t) OSUtils::jsonInt(
                                    member["authenticationExpiryTime"], 0);
                            if (authenticationExpiryTime <= now) {
