...
jobject createResultObject(JNIEnv *env, ZT_ResultCode code) {
    jobject resultObject = env->CallStaticObjectMethod(ResultCode_class, ResultCode_fromInt_method,
                                                       (jint) code);
...
jobject createVirtualNetworkStatus(JNIEnv *env, ZT_VirtualNetworkStatus status) {
    jobject statusObject = env->CallStaticObjectMethod(VirtualNetworkStatus_class,
                                                       VirtualNetworkStatus_fromInt_method,
                                                       (jint) status);
...
jobject createEvent(JNIEnv *env, ZT_Event event) {
    jobject eventObject = env->CallStaticObjectMethod(Event_class, Event_fromInt_method,
                                                      (jint) event);
...
jobject createPeerRole(JNIEnv *env, ZT_PeerRole role) {
    jobject peerRoleObject = env->CallStaticObjectMethod(PeerRole_class, PeerRole_fromInt_method,
                                                         (jint) role);
...
jobject createVirtualNetworkType(JNIEnv *env, ZT_VirtualNetworkType type) {
    jobject vntypeObject = env->CallStaticObjectMethod(VirtualNetworkType_class,
                                                       VirtualNetworkType_fromInt_method,
                                                       (jint) type);
...
jobject createVirtualNetworkConfigOperation(JNIEnv *env, ZT_VirtualNetworkConfigOperation op) {
    jobject vnetConfigOpObject = env->CallStaticObjectMethod(VirtualNetworkConfigOperation_class,
                                                             VirtualNetworkConfigOperation_fromInt_method,
                                                             (jint) op);
...
jobject newInetSocketAddress(JNIEnv *env, const sockaddr_storage &addr) {
    ...
    jobject inetSocketAddressObject = env->NewObject(InetSocketAddress_class,
                                                     InetSocketAddress_ctor,
                                                     (jobject) inetAddressObject, (jint) port);
...
jobject newNetworkConfig(JNIEnv *env, const ZT_VirtualNetworkConfig &vnetConfig) {
    ...
    jobjectArray assignedAddrArrayObj = newInetSocketAddressArray(env, vnetConfig.assignedAddresses,
                                                                  vnetConfig.assignedAddressCount);
    if (env->ExceptionCheck() || assignedAddrArrayObj == NULL) {
        return NULL;
    }

    jobjectArray routesArrayObj = newVirtualNetworkRouteArray(env, vnetConfig.routes,
                                                              vnetConfig.routeCount);
...
jobject newVersion(JNIEnv *env, int major, int minor, int rev) {
    ...
    jobject versionObj = env->NewObject(Version_class, Version_ctor, (jint) major, (jint) minor,
                                        (jint) rev);
...
jobjectArray
newVirtualNetworkConfigArray(JNIEnv *env, const ZT_VirtualNetworkConfig *networks, size_t count) {
    return newArrayObject<ZT_VirtualNetworkConfig, newNetworkConfig>(env, networks, count,
                                                                     VirtualNetworkConfig_class);
}

jobjectArray newPeerPhysicalPathArray(JNIEnv *env, const ZT_PeerPhysicalPath *paths, size_t count) {
    return newArrayObject<ZT_PeerPhysicalPath, newPeerPhysicalPath>(env, paths, count,
                                                                    PeerPhysicalPath_class);
}

jobjectArray
newInetSocketAddressArray(JNIEnv *env, const sockaddr_storage *addresses, size_t count) {
    return newArrayObject<sockaddr_storage, newInetSocketAddress>(env, addresses, count,
                                                                  InetSocketAddress_class);
}

jobjectArray
newVirtualNetworkRouteArray(JNIEnv *env, const ZT_VirtualNetworkRoute *routes, size_t count) {
    return newArrayObject<ZT_VirtualNetworkRoute, newVirtualNetworkRoute>(env, routes, count,
                                                                          VirtualNetworkRoute_class);
}
...
sockaddr_storage fromSocketAddressObject(JNIEnv *env, jobject sockAddressObject) {
    ...
    jobject addressObject = env->CallObjectMethod(sockAddressObject,
                                                  InetSocketAddress_getAddress_method);
    if (env->ExceptionCheck() || addressObject == NULL) {
        LOGE("Exception calling getAddress");
        return emptyAddress;
    }

    jbyteArray addressArrayObj = reinterpret_cast<jbyteArray>(env->CallObjectMethod(addressObject,
                                                                                    InetAddress_getAddress_method));
