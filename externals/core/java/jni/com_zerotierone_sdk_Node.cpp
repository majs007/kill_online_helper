...
namespace {
    struct JniRef {
        JniRef(
                ...
                : id(id), jvm(jvm), node(), dataStoreGetListener(), dataStorePutListener(),
                  packetSender(), eventListener(), frameListener(), configListener(), pathChecker(),
                  inited() {
    ...
    */
    ...
    void VirtualNetworkFrameFunctionCallback(ZT_Node *node,
        ...
        env->CallVoidMethod(ref->frameListener,
                            VirtualNetworkFrameListener_onVirtualNetworkFrame_method, (jlong) nwid,
                            (jlong) sourceMac, (jlong) destMac, (jlong) etherType, (jlong) vlanid,
    ...
    void EventCallback(ZT_Node *node,
        ...
        switch (event) {
            case ZT_EVENT_UP: {
                LOGD("Event Up");
                env->CallVoidMethod(ref->eventListener, EventListener_onEvent_method,
                                    (jobject) eventObject);
                if (env->ExceptionCheck()) {
            ...
            case ZT_EVENT_TRACE: {
                ...
                env->CallVoidMethod(ref->eventListener, EventListener_onTrace_method,
                                    (jstring) messageStr);
                if (env->ExceptionCheck()) {
    ...
    void StatePutFunction(
        ...
        if (bufferLength >= 0) {
            ...
            int retval = env->CallIntMethod(ref->dataStorePutListener,
                                            ...
                                            (jstring) nameStr, (jbyteArray) bufferObj,
                                            (jboolean) secure);
            if (env->ExceptionCheck()) {
            ...
            int retval = env->CallIntMethod(ref->dataStorePutListener,
                                            DataStorePutListener_onDelete_method,
                                            (jstring) nameStr);
            if (env->ExceptionCheck()) {
    ...
    int WirePacketSendFunction(ZT_Node *node,
                               ...
                               unsigned int ttl) {
        LOGV("WirePacketSendFunction(%" PRId64 ", %p, %p, %u, %u)", localSocket, remoteAddress,
             buffer, bufferSize, ttl);
        JniRef *ref = (JniRef *) userData;
        assert(ref);
        assert(ref->node == node);
        ...
        int retval = env->CallIntMethod(ref->packetSender,
                                        PacketSender_onSendPacketRequested_method,
                                        (jlong) localSocket, (jobject) remoteAddressObj,
                                        (jbyteArray) bufferObj, (jint) 0);
        if (env->ExceptionCheck()) {
    ...
    int PathCheckFunction(ZT_Node *node,
        ...
        jboolean ret = env->CallBooleanMethod(ref->pathChecker, PathChecker_onPathCheck_method,
                                              (jlong) address, (jlong) localSocket,
                                              (jobject) remoteAddressObj);
        if (env->ExceptionCheck()) {
    ...
    int PathLookupFunction(ZT_Node *node,
        ...
        // may be NULL
        //
        jobject sockAddressObject = env->CallObjectMethod(ref->pathChecker,
                                                          PathChecker_onPathLookup_method,
                                                          (jlong) address, (jint) ss_family);
        if (env->ExceptionCheck()) {
...
JNIEXPORT jobject JNICALL Java_com_zerotier_sdk_Node_processVirtualNetworkFrame(
    ...
    jlong *outDeadline = (jlong *) env->GetPrimitiveArrayCritical(out_nextBackgroundTaskDeadline,
                                                                  NULL);
...
JNIEXPORT jobjectArray JNICALL Java_com_zerotier_sdk_Node_networkConfigs(
    ...
    jobjectArray networkListObject = newVirtualNetworkConfigArray(env, networkList->networks,
                                                                  networkList->networkCount);
