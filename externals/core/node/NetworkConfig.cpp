...
namespace ZeroTier {
    ...
    bool NetworkConfig::toDictionary(Dictionary<ZT_NETWORKCONFIG_DICT_CAPACITY> &d,
        ...
        try {
            ...
            if (!d.add(ZT_NETWORKCONFIG_DICT_KEY_CREDENTIAL_TIME_MAX_DELTA,
            ...
            if (!d.add(ZT_NETWORKCONFIG_DICT_KEY_REMOTE_TRACE_TARGET,
                       this->remoteTraceTarget.toString(tmp2))) {
                delete tmp;
                return false;
            }
            if (!d.add(ZT_NETWORKCONFIG_DICT_KEY_REMOTE_TRACE_LEVEL,
                       (uint64_t) this->remoteTraceLevel)) {
                delete tmp;
                return false;
            }
            ...
            if (!d.add(ZT_NETWORKCONFIG_DICT_KEY_MULTICAST_LIMIT,
                       (uint64_t) this->multicastLimit)) {
                delete tmp;
                return false;
            }
            ...
            if (this->ssoVersion == 0) {
                ...
                if (this->ssoEnabled) {
                    ...
                    if (!d.add(ZT_NETWORKCONFIG_DICT_KEY_AUTHENTICATION_EXPIRY_TIME,
                               this->authenticationExpiryTime)) {
                        return false;
                    }
                }
    ...
    bool NetworkConfig::fromDictionary(const Dictionary<ZT_NETWORKCONFIG_DICT_CAPACITY> &d) {
        ...
        try {
            *this = NIL_NC;
            ...
            this->credentialTimeMaxDelta = d.getUI(
                    ZT_NETWORKCONFIG_DICT_KEY_CREDENTIAL_TIME_MAX_DELTA, 0);
            ...
            this->remoteTraceLevel = (Trace::Level) d.getUI(
                    ZT_NETWORKCONFIG_DICT_KEY_REMOTE_TRACE_LEVEL);
            this->multicastLimit = (unsigned int) d.getUI(ZT_NETWORKCONFIG_DICT_KEY_MULTICAST_LIMIT,
                                                          0);
            ...
            if (d.getUI(ZT_NETWORKCONFIG_DICT_KEY_VERSION, 0) < 6) {
                ...
                this->type = (ZT_VirtualNetworkType) d.getUI(ZT_NETWORKCONFIG_DICT_KEY_TYPE,
                                                             (uint64_t) ZT_NETWORK_TYPE_PRIVATE);
                ...
                if (d.get(ZT_NETWORKCONFIG_DICT_KEY_CAPABILITIES, *tmp)) {
                    ...
                    std::sort(&(this->capabilities[0]),
                              &(this->capabilities[this->capabilityCount]));
                }
                ...
                if (d.get(ZT_NETWORKCONFIG_DICT_KEY_CERTIFICATES_OF_OWNERSHIP, *tmp)) {
                    ...
                    while (p < tmp->size()) {
                        if (certificateOfOwnershipCount < ZT_MAX_CERTIFICATES_OF_OWNERSHIP) {
                            p += certificatesOfOwnership[certificateOfOwnershipCount++].deserialize(
                                    *tmp, p);
                        } else {
                ...
                if (d.get(ZT_NETWORKCONFIG_DICT_KEY_ROUTES, *tmp)) {
                    ...
                    while ((p < tmp->size()) && (routeCount < ZT_MAX_NETWORK_ROUTES)) {
                        p += reinterpret_cast<InetAddress *>(&(this->routes[this->routeCount].target))->deserialize(
                                *tmp, p);
                        p += reinterpret_cast<InetAddress *>(&(this->routes[this->routeCount].via))->deserialize(
                                *tmp, p);
                ...
                if (d.get(ZT_NETWORKCONFIG_DICT_KEY_RULES, *tmp)) {
                    this->ruleCount = 0;
                    unsigned int p = 0;
                    Capability::deserializeRules(*tmp, p, this->rules, this->ruleCount,
                ...
                if (this->ssoVersion == 0) {
                    // implicit flow
                    if (this->ssoEnabled) {
                        if (d.get(ZT_NETWORKCONFIG_DICT_KEY_AUTHENTICATION_URL,
                                  this->authenticationURL,
                                  (unsigned int) sizeof(this->authenticationURL)) > 0) {
                            this->authenticationURL[sizeof(this->authenticationURL) -
                                                    1] = 0; // ensure null terminated
                        } else {
                            this->authenticationURL[0] = 0;
                        }
                        this->authenticationExpiryTime = d.getI(
                                ZT_NETWORKCONFIG_DICT_KEY_AUTHENTICATION_EXPIRY_TIME, 0);
                    } else {
                        this->authenticationURL[0] = 0;
                    ...
                    if (this->ssoEnabled) {
                        ...
                        if (d.get(ZT_NETWORKCONFIG_DICT_KEY_ISSUER_URL, this->issuerURL,
                                  (unsigned int) sizeof(this->issuerURL)) > 0) {
                            this->issuerURL[sizeof(this->issuerURL) - 1] = 0;
                        }
                        if (d.get(ZT_NETWORKCONFIG_DICT_KEY_CENTRAL_ENDPOINT_URL,
                                  this->centralAuthURL,
                                  (unsigned int) sizeof(this->centralAuthURL)) > 0) {
                            this->centralAuthURL[sizeof(this->centralAuthURL) - 1] = 0;
                        }
                        if (d.get(ZT_NETWORKCONFIG_DICT_KEY_NONCE, this->ssoNonce,
                                  (unsigned int) sizeof(this->ssoNonce)) > 0) {
                            this->ssoNonce[sizeof(this->ssoNonce) - 1] = 0;
                        }
                        if (d.get(ZT_NETWORKCONFIG_DICT_KEY_STATE, this->ssoState,
                                  (unsigned int) sizeof(this->ssoState)) > 0) {
                            this->ssoState[sizeof(this->ssoState) - 1] = 0;
                        }
                        if (d.get(ZT_NETWORKCONFIG_DICT_KEY_CLIENT_ID, this->ssoClientID,
                                  (unsigned int) sizeof(this->ssoClientID)) > 0) {
                            this->ssoClientID[sizeof(this->ssoClientID) - 1] = 0;
                        }
                        if (d.get(ZT_NETWORKCONFIG_DICT_KEY_SSO_PROVIDER, this->ssoProvider,
                                  (unsigned int) (sizeof(this->ssoProvider))) > 0) {
