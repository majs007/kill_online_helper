...
#define RND(a, b, c, d, e, f, g, h, i) \
    ...
    void SHA384(void *digest, const void *data0, unsigned int len0, const void *data1,
                unsigned int len1) {
        uint8_t tmp[64];
        sha512_state state;
        sha384_init(&state);
    ...
    void
    HMACSHA384(const uint8_t key[ZT_SYMMETRIC_KEY_SIZE], const void *msg, const unsigned int msglen,
               uint8_t mac[48]) {
    ...
    void
    KBKDFHMACSHA384(const uint8_t key[ZT_SYMMETRIC_KEY_SIZE], const char label, const char context,
                    const uint32_t iter, uint8_t out[ZT_SYMMETRIC_KEY_SIZE]) {
        uint8_t kbkdfMsg[13];
        ...
        static_assert(ZT_SYMMETRIC_KEY_SIZE == ZT_SHA384_DIGEST_SIZE,
                      "sizeof(out) != ZT_SHA384_DIGEST_SIZE");
