...
namespace ZeroTier {
    ...
    void Bond::addCustomLink(std::string &policyAlias, SharedPtr<Link> link) {
        ...
        if (search == _interfaceToLinkMap[policyAlias].end()) {
            ...
            _interfaceToLinkMap[policyAlias].insert(
                    std::pair<std::string, SharedPtr<Link> >(link->ifname(), link));
        }
    }
        ...
        if (!_bonds.count(identity)) {
            if (!_policyTemplateAssignments.count(identity)) {
                ...
                if (!_defaultPolicy && _defaultPolicyStr.length()) {
                    ...
                    bond->debug("new default custom bond (based on %s)",
                                bond->getPolicyStrByCode(bond->policy()).c_str());
                }
            } else {
                if (!_bondPolicyTemplates[_policyTemplateAssignments[identity]]) {
                    bond = new Bond(renv, _defaultPolicy, peer);
                    bond->debug(
                            "peer-specific bond, was specified as %s but the bond definition was not found, using default %s",
                            _policyTemplateAssignments[identity].c_str(),
                            getPolicyStrByCode(_defaultPolicy).c_str());
                } else {
                    bond = new Bond(renv,
                                    _bondPolicyTemplates[_policyTemplateAssignments[identity]].ptr(),
                                    peer);
                    bond->debug("new default bond");
                }
            }
        ...
        if (bond) {
            ...
            if (_interfaceToLinkMap.count(bond->policyAlias())) {
                ...
                while (it != _interfaceToLinkMap[bond->policyAlias()].end()) {
                    ...
                    if (it->second->isUserSpecified() &&
                        it->second->userHasSpecifiedFailoverInstructions()) {
        ...
        if (search == _interfaceToLinkMap[policyAlias].end()) {
            if (createIfNeeded) {
                SharedPtr<Link> s = new Link(ifnameStr, 0, 0, 0, true, ZT_BOND_SLAVE_MODE_PRIMARY,
                                             "");
                _interfaceToLinkMap[policyAlias].insert(
                        std::pair<std::string, SharedPtr<Link> >(ifnameStr, s));
                return s;
            } else {
    ...
    void Bond::processBackgroundTasks(void *tPtr, const int64_t now) {
        ...
        while (bondItr != _bonds.end()) {
            ...
            _currMinReqMonitorInterval = std::min(_currMinReqMonitorInterval,
                                                  (unsigned long) (bondItr->second->monitorInterval()));
            bondItr->second->processBackgroundBondTasks(tPtr, now);
            ++bondItr;
        }
        _minReqMonitorInterval = std::min(_currMinReqMonitorInterval,
                                          (unsigned long) ZT_BOND_FAILOVER_DEFAULT_INTERVAL);
    }
    ...
    Bond::Bond(const RuntimeEnvironment *renv, int policy, const SharedPtr<Peer> &peer) : RR(renv),
                                                                                          _freeRandomByte(
                                                                                                  (unsigned char) (
                                                                                                          (uintptr_t) this
                                                                                                                  >> 4) ^
                                                                                                  ++s_freeRandomByteCounter),
                                                                                          _peer(peer),
    ...
    Bond::Bond(const RuntimeEnvironment *renv, std::string &basePolicy, std::string &policyAlias,
               const SharedPtr<Peer> &peer) : RR(renv), _policyAlias(policyAlias), _peer(peer) {
        initTimers();
        setBondParameters(getPolicyCodeByStr(basePolicy), SharedPtr<Bond>(), false);
    }

    Bond::Bond(const RuntimeEnvironment *renv, SharedPtr<Bond> originalBond,
               const SharedPtr<Peer> &peer)
            : RR(renv),
              _freeRandomByte((unsigned char) ((uintptr_t) this >> 4) ^ ++s_freeRandomByteCounter),
              _peer(peer), _peerId(_peer->_id.address().toInt()) {
        initTimers();
    ...
    void Bond::nominatePathToBond(const SharedPtr<Path> &path, int64_t now) {
         ...
         */
        if (!RR->bc->linkAllowed(_policyAlias,
                                 getLinkBySocket(_policyAlias, path->localSocket(), true))) {
            debug("link %s is not allowed according to user-specified rules",
                  pathToStr(path).c_str());
            return;
        }
        ...
        if (!alreadyPresent) {
            ...
            if (link) {
                ...
                memcpy(path->_ifname, ifnameStr.c_str(),
                       std::min((int) ifnameStr.length(), ZT_MAX_PHYSIFNAME));
            }
            /**
            ...
            for (int i = 0; i < ZT_MAX_PEER_NETWORK_PATHS; ++i) {
                if (!_paths[i].p) {
                    ...
                    if (sl) {
                        ...
                        SharedPtr<Link> commonLink = RR->bc->getLinkBySocket(_policyAlias,
                                                                             _paths[i].p->localSocket());
                        if (commonLink) {
                            for (unsigned int j = 0; j < ZT_MAX_PEER_NETWORK_PATHS; ++j) {
                                if (_paths[j].p && _paths[j].p.ptr() != _paths[i].p.ptr()) {
                                    if (RR->bc->getLinkBySocket(_policyAlias,
                                                                _paths[j].p->localSocket(), true) ==
                                        commonLink) {
            ...
            if (_numBondedPaths == 1 || _rrIdx >= (ZT_MAX_PEER_NETWORK_PATHS - 1)) {
                ...
                for (int searchCount = 0; searchCount < (_numBondedPaths - 1); searchCount++) {
                    ...
                    if (_realIdxMap[_tempIdx] != ZT_MAX_PEER_NETWORK_PATHS) {
                        if (_paths[_realIdxMap[_tempIdx]].p &&
                            _paths[_realIdxMap[_tempIdx]].eligible) {
                            _rrIdx = _tempIdx;
                            break;
                        }
                    }
    ...
    void Bond::recordOutgoingPacket(const SharedPtr<Path> &path, uint64_t packetId,
        ...
        _freeRandomByte += (unsigned char) (packetId
                >> 8);     // Grab entropy to use in path selection logic
        bool isFrame = (verb == Packet::Packet::VERB_ECHO || verb == Packet::VERB_FRAME ||
                        verb == Packet::VERB_EXT_FRAME);
        bool shouldRecord = (packetId & (ZT_QOS_ACK_DIVISOR - 1) && (verb != Packet::VERB_ACK) &&
    ...
    void Bond::recordIncomingPacket(const SharedPtr<Path> &path, uint64_t packetId,
                                    uint16_t payloadLength, Packet::Verb verb, int32_t flowId,
                                    int64_t now) {
        bool isFrame = (verb == Packet::Packet::VERB_ECHO || verb == Packet::VERB_FRAME ||
                        verb == Packet::VERB_EXT_FRAME);
        bool shouldRecord = (packetId & (ZT_QOS_ACK_DIVISOR - 1) && (verb != Packet::VERB_ACK) &&
         ...
         */
        if ((flowId != ZT_QOS_NO_FLOW) &&
            (_policy == ZT_BOND_POLICY_BALANCE_RR || _policy == ZT_BOND_POLICY_BALANCE_XOR ||
             _policy == ZT_BOND_POLICY_BALANCE_AWARE)) {
            Mutex::Lock _l(_flows_m);
            SharedPtr<Flow> flow;
            if (!_flows.count(flowId)) {
    ...
    void Bond::receivedQoS(const SharedPtr<Path> &path, int64_t now, int count, uint64_t *rx_id,
                           uint16_t *rx_ts) {
        Mutex::Lock _l(_paths_m);
        int pathIdx = getNominatedPathIdx(path);
        if (pathIdx == ZT_MAX_PEER_NETWORK_PATHS) {
    ...
    bool Bond::assignFlowToBondedPath(SharedPtr<Flow> &flow, int64_t now, bool reassign = false) {
        ...
        if (_policy == ZT_BOND_POLICY_BALANCE_AWARE) {
            ...
            if (reassign) {
                log("attempting to re-assign out-flow %04x previously on idx %d (%u / %zu flows)",
                    flow->id, flow->assignedPath,
                    _paths[_realIdxMap[flow->assignedPath]].assignedFlowCount, _flows.size());
            } else {
            ...
            unsigned char entropy;
            Utils::getSecureRandom(&entropy, 1);
            float randomLinkCapacity = ((float) entropy /
                                        255.0);   // Used to random but proportional choices
            ...
            while (offset < _numBondedPaths) {
                ...
                if (!_paths[_realIdxMap[bondedIdx]].shouldAvoid &&
                    randomLinkCapacity <= _paths[_realIdxMap[bondedIdx]].relativeLinkCapacity) {
        ...
        log("assign out-flow %04x to link %s (%u / %zu flows)", flow->id,
            pathToStr(_paths[flow->assignedPath].p).c_str(),
            _paths[flow->assignedPath].assignedFlowCount, _flows.size());
        return true;
    }

    SharedPtr<Bond::Flow>
    Bond::createFlow(int pathIdx, int32_t flowId, unsigned char entropy, int64_t now) {
        if (!_numBondedPaths) {
        ...
        if (pathIdx != ZT_MAX_PEER_NETWORK_PATHS) {
            ...
            debug("assign in-flow %04x to link %s (%u / %zu)", flow->id,
                  pathToStr(_paths[pathIdx].p).c_str(), _paths[pathIdx].assignedFlowCount,
                  _flows.size());
        }
            /**
        ...
        if (age) {     // Remove by specific age
            while (it != _flows.end()) {
                if (it->second->age(now) > age) {
                    debug("forget flow %04x (age %" PRId64 ") (%u / %zu)", it->first,
                          it->second->age(now), _paths[it->second->assignedPath].assignedFlowCount,
                          (_flows.size() - 1));
            ...
            if (oldestFlow != _flows.end()) {
                debug("forget oldest flow %04x (age %" PRId64 ") (total flows: %zu)",
                      oldestFlow->first, oldestFlow->second->age(now), _flows.size() - 1);
    ...
    void Bond::processIncomingPathNegotiationRequest(uint64_t now, SharedPtr<Path> &path,
                                                     int16_t remoteUtility) {
        char pathStr[64] = {0};
        ...
        SharedPtr<Link> link = RR->bc->getLinkBySocket(_policyAlias,
                                                       _paths[pathIdx].p->localSocket());
        if (link) {
    ...
    void Bond::pathNegotiationCheck(void *tPtr, int64_t now) {
        ...
        if (!_peerLinksSynchronized) {
            _localUtility =
                    _paths[maxOutPathIdx].failoverScore - _paths[maxInPathIdx].failoverScore;
    ...
    void Bond::sendPATH_NEGOTIATION_REQUEST(void *tPtr, int pathIdx) {
        debug("send link negotiation request to peer via link %s, local utility is %d",
              pathToStr(_paths[pathIdx].p).c_str(), _localUtility);
        if (_abLinkSelectMethod != ZT_BOND_RESELECTION_POLICY_OPTIMIZE) {
            return;
        }
        Packet outp(_peer->_id.address(), RR->identity.address(),
                    Packet::VERB_PATH_NEGOTIATION_REQUEST);
        ...
        if (_paths[pathIdx].p->address()) {
            ...
            RR->node->putPacket(tPtr, _paths[pathIdx].p->localSocket(),
                                _paths[pathIdx].p->address(), outp.data(), outp.size());
            _overheadBytes += outp.size();
        }
    }

    void Bond::sendACK(void *tPtr, int pathIdx, int64_t localSocket, const InetAddress &atAddress,
                       int64_t now) {
        /*
    ...
    void Bond::sendQOS_MEASUREMENT(void *tPtr, int pathIdx, int64_t localSocket,
    ...
    void Bond::processBackgroundBondTasks(void *tPtr, int64_t now) {
        ...
        if (!_peer->_localMultipathSupported ||
            (now - _lastBackgroundTaskCheck) < ZT_BOND_BACKGROUND_TASK_MIN_INTERVAL) {
            return;
        }
        ...
        for (unsigned int i = 0; i < ZT_MAX_PEER_NETWORK_PATHS; ++i) {
            if (_paths[i].p && _paths[i].allowed()) {
                if (_isLeaf) {
                    if ((_monitorInterval > 0) && (((now - _paths[i].p->_lastIn) >=
                                                    (_paths[i].alive ? _monitorInterval
                                                                     : _failoverInterval)))) {
                        if ((_peer->remoteVersionProtocol() >= 5) &&
                            (!((_peer->remoteVersionMajor() == 1) &&
                               (_peer->remoteVersionMinor() == 1) &&
                               (_peer->remoteVersionRevision() == 0)))) {
                            Packet outp(_peer->address(), RR->identity.address(),
                                        Packet::VERB_ECHO);    // ECHO (this is our bond's heartbeat)
                            outp.armor(_peer->key(), true, _peer->aesKeysIfSupported());
                            RR->node->expectReplyTo(outp.packetId());
                            RR->node->putPacket(tPtr, _paths[i].p->localSocket(),
                                                _paths[i].p->address(), outp.data(), outp.size());
                    ...
                    if (_paths[i].needsToSendQoS(now, _qosSendInterval)) {
                        sendQOS_MEASUREMENT(tPtr, i, _paths[i].p->localSocket(),
                                            _paths[i].p->address(), now);
                    }
                    // ACK
                    /*
        ...
        if (((now - _lastPathNegotiationCheck) > ZT_BOND_OPTIMIZE_INTERVAL) &&
            _allowPathNegotiation) {
    ...
    void Bond::curateBond(int64_t now, bool rebuildBond) {
        ...
        for (unsigned int i = 0; i < ZT_MAX_PEER_NETWORK_PATHS; ++i) {
            ...
            if ((now - _paths[i].lastEligibility) > (ZT_PEER_EXPIRED_PATH_TRIAL_PERIOD) &&
                !inTrial) {
                log("link (%s) has expired or is invalid, removing from bond",
             ...
             * Determine aliveness
             */
            _paths[i].alive = _isLeaf ? (now - _paths[i].p->_lastIn) < _failoverInterval :
                              (now - _paths[i].p->_lastIn) < ZT_PEER_PATH_EXPIRATION;
            ...
            bool acceptableAge = _isLeaf ? (_paths[i].p->age(now) <
                                            (_failoverInterval + _downDelay)) : _paths[i].alive;
            ...
            bool acceptableQoSAge = (_paths[i].lastQoSReceived == 0 && inTrial) ||
                                    ((now - _paths[i].lastQoSReceived) <
                                     ZT_PEER_EXPIRED_PATH_TRIAL_PERIOD);
            currEligibility = _paths[i].allowed() &&
                              ((acceptableAge && satisfiedUpDelay && acceptableQoSAge) || inTrial);
            ...
            if (currEligibility != _paths[i].eligible) {
                ...
                debug("\t[%d] allowed=%d, age=%d, qa=%d, ud=%d, trial=%d", i, _paths[i].allowed(),
                      acceptableAge, acceptableQoSAge, satisfiedUpDelay, inTrial);
                dumpPathStatus(now, i);
                if (currEligibility) {
                    rebuildBond = true;
                }
                if (!currEligibility) {
                    _paths[i].adjustRefractoryPeriod(now, _defaultPathRefractoryPeriod,
                                                     !currEligibility);
                    if (_paths[i].bonded) {
                        debug("link %s was bonded, flow reallocation will occur soon",
                              pathToStr(_paths[i].p).c_str());
                        rebuildBond = true;
         ...
         */
        if ((_policy == ZT_BOND_POLICY_BALANCE_RR) || (_policy == ZT_BOND_POLICY_BALANCE_XOR) ||
            (_policy == ZT_BOND_POLICY_BALANCE_AWARE)) {
            if (!_numBondedPaths) {
                rebuildBond = true;
            }
                ...
                while (it != linkMap.end()) {
                    ...
                    if (ipvPref == 0) {
                        for (int j = 0; j < it->second.size(); j++) {
                            ...
                            addPathToBond(idx, updatedBondedPathCount);
                            ++updatedBondedPathCount;
                            debug("add %s (no user addr preference)",
                                  pathToStr(_paths[idx].p).c_str());
                        }
                    }
                    ...
                    if (ipvPref == 4 || ipvPref == 6) {
                        for (int j = 0; j < it->second.size(); j++) {
                            ...
                            if (!_paths[idx].allowed()) {
                                debug("did not add %s (user addr preference %d)",
                                      pathToStr(_paths[idx].p).c_str(), ipvPref);
                                continue;
                            }
                            addPathToBond(idx, updatedBondedPathCount);
                            ++updatedBondedPathCount;
                            debug("add path %s (user addr preference %d)",
                                  pathToStr(_paths[idx].p).c_str(), ipvPref);
                        }
                    }
                    ...
                    if (ipvPref == 46 || ipvPref == 64) {
                        ...
                        for (int j = 0; j < it->second.size(); j++) {
                            ...
                            if (_paths[idx].preferred()) {
                                addPathToBond(idx, updatedBondedPathCount);
                                ++updatedBondedPathCount;
                                debug("add %s (user addr preference %d)",
                                      pathToStr(_paths[idx].p).c_str(), ipvPref);
                        ...
                        if (!foundPreferredPath) {
                            debug("did not find first-choice path type on link %s (user preference %d)",
                                  link->ifname().c_str(), ipvPref);
                            for (int j = 0; j < it->second.size(); j++) {
                                int idx = it->second.at(j);
    ...
    void Bond::estimatePathQuality(int64_t now) {
        ...
        for (unsigned int i = 0; i < ZT_MAX_PEER_NETWORK_PATHS; ++i) {
            ...
            */
            ...
            lat[i] = 1.0 / expf(4 *
                                Utils::normalize(_paths[i].latency, 0, _qw[ZT_QOS_LAT_MAX_IDX], 0,
                                                 1));
            pdv[i] = 1.0 / expf(4 * Utils::normalize(_paths[i].latencyVariance, 0,
                                                     _qw[ZT_QOS_PDV_MAX_IDX], 0, 1));
            plr[i] = 1.0 / expf(4 * Utils::normalize(_paths[i].packetLossRatio, 0,
                                                     _qw[ZT_QOS_PLR_MAX_IDX], 0, 1));
            per[i] = 1.0 / expf(4 * Utils::normalize(_paths[i].packetErrorRatio, 0,
                                                     _qw[ZT_QOS_PER_MAX_IDX], 0, 1));
        ...
        for (unsigned int i = 0; i < ZT_MAX_PEER_NETWORK_PATHS; ++i) {
            if (_paths[i].p && _paths[i].allowed()) {
                ...
                if (link) {
                    ...
                    maxObservedLinkCap =
                            linkSpeed > maxObservedLinkCap ? linkSpeed : maxObservedLinkCap;
                }
            }
        }
        ...
        for (unsigned int i = 0; i < ZT_MAX_PEER_NETWORK_PATHS; ++i) {
            if (_paths[i].p && _paths[i].bonded) {
                absoluteQuality[i] += ((maxLAT > 0.0f ? lat[i] / maxLAT : 0.0f) *
                                       _qw[ZT_QOS_LAT_WEIGHT_IDX]);
                absoluteQuality[i] += ((maxPDV > 0.0f ? pdv[i] / maxPDV : 0.0f) *
                                       _qw[ZT_QOS_PDV_WEIGHT_IDX]);
                absoluteQuality[i] += ((maxPLR > 0.0f ? plr[i] / maxPLR : 0.0f) *
                                       _qw[ZT_QOS_PLR_WEIGHT_IDX]);
                absoluteQuality[i] += ((maxPER > 0.0f ? per[i] / maxPER : 0.0f) *
                                       _qw[ZT_QOS_PER_WEIGHT_IDX]);
        ...
        for (unsigned int i = 0; i < ZT_MAX_PEER_NETWORK_PATHS; ++i) {
            ...
            if (!_paths[i].shouldAvoid) {
                if (_paths[i].latency > _qw[ZT_QOS_LAT_MAX_IDX]) {
                    log("avoiding link %s because (lat %6.4f > %6.4f)",
                        pathToStr(_paths[i].p).c_str(), _paths[i].latency, _qw[ZT_QOS_LAT_MAX_IDX]);
                    shouldAvoid = true;
                }
                if (_paths[i].latencyVariance > _qw[ZT_QOS_PDV_MAX_IDX]) {
                    log("avoiding link %s because (pdv %6.4f > %6.4f)",
                        pathToStr(_paths[i].p).c_str(), _paths[i].latencyVariance,
                        _qw[ZT_QOS_PDV_MAX_IDX]);
                    shouldAvoid = true;
                }
                if (_paths[i].packetErrorRatio > _qw[ZT_QOS_PER_MAX_IDX]) {
                    log("avoiding link %s because (per %6.4f > %6.4f)",
                        pathToStr(_paths[i].p).c_str(), _paths[i].packetErrorRatio,
                        _qw[ZT_QOS_PER_MAX_IDX]);
                    shouldAvoid = true;
                }
                if (_paths[i].packetLossRatio > _qw[ZT_QOS_PLR_MAX_IDX]) {
                    log("avoiding link %s because (plr %6.4f > %6.4f)",
                        pathToStr(_paths[i].p).c_str(), _paths[i].packetLossRatio,
                        _qw[ZT_QOS_PLR_MAX_IDX]);
                    shouldAvoid = true;
                }
            ...
            while (flow_it != _flows.end()) {
                if (_paths[flow_it->second->assignedPath].p) {
                    ...
                    if (!_paths[originalPathIdx].eligible) {
                        log("moving all flows from dead link %s",
                            pathToStr(_paths[originalPathIdx].p).c_str());
    ...
    bool Bond::abForciblyRotateLink() {
        ...
        if (_policy == ZT_BOND_POLICY_ACTIVE_BACKUP) {
            ...
            log("active link rotated from %s to %s", pathToStr(_paths[prevPathIdx].p).c_str(),
    ...
    void Bond::processActiveBackupTasks(void *tPtr, int64_t now) {
        ...
        if ((now - _lastBondStatusLog) > ZT_BOND_STATUS_INTERVAL) {
            ...
            if (_abPathIdx == ZT_MAX_PEER_NETWORK_PATHS) {
                ...
                log("active link is %s, failover queue size is %zu",
                    pathToStr(_paths[_abPathIdx].p).c_str(), _abFailoverQueue.size());
            }
            if (_abFailoverQueue.empty()) {
            ...
            if (!userHasSpecifiedLinks()) {
                ...
                if (userHasSpecifiedPrimaryLink()) {
                            ...
                            if (_paths[i].eligible && link->primary()) {
                                ...
                                if (_paths[i].preferred()) {
                                    ...
                                    if (_paths[_abPathIdx].p) {
                                        SharedPtr<Link> abLink = RR->bc->getLinkBySocket(
                                                _policyAlias, _paths[_abPathIdx].p->localSocket());
                                        if (abLink) {
                                            log("found preferred primary link %s",
                                                pathToStr(_paths[_abPathIdx].p).c_str());
                                        }
                    ...
                    if (_abPathIdx != ZT_MAX_PEER_NETWORK_PATHS) {
                        if (_paths[_abPathIdx].p) {
                            SharedPtr<Link> link = RR->bc->getLinkBySocket(_policyAlias,
                                                                           _paths[_abPathIdx].p->localSocket());
                            if (link) {
                                log("select non-primary link %s",
                                    pathToStr(_paths[_abPathIdx].p).c_str());
                            }
                        }
                    }
        ...
        for (std::deque<int>::iterator it(_abFailoverQueue.begin());
             it != _abFailoverQueue.end();) {
            if (!_paths[(*it)].p) {
                log("link is no longer valid, removing from failover queue (%zu links remain in queue)",
            ...
            if (_paths[(*it)].p && !_paths[(*it)].eligible) {
                SharedPtr<Link> link = RR->bc->getLinkBySocket(_policyAlias,
                                                               _paths[(*it)].p->localSocket());
                it = _abFailoverQueue.erase(it);
                if (link) {
                    log("link %s is ineligible, removing from failover queue (%zu links remain in queue)",
                        pathToStr(_paths[_abPathIdx].p).c_str(), _abFailoverQueue.size());
                }
                continue;
            } else {
        ...
        if (userHasSpecifiedFailoverInstructions()) {
            ...
            for (int i = 0; i < ZT_MAX_PEER_NETWORK_PATHS; ++i) {
                ...
                if (!_paths[i].failoverScore) {
                    ...
                    int newHandicap = failoverScoreHandicap ? failoverScoreHandicap : (
                            _paths[i].relativeQuality * 255.0);
                ...
                if (failoverLink) {
                    for (int j = 0; j < ZT_MAX_PEER_NETWORK_PATHS; j++) {
                        if (_paths[j].p && getLink(_paths[j].p) == failoverLink.ptr()) {
                            ...
                            int newHandicap = _paths[j].failoverScore > inheritedHandicap
                                              ? _paths[j].failoverScore : inheritedHandicap;
                ...
                if (_paths[i].p) {
                    if (_paths[i].p.ptr() != _paths[_abPathIdx].p.ptr()) {
                        ...
                        if (!bFoundPathInQueue) {
                            ...
                            log("add link %s to failover queue (%zu links in queue)",
                                pathToStr(_paths[i].p).c_str(), _abFailoverQueue.size());
                            addPathToBond(0, i);
                        }
                    }
                }
        ...
        if (!_abFailoverQueue.empty()) {
            for (int i = 0; i < _abFailoverQueue.size(); i++) {
                ...
                int hole_position = i;
                while (hole_position > 0 &&
                       (_abFailoverQueue[hole_position - 1] > value_to_insert)) {
        ...
        if (!_paths[_abPathIdx].eligible) {   // Implicit ZT_BOND_RESELECTION_POLICY_FAILURE
            log("link %s has failed, select link from failover queue (%zu links in queue)",
                pathToStr(_paths[_abPathIdx].p).c_str(), _abFailoverQueue.size());
            if (!_abFailoverQueue.empty()) {
        ...
        if (_abLinkSelectMethod == ZT_BOND_RESELECTION_POLICY_ALWAYS) {
            ...
            if (abLink && !abLink->primary() && _paths[_abFailoverQueue.front()].p &&
                abFailoverLink && abFailoverLink->primary()) {
                dequeueNextActiveBackupPath(now);
                log("switch back to available primary link %s (select mode: always)",
                    pathToStr(_paths[_abPathIdx].p).c_str());
            }
        }
        if (_abLinkSelectMethod == ZT_BOND_RESELECTION_POLICY_BETTER) {
            ...
            if (abLink && !abLink->primary()) {
                ...
                SharedPtr<Link> abFailoverLink = getLink(_paths[_abFailoverQueue.front()].p);
                if (_paths[_abFailoverQueue.front()].p && abFailoverLink &&
                    abFailoverLink->primary() && (_paths[_abFailoverQueue.front()].failoverScore >
                                                  _paths[_abPathIdx].failoverScore)) {
                    dequeueNextActiveBackupPath(now);
                    log("switch back to user-defined primary link %s (select mode: better)",
                        pathToStr(_paths[_abPathIdx].p).c_str());
                }
            }
        }
        if (_abLinkSelectMethod == ZT_BOND_RESELECTION_POLICY_OPTIMIZE &&
            !_abFailoverQueue.empty()) {
            /**
            ...
            if (_paths[_abFailoverQueue.front()].negotiated) {
                dequeueNextActiveBackupPath(now);
                _lastPathNegotiationCheck = now;
                log("switch negotiated link %s (select mode: optimize)",
                    pathToStr(_paths[_abPathIdx].p).c_str());
            } else {
                ...
                if ((now - _lastActiveBackupPathChange) > ZT_BOND_OPTIMIZE_INTERVAL) {
                    if (!_abFailoverQueue.empty()) {
                        ...
                        int failoverScoreDifference =
                                _paths[_abFailoverQueue.front()].failoverScore -
                                _paths[_abPathIdx].failoverScore;
                        int thresholdQuantity = (int) (
                                ZT_BOND_ACTIVE_BACKUP_OPTIMIZE_MIN_THRESHOLD *
                                (float) _paths[_abPathIdx].relativeQuality);
                        if ((failoverScoreDifference > 0) &&
                            (failoverScoreDifference > thresholdQuantity)) {
                            SharedPtr<Path> oldPath = _paths[_abPathIdx].p;
                            dequeueNextActiveBackupPath(now);
                            log("switch from %s (score: %d) to better link %s (score: %d) (select mode: optimize)",
                                pathToStr(oldPath).c_str(), prevFScore,
                                pathToStr(_paths[_abPathIdx].p).c_str(), newFScore);
                        }
                    }
                }
            }
    ...
    void Bond::setBondParameters(int policy, SharedPtr<Bond> templateBond, bool useTemplate) {
        ...
        _defaultPolicy = (_defaultPolicy <= ZT_BOND_POLICY_NONE ||
                          _defaultPolicy > ZT_BOND_POLICY_BALANCE_AWARE) ? ZT_BOND_POLICY_NONE
                                                                         : _defaultPolicy;
        _policy = (policy <= ZT_BOND_POLICY_NONE || policy > ZT_BOND_POLICY_BALANCE_AWARE)
        ...
        if (useTemplate) {
            ...
            _failoverInterval = templateBond->_failoverInterval >= ZT_BOND_FAILOVER_MIN_INTERVAL
                                ? templateBond->_failoverInterval : ZT_BOND_FAILOVER_MIN_INTERVAL;
            ...
            debug("user link quality spec = {%6.3f, %6.3f, %6.3f, %6.3f, %6.3f, %6.3f, %6.3f, %6.3f}",
                  _qw[0], _qw[1], _qw[2], _qw[3], _qw[4], _qw[5], _qw[6], _qw[7]);
        }
    ...
    void Bond::setUserLinkQualitySpec(float weights[], int len) {
        if (len != ZT_QOS_PARAMETER_SIZE) {
            debug("link quality spec has an invalid number of parameters (%d out of %d), ignoring",
                  len, ZT_QOS_PARAMETER_SIZE);
            return;
        }
    ...
    SharedPtr<Link> Bond::getLink(const SharedPtr<Path> &path) {
        return !path ? SharedPtr<Link>() : RR->bc->getLinkBySocket(_policyAlias,
                                                                   path->localSocket());
    }
