 ...
 */
    ...
    public:
         ...
         */
        inline void
        getUpstreamsToContact(Hashtable<Address, std::vector<InetAddress> > &eps) const {
            ...
            for (std::vector<World::Root>::const_iterator i(_planet.roots().begin());
                ...
                if (i->identity != RR->identity) {
                    ...
                    for (std::vector<InetAddress>::const_iterator j(i->stableEndpoints.begin());
                         j != i->stableEndpoints.end(); ++j) {
         ...
         */
        inline void getOutboundPathInfo(const InetAddress &physicalAddress, unsigned int &mtu,
         ...
         */
        inline bool shouldInboundPathBeTrusted(const InetAddress &physicalAddress,
                                               const uint64_t trustedPathId) {
            for (unsigned int i = 0, j = _numConfiguredPhysicalPaths; i < j; ++i) {
                if ((_physicalPathConfig[i].second.trustedPathId == trustedPathId) &&
                    (_physicalPathConfig[i].first.containsAddress(physicalAddress))) {
                    return true;
                }
            }
            return false;
         ...
         */
        inline void setPhysicalPathConfiguration(const struct sockaddr_storage *pathNetwork,
            ...
            if (!pathNetwork) {
                ...
                unsigned int cnt = 0;
                for (std::map<InetAddress, ZT_PhysicalPathConfiguration>::const_iterator i(
                        cpaths.begin()); ((i != cpaths.end()) &&
                                          (cnt < ZT_MAX_CONFIGURABLE_PATHS)); ++i) {
