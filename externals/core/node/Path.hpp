 ...
 */
    ...
    public:
        ...
        public:
            ...
            HashKey(const int64_t l, const InetAddress &r) {
                if (r.ss_family == AF_INET) {
                    ...
                    memcpy(_k, reinterpret_cast<const struct sockaddr_in6 *>(&r)->sin6_addr.s6_addr,
                           16);
         ...
         */
        inline void
        trustedPacketReceived(const uint64_t t) { _lastTrustEstablishedPacketReceived = t; }
         ...
         */
        bool send(const RuntimeEnvironment *RR, void *tPtr, const void *data, unsigned int len,
                  int64_t now);
                ...
                switch (a.ipScope()) {
                    ...
                    case InetAddress::IP_SCOPE_GLOBAL:
                        if (a.ss_family == AF_INET6) {
                            ...
                            if ((ipd[0] == 0x20) && (ipd[1] == 0x01) && (ipd[2] == 0x04) &&
                                (ipd[3] == 0x70)) {
                                return false;
                            }
                        }
        ...
        inline long quality(const int64_t now) const {
            ...
            const int age = (long) std::min((now - _lastIn), (int64_t) (ZT_PATH_HEARTBEAT_PERIOD *
                                                                        10)); // set an upper sanity limit to avoid overflow
         ...
         */
        inline int64_t
        lastTrustEstablishedPacketReceived() const { return _lastTrustEstablishedPacketReceived; }
