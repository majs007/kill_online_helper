...
namespace ZeroTier {
    ...
    Peer::Peer(const RuntimeEnvironment *renv, const Identity &myIdentity,
               const Identity &peerIdentity)
            : RR(renv), _lastReceive(0), _lastNontrivialReceive(0), _lastTriedMemorizedPath(0),
              _lastDirectPathPushSent(0), _lastDirectPathPushReceive(0),
              _lastCredentialRequestSent(0), _lastWhoisRequestReceived(0),
              _lastCredentialsReceived(0), _lastTrustEstablishedPacketReceived(0),
              _lastSentFullHello(0), _lastEchoCheck(0),
              _freeRandomByte((unsigned char) ((uintptr_t) this >> 4) ^ ++s_freeRandomByteCounter),
              _vProto(0), _vMajor(0), _vMinor(0), _vRevision(0), _id(peerIdentity),
              _directPathPushCutoffCount(0), _echoRequestCutoffCount(0),
              _localMultipathSupported(false), _lastComputedAggregateMeanLatency(0)
#ifndef ZT_NO_PEER_METRICS
            , _peer_latency{Metrics::peer_latency.Add(
              ...
              _alive_path_count{Metrics::peer_path_count.Add(
                      {{"node_id", OSUtils::nodeIDStr(peerIdentity.address().toInt())},
                       {"status",  "alive"}})}, _dead_path_count{Metrics::peer_path_count.Add(
                    {{"node_id", OSUtils::nodeIDStr(peerIdentity.address().toInt())},
                     {"status",  "dead"}})}, _incoming_packet{Metrics::peer_packets.Add(
                    {{"direction", "rx"},
                     {"node_id",   OSUtils::nodeIDStr(peerIdentity.address().toInt())}})},
              _outgoing_packet{Metrics::peer_packets.Add({{"direction", "tx"},
                                                          {"node_id",   OSUtils::nodeIDStr(
                                                                  peerIdentity.address().toInt())}})},
    ...
    void Peer::received(
        ...
        if (hops == 0) {
            ...
            if ((!havePath) &&
                RR->node->shouldUsePathForZeroTierTraffic(tPtr, _id.address(), path->localSocket(),
                                                          path->address())) {
                if (verb == Packet::VERB_OK) {
                    ...
                    replacePath = (replacePath == ZT_MAX_PEER_NETWORK_PATHS) ? oldestPathIdx
                                                                             : replacePath;
                    ...
                    if (!triedTooRecently) {
                        ...
                        path->sent(now);
                        RR->t->peerConfirmingUnknownPath(tPtr, networkId, *this, path, packetId,
                                                         verb);
                    }
                }
            }
        ...
        if (this->trustEstablished(now)) {
            ...
            if (sinceLastPush >= ((hops == 0) ? ZT_DIRECT_PATH_PUSH_INTERVAL_HAVEPATH * timerScale
                ...
                if (!pathsToPush.empty()) {
                    ...
                    while (p != pathsToPush.end()) {
                        Packet *const outp = new Packet(_id.address(), RR->identity.address(),
                                                        Packet::VERB_PUSH_DIRECT_PATHS);
    ...
    void Peer::introduce(void *const tPtr, const int64_t now, const SharedPtr<Peer> &other) const {
        ...
        for (int s = ZT_INETADDRESS_MAX_SCOPE; s >= 0; --s) {
            if ((myBestV6ByScope[s] != ZT_MAX_PEER_NETWORK_PATHS) &&
                (theirBestV6ByScope[s] != ZT_MAX_PEER_NETWORK_PATHS)) {
        ...
        if (mine != ZT_MAX_PEER_NETWORK_PATHS) {
            unsigned int alt = (unsigned int) RR->node->prng() &
                               1; // randomize which hint we send first for black magickal NAT-t reasons
            ...
            while (alt != completed) {
                if ((alt & 1) == 0) {
                    ...
                    Packet outp(other->_id.address(), RR->identity.address(),
    ...
    void Peer::sendHELLO(void *tPtr, const int64_t localSocket, const InetAddress &atAddress,
                         int64_t now) {
        ...
        for (std::vector<uint64_t>::const_iterator m(moonsWanted.begin());
             m != moonsWanted.end(); ++m) {
        ...
        if (atAddress) {
            ...
            RR->node->putPacket(tPtr, RR->node->lowBandwidthModeEnabled() ? localSocket : -1,
    ...
    void
    Peer::attemptToContactAt(void *tPtr, const int64_t localSocket, const InetAddress &atAddress,
                             int64_t now, bool sendFullHello) {
        if ((!sendFullHello) && (_vProto >= 5) &&
            (!((_vMajor == 1) && (_vMinor == 1) && (_vRevision == 0)))) {
    ...
    void Peer::performMultipathStateCheck(void *tPtr, int64_t now) {
        ...
        _localMultipathSupported = ((numAlivePaths >= 1) && (RR->bc->inUse()) &&
                                    (ZT_PROTO_VERSION > 9));
        ...
        unsigned int sent = 0;
        {
            ...
            for (unsigned int i = 0; i < ZT_MAX_PEER_NETWORK_PATHS; ++i) {
                if (_paths[i].p) {
                    ...
                    if (((now - _paths[i].lr) < ZT_PEER_PATH_EXPIRATION) &&
                        (_paths[i].priority == maxPriority)) {
                        if ((sendFullHello) || (_paths[i].p->needsHeartbeat(now))) {
                            attemptToContactAt(tPtr, _paths[i].p->localSocket(),
                                               _paths[i].p->address(), now, sendFullHello);
                            _paths[i].p->sent(now);
    ...
    void Peer::clusterRedirect(void *tPtr, const SharedPtr<Path> &originatingPath,
                               const InetAddress &remoteAddress, const int64_t now) {
...

        {
            ...
            for (unsigned int i = 0; i < ZT_MAX_PEER_NETWORK_PATHS; ++i) {
                if (_paths[i].p) {
                    if ((_paths[i].priority >= newPriority) &&
                        (!_paths[i].p->address().ipsEqual2(remoteAddress))) {
                        if (i != j) {
                            _paths[j] = _paths[i];
                        }
                        ++j;
        ...
        for (unsigned int i = 0; i < ZT_MAX_PEER_NETWORK_PATHS; ++i) {
            if (_paths[i].p) {
                if ((_paths[i].p->address().ss_family == inetAddressFamily) &&
                    (_paths[i].p->ipScope() == scope)) {
                    attemptToContactAt(tPtr, _paths[i].p->localSocket(), _paths[i].p->address(),
                                       now, false);
                    _paths[i].p->sent(now);
