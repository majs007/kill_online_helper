...
/****************************************************************************/
    ...
    Node::Node(void *uptr, void *tptr, const struct ZT_Node_Callbacks *callbacks, int64_t now) :
        ...
        if (n <= 0) {
            ...
            if ((n > 0) && (n < (int) sizeof(RR->publicIdentityStr)) && (n < (int) sizeof(tmp))) {
                if (memcmp(tmp, RR->publicIdentityStr, n)) {
                    stateObjectPut(tptr, ZT_STATE_OBJECT_IDENTITY_PUBLIC, idtmp,
                                   RR->publicIdentityStr,
                                   (unsigned int) strlen(RR->publicIdentityStr));
                }
            }
        }
        ...
        char *m = (char *) 0;
        try {
            const unsigned long ts = sizeof(Trace) +
                                     (((sizeof(Trace) & 0xf) != 0) ? (16 - (sizeof(Trace) & 0xf))
                                                                   : 0);
            const unsigned long sws = sizeof(Switch) +
                                      (((sizeof(Switch) & 0xf) != 0) ? (16 - (sizeof(Switch) & 0xf))
                                                                     : 0);
            const unsigned long mcs = sizeof(Multicaster) +
                                      (((sizeof(Multicaster) & 0xf) != 0) ? (16 -
                                                                             (sizeof(Multicaster) &
                                                                              0xf)) : 0);
            const unsigned long topologys = sizeof(Topology) +
                                            (((sizeof(Topology) & 0xf) != 0) ? (16 -
                                                                                (sizeof(Topology) &
                                                                                 0xf)) : 0);
            const unsigned long sas = sizeof(SelfAwareness) +
                                      (((sizeof(SelfAwareness) & 0xf) != 0) ? (16 -
                                                                               (sizeof(SelfAwareness) &
                                                                                0xf)) : 0);
            const unsigned long bc =
                    sizeof(Bond) + (((sizeof(Bond) & 0xf) != 0) ? (16 - (sizeof(Bond) & 0xf)) : 0);
    ...
    ZT_ResultCode Node::processWirePacket(
        ...
        _now = now;
        RR->sw->onRemotePacket(tptr, localSocket,
                               *(reinterpret_cast<const InetAddress *>(remoteAddress)), packetData,
                               packetLength);
        return ZT_RESULT_OK;
    }
    ...
    ZT_ResultCode Node::processVirtualNetworkFrame(
        ...
        if (nw) {
            RR->sw->onLocalEthernet(tptr, nw, MAC(sourceMac), MAC(destMac), etherType, vlanId,
                                    frameData, frameLength);
            return ZT_RESULT_OK;
        } else {
...
// Closure used to ping upstream and active/online peers
    ...
    public:
        _PingPeersThatNeedPing(const RuntimeEnvironment *renv, void *tPtr,
        ...
        inline void operator()(Topology &t, const SharedPtr<Peer> &p) {
            const std::vector<InetAddress> *const alwaysContactEndpoints = _alwaysContact.get(
                    p->address());
            if (alwaysContactEndpoints) {
                ...
                roleBasedTimerScale = (role != ZT_PEER_ROLE_LEAF && !hasPaths) ? 0
                                                                               : roleBasedTimerScale;

                if ((RR->node->now() - p->lastSentFullHello()) <=
                    (ZT_PATH_HEARTBEAT_PERIOD * roleBasedTimerScale)) {
                    return;
                }
                ...
                if ((sent & 0x1) == 0) { // bit 0x1 == IPv4 sent
                    for (unsigned long k = 0, ptr = (unsigned long) RR->node->prng();
                        ...
                        const InetAddress &addr = (*alwaysContactEndpoints)[ptr++ %
                                                                            alwaysContactEndpoints->size()];
                ...
                _alwaysContact.erase(
                        p->address()); // after this we'll WHOIS all upstreams that remain
    ...
    ZT_ResultCode Node::processBackgroundTasks(void *tptr, int64_t now,
                                               volatile int64_t *nextBackgroundTaskDeadline) {
        _now = now;
        ...
        if (RR->bc->inUse()) {
            bondCheckInterval = std::max(RR->bc->minReqMonitorInterval(),
                                         ZT_CORE_TIMER_TASK_GRANULARITY);
        ...
        unsigned long timeUntilNextPingCheck = _lowBandwidthMode ? (ZT_PING_CHECK_INTERVAL * 5)
                                                                 : ZT_PING_CHECK_INTERVAL;
                ...
                */
                ...
                int64_t lastReceivedFromUpstream = 0;
                {
                    ...
                    while (i.next(upstreamAddress, upstreamStableEndpoints)) {
                        ...
                        if (p) {
                            lastReceivedFromUpstream = std::max(p->lastReceive(),
                                                                lastReceivedFromUpstream);
                        }
                    }
                }
                ...
                // Clean up any old local controller auth memorizations.
                {
                    ...
                    Hashtable<_LocalControllerAuth, int64_t>::Iterator i(
                            _localControllerAuthorizations);
                ...
                std::vector<std::pair<SharedPtr<Network>, bool> > networkConfigNeeded;
                {
                    ...
                    while (i.next(nwid, network)) {
                        ...
                        networkConfigNeeded.push_back(std::pair<SharedPtr<Network>, bool>(*network,
                                                                                          (((now -
                                                                                             (*network)->lastConfigUpdate()) >=
                                                                                            ZT_NETWORK_AUTOCONF_DELAY *
                                                                                            timerScale) ||
                                                                                           (!(*network)->hasConfig()))));
                    }
                }
                ...
                for (std::vector<std::pair<SharedPtr<Network>, bool> >::const_iterator n(
                        networkConfigNeeded.begin()); n != networkConfigNeeded.end(); ++n) {
                    if (n->second) {
                ...
                _online = (((now - lastReceivedFromUpstream) < ZT_PEER_ACTIVITY_TIMEOUT) ||
                           (RR->topology->amUpstream()));
        ...
        try {
            *nextBackgroundTaskDeadline = now + (int64_t) std::max(std::min(bondCheckInterval,
                                                                            std::min(
                                                                                    timeUntilNextPingCheck,
                                                                                    RR->sw->doTimerTasks(
                                                                                            tptr,
                                                                                            now))),
    ...
    ZT_ResultCode Node::leave(uint64_t nwid, void **uptr, void *tptr) {
        ...
        if (nUserPtr) {
            RR->node->configureVirtualNetworkPort(tptr, nwid, nUserPtr,
    ...
    ZT_ResultCode Node::multicastSubscribe(void *tptr, uint64_t nwid, uint64_t multicastGroup,
                                           unsigned long multicastAdi) {
        SharedPtr<Network> nw(this->network(nwid));
        if (nw) {
            nw->multicastSubscribe(tptr, MulticastGroup(MAC(multicastGroup),
                                                        (uint32_t) (multicastAdi & 0xffffffff)));
            return ZT_RESULT_OK;
        } else {
            return ZT_RESULT_ERROR_NETWORK_NOT_FOUND;
        }
    ...
    ZT_ResultCode
    Node::multicastUnsubscribe(uint64_t nwid, uint64_t multicastGroup, unsigned long multicastAdi) {
        SharedPtr<Network> nw(this->network(nwid));
        if (nw) {
            nw->multicastUnsubscribe(
                    MulticastGroup(MAC(multicastGroup), (uint32_t) (multicastAdi & 0xffffffff)));
            return ZT_RESULT_OK;
        } else {
            return ZT_RESULT_ERROR_NETWORK_NOT_FOUND;
        }
    ...
    ZT_PeerList *Node::peers() const {
        ...
        pl->peerCount = 0;
        for (std::vector<std::pair<Address, SharedPtr<Peer> > >::iterator pi(peers.begin());
             pi != peers.end(); ++pi) {
            ...
            p->pathCount = 0;
            for (std::vector<SharedPtr<Path> >::iterator path(paths.begin());
                 path != paths.end(); ++path) {
                if ((*path)->valid()) {
                    memcpy(&(p->paths[p->pathCount].address), &((*path)->address()),
                           sizeof(struct sockaddr_storage));
                    p->paths[p->pathCount].localSocket = (*path)->localSocket();
                    p->paths[p->pathCount].lastSend = (*path)->lastOut();
                    p->paths[p->pathCount].lastReceive = (*path)->lastIn();
                    p->paths[p->pathCount].trustedPathId = RR->topology->getOutboundPathTrust(
                            (*path)->address());
                    ...
                    if (pi->second->bond()) {
                        ...
                        memset(p->paths[p->pathCount].ifname, 0x0,
                               std::min((int) ifname.length() + 1, ZT_MAX_PHYSIFNAME));
    ...
    ZT_VirtualNetworkConfig *Node::networkConfig(uint64_t nwid) const {
        ...
        if (nw) {
            ZT_VirtualNetworkConfig *nc = (ZT_VirtualNetworkConfig *) ::malloc(
                    sizeof(ZT_VirtualNetworkConfig));
    ...
    ZT_VirtualNetworkList *Node::networks() const {
        ...
        char *buf = (char *) ::malloc(sizeof(ZT_VirtualNetworkList) +
                                      (sizeof(ZT_VirtualNetworkConfig) * _networks.size()));
        if (!buf) {
    ...
    int Node::addLocalInterfaceAddress(const struct sockaddr_storage *addr) {
        if (Path::isAddressValidForPath(*(reinterpret_cast<const InetAddress *>(addr)))) {
            ...
            if (std::find(_directPaths.begin(), _directPaths.end(),
                          *(reinterpret_cast<const InetAddress *>(addr))) == _directPaths.end()) {
    ...
    int Node::sendUserMessage(void *tptr, uint64_t dest, uint64_t typeId, const void *data,
                              unsigned int len) {
        try {
...
/****************************************************************************/
    ...
    bool Node::shouldUsePathForZeroTierTraffic(void *tPtr, const Address &ztaddr,
        ...
        return ((_cb.pathCheckFunction) ? (
                _cb.pathCheckFunction(reinterpret_cast<ZT_Node *>(this), _uPtr, tPtr,
                                      ztaddr.toInt(), localSocket,
    ...
    void Node::ncSendConfig(uint64_t nwid, uint64_t requestPacketId, const Address &destination,
                ...
                if (nc.toDictionary(*dconf, sendLegacyFormatConfig)) {
                    ...
                    while (chunkIndex < totalSize) {
                        const unsigned int chunkLen = std::min(totalSize - chunkIndex,
                                                               (unsigned int) (
                                                                       ZT_PROTO_MAX_PACKET_LENGTH -
                                                                       (ZT_PACKET_IDX_PAYLOAD +
                                                                        256)));
                        Packet outp(destination, RR->identity.address(),
                                    (requestPacketId) ? Packet::VERB_OK
                                                      : Packet::VERB_NETWORK_CONFIG);
                        if (requestPacketId) {
                        ...
                        C25519::Signature sig(RR->identity.sign(
                                reinterpret_cast<const uint8_t *>(outp.data()) + sigStart,
                                outp.size() - sigStart));
    ...
    void Node::ncSendError(uint64_t nwid, uint64_t requestPacketId, const Address &destination,
                           NetworkController::ErrorCode errorCode, const void *errorData,
...
enum ZT_ResultCode
ZT_Node_new(ZT_Node **node, void *uptr, void *tptr, const struct ZT_Node_Callbacks *callbacks,
            int64_t now) {
    *node = (ZT_Node *) 0;
    try {
...
enum ZT_ResultCode ZT_Node_processWirePacket(
        ...
        volatile int64_t *nextBackgroundTaskDeadline) {
    try {
        return reinterpret_cast<ZeroTier::Node *>(node)->processWirePacket(tptr, now, localSocket,
                                                                           remoteAddress,
...
enum ZT_ResultCode ZT_Node_processVirtualNetworkFrame(
        ...
        volatile int64_t *nextBackgroundTaskDeadline) {
    try {
        return reinterpret_cast<ZeroTier::Node *>(node)->processVirtualNetworkFrame(tptr, now, nwid,
                                                                                    sourceMac,
                                                                                    destMac,
                                                                                    etherType,
                                                                                    vlanId,
...
enum ZT_ResultCode ZT_Node_processBackgroundTasks(ZT_Node *node, void *tptr, int64_t now,
                                                  volatile int64_t *nextBackgroundTaskDeadline) {
    try {
        return reinterpret_cast<ZeroTier::Node *>(node)->processBackgroundTasks(tptr, now,
                                                                                nextBackgroundTaskDeadline);
    } catch (std::bad_alloc &exc) {
        return ZT_RESULT_FATAL_ERROR_OUT_OF_MEMORY;
    } catch (...) {
...
enum ZT_ResultCode
ZT_Node_multicastSubscribe(ZT_Node *node, void *tptr, uint64_t nwid, uint64_t multicastGroup,
                           unsigned long multicastAdi) {
    try {
        return reinterpret_cast<ZeroTier::Node *>(node)->multicastSubscribe(tptr, nwid,
                                                                            multicastGroup,
                                                                            multicastAdi);
    } catch (std::bad_alloc &exc) {
        return ZT_RESULT_FATAL_ERROR_OUT_OF_MEMORY;
...
enum ZT_ResultCode
ZT_Node_multicastUnsubscribe(ZT_Node *node, uint64_t nwid, uint64_t multicastGroup,
                             unsigned long multicastAdi) {
    try {
        return reinterpret_cast<ZeroTier::Node *>(node)->multicastUnsubscribe(nwid, multicastGroup,
                                                                              multicastAdi);
    } catch (std::bad_alloc &exc) {
        return ZT_RESULT_FATAL_ERROR_OUT_OF_MEMORY;
    } catch (...) {
...
enum ZT_ResultCode
ZT_Node_orbit(ZT_Node *node, void *tptr, uint64_t moonWorldId, uint64_t moonSeed) {
    try {
...
int
ZT_Node_sendUserMessage(ZT_Node *node, void *tptr, uint64_t dest, uint64_t typeId, const void *data,
                        unsigned int len) {
    try {
        return reinterpret_cast<ZeroTier::Node *>(node)->sendUserMessage(tptr, dest, typeId, data,
                                                                         len);
    } catch (...) {
        return 0;
    }
...
enum ZT_ResultCode
ZT_Node_setPhysicalPathConfiguration(ZT_Node *node, const struct sockaddr_storage *pathNetwork,
                                     const ZT_PhysicalPathConfiguration *pathConfig) {
    try {
        return reinterpret_cast<ZeroTier::Node *>(node)->setPhysicalPathConfiguration(pathNetwork,
                                                                                      pathConfig);
    } catch (...) {
        return ZT_RESULT_FATAL_ERROR_INTERNAL;
    }
