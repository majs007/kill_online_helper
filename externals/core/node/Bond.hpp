 ...
 */
enum ZT_BondQualityWeightIndex {
    ZT_QOS_LAT_MAX_IDX,
    ZT_QOS_PDV_MAX_IDX,
    ZT_QOS_PLR_MAX_IDX,
    ZT_QOS_PER_MAX_IDX,
    ZT_QOS_LAT_WEIGHT_IDX,
    ZT_QOS_PDV_WEIGHT_IDX,
    ZT_QOS_PLR_WEIGHT_IDX,
    ZT_QOS_PER_WEIGHT_IDX,
    ZT_QOS_PARAMETER_SIZE
};
 ...
 */
enum ZT_BondLinkMode {
    ZT_BOND_SLAVE_MODE_PRIMARY = 0, ZT_BOND_SLAVE_MODE_SPARE = 1
};
...
namespace ZeroTier {
    ...
    public:
         ...
         */
        Link(std::string ifnameStr, uint8_t ipvPref, uint16_t mtu, uint32_t capacity, bool enabled,
             uint8_t mode, std::string failoverToLinkStr)
                : _ifnameStr(ifnameStr), _ipvPref(ipvPref), _mtu(mtu), _capacity(capacity),
                  _relativeCapacity(0.0), _enabled(enabled), _mode(mode),
                  _failoverToLinkStr(failoverToLinkStr), _isUserSpecified(false) {
        }
    ...
    public:
         ...
         */
        static SharedPtr<Bond>
        createBond(const RuntimeEnvironment *renv, const SharedPtr<Peer> &peer);
         ...
         */
        SharedPtr<Link>
        getLinkBySocket(const std::string &policyAlias, uint64_t localSocket, bool createIfNeeded);
         ...
         * @return Physical link definition
         */
        static SharedPtr<Link>
        getLinkByName(const std::string &policyAlias, const std::string &ifname);
    ...
    public:
         ...
         * @param peer
         */
        Bond(const RuntimeEnvironment *renv, std::string &basePolicy, std::string &policyAlias,
         ...
         * @param peer
         */
        Bond(const RuntimeEnvironment *renv, SharedPtr<Bond> originalBond,
             const SharedPtr<Peer> &peer);
         ...
         */
        void
        recordOutgoingPacket(const SharedPtr<Path> &path, uint64_t packetId, uint16_t payloadLength,
         ...
         */
        void receivedQoS(const SharedPtr<Path> &path, int64_t now, int count, uint64_t *rx_id,
                         uint16_t *rx_ts);
         ...
         */
        void
        recordIncomingPacket(const SharedPtr<Path> &path, uint64_t packetId, uint16_t payloadLength,
                             Packet::Verb verb, int32_t flowId, int64_t now);
         ...
         */
        void processIncomingPathNegotiationRequest(uint64_t now, SharedPtr<Path> &path,
         ...
         * @param now Current time
         */
        void sendACK(void *tPtr, int pathIdx, int64_t localSocket, const InetAddress &atAddress,
                     int64_t now);
         ...
         * @param now Current time
         */
        void sendQOS_MEASUREMENT(void *tPtr, int pathIdx, int64_t localSocket,
        ...
        inline bool rateGateACK(const int64_t now) {
            _ackCutoffCount++;
            int numToDrain = _lastAckRateCheck ? (now - _lastAckRateCheck) / ZT_ACK_DRAINAGE_DIVISOR
                                               : _ackCutoffCount;
...
#undef MAX_MSG_LEN
    ...
    private:
        struct NominatedPath {
            NominatedPath()
                    : lastAckSent(0), lastAckReceived(0), lastQoSReceived(0), unackedBytes(0),
                      packetsReceivedSinceLastAck(0), lastQoSMeasurement(0),
                      lastThroughputEstimation(0), lastRefractoryUpdate(0), lastAliveToggle(0),
                      alive(false), eligible(true), lastEligibility(0), whenNominated(0),
                      refractoryPeriod(0), ipvPref(0), mode(0), onlyPathOnLink(false),
                      bonded(false), negotiated(false), shouldAvoid(false), assignedFlowCount(0),
                      latency(0), latencyVariance(0), packetLossRatio(0), packetErrorRatio(0),
                      relativeQuality(0), relativeLinkCapacity(0), failoverScore(0),
                      packetsReceivedSinceLastQoS(0), packetsIn(0), packetsOut(0) {
            }
            ...
            inline void adjustRefractoryPeriod(int64_t now, uint32_t punishment, bool pathFailure) {
                if (pathFailure) {
                    unsigned int suggestedRefractoryPeriod = refractoryPeriod ? punishment +
                                                                                (refractoryPeriod *
                                                                                 2) : punishment;
                    refractoryPeriod = std::min(suggestedRefractoryPeriod,
                                                (unsigned int) ZT_BOND_MAX_REFRACTORY_PERIOD);
            ...
            inline bool allowed() {
                return (!ipvPref ||
                        ((p->_addr.isV4() && (ipvPref == 4 || ipvPref == 46 || ipvPref == 64)) ||
                         ((p->_addr.isV6() && (ipvPref == 6 || ipvPref == 46 || ipvPref == 64)))));
            }
            ...
            inline bool preferred() {
                return onlyPathOnLink || (p->_addr.isV4() && (ipvPref == 4 || ipvPref == 46)) ||
                       (p->_addr.isV6() && (ipvPref == 6 || ipvPref == 64));
            }
            ...
            inline bool needsToSendQoS(int64_t now, uint64_t qosSendInterval) {
                ...
                return ((packetsReceivedSinceLastQoS >= ZT_QOS_TABLE_SIZE) ||
                        ((now - lastQoSMeasurement) > qosSendInterval)) &&
                       packetsReceivedSinceLastQoS;
            }
            ...
            inline bool needsToSendAck(int64_t now, uint64_t ackSendInterval) {
                return ((now - lastAckSent) >= ackSendInterval ||
                        (packetsReceivedSinceLastAck == ZT_QOS_TABLE_SIZE)) &&
        ...
        struct Flow {
             ...
             * @param now Current time
             */
            Flow(int32_t flowId, int64_t now) : id(flowId), bytesIn(0), bytesOut(0),
                                                lastActivity(now), lastPathReassignment(0),
                                                assignedPath(ZT_MAX_PEER_NETWORK_PATHS) {
            }
