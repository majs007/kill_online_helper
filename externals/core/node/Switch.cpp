...
namespace ZeroTier {
    ...
    Switch::Switch(const RuntimeEnvironment *renv) :
            ...
            _lastUniteAttempt(
                    8) // only really used on root servers and upstreams, and it'll grow there just fine
    {
    }
...
// Returns true if packet appears valid; pos and proto will be set
    ...
    void Switch::onRemotePacket(void *tPtr, const int64_t localSocket, const InetAddress &fromAddr,
        ...
        try {
            ...
            if (len == 13) {
                ...
                if (!RR->node->shouldUsePathForZeroTierTraffic(tPtr, beaconAddr, localSocket,
                                                               fromAddr)) {
                    return;
                }
                ...
                if (peer) { // we'll only respond to beacons from known peers
                    if ((now - _lastBeaconResponse) >= 2500) { // limit rate of responses
                       ...
                       ZT_PROTO_MIN_FRAGMENT_LENGTH) { // SECURITY: min length check is important since we do some C-style stuff below!
                if (reinterpret_cast<const uint8_t *>(data)[ZT_PACKET_FRAGMENT_IDX_FRAGMENT_INDICATOR] ==
                    ...
                    if (destination != RR->identity.address()) {
                        ...
                        if ((totalFragments <= ZT_MAX_PACKET_FRAGMENTS) &&
                            (fragmentNumber < ZT_MAX_PACKET_FRAGMENTS) && (fragmentNumber > 0) &&
                            (totalFragments > 1)) {
                            ...
                            if (rq->packetId != fragmentPacketId) {
                                ...
                                rq->haveFragments =
                                        1 << fragmentNumber; // we have only this fragment
                                rq->complete = false;
                                ...
                                if (Utils::countBits(rq->haveFragments |= (1 << fragmentNumber)) ==
                                    totalFragments) {
                                    // We have all fragments -- assemble and process full Packet

                                    for (unsigned int f = 1; f < totalFragments; ++f) {
                                        rq->frag0.append(rq->frags[f - 1].payload(),
                                                         rq->frags[f - 1].payloadLength());
                                    }
                    ...
                    const Address destination(reinterpret_cast<const uint8_t *>(data) + 8,
                                              ZT_ADDRESS_LENGTH);
                    const Address source(reinterpret_cast<const uint8_t *>(data) + 13,
                                         ZT_ADDRESS_LENGTH);
                        ...
                        if (packet.hops() < ZT_RELAY_MAX_HOPS) {
                            ...
                            if ((relayTo) &&
                                (relayTo->sendDirect(tPtr, packet.data(), packet.size(), now,
                                                     false))) {
                                if ((source != RR->identity.address()) &&
                                    (_shouldUnite(now, source, destination))) {
                                    const SharedPtr<Peer> sourcePeer(
                                            RR->topology->getPeer(tPtr, source));
                                    if (sourcePeer) {
                                ...
                                if ((relayTo) && (relayTo->address() != source)) {
                                    if (relayTo->sendDirect(tPtr, packet.data(), packet.size(), now,
                                                            true)) {
                                        const SharedPtr<Peer> sourcePeer(
                                                RR->topology->getPeer(tPtr, source));
                                        if (sourcePeer) {
                    ...
                    } else if ((reinterpret_cast<const uint8_t *>(data)[ZT_PACKET_IDX_FLAGS] &
                                ZT_PROTO_FLAG_FRAGMENTED) != 0) {
    ...
    void Switch::onLocalEthernet(void *tPtr, const SharedPtr<Network> &network, const MAC &from,
                                 const MAC &to, unsigned int etherType, unsigned int vlanId,
                                 const void *data, unsigned int len) {
        ...
        if (to.isMulticast()) {
            ...
            if (to.isBroadcast()) {
                if ((etherType == ZT_ETHERTYPE_ARP) && (len >= 28) &&
                    ((((const uint8_t *) data)[2] == 0x08) &&
                     (((const uint8_t *) data)[3] == 0x00) && (((const uint8_t *) data)[4] == 6) &&
                     (((const uint8_t *) data)[5] == 4) && (((const uint8_t *) data)[7] == 0x01))) {
                    ...
                    RR->t->outgoingNetworkFrameDropped(tPtr, network, from, to, etherType, vlanId,
                                                       len, "broadcast disabled");
                    return;
                }
                ...
                if ((network->config().ndpEmulation()) &&
                    (reinterpret_cast<const uint8_t *>(data)[6] == 0x3a) &&
                    (reinterpret_cast<const uint8_t *>(data)[40] ==
                     0x87)) { // ICMPv6 neighbor solicitation
                    ...
                    for (unsigned int sipk = 0; sipk < network->config().staticIpCount; ++sipk) {
                        ...
                        if (sip->ss_family == AF_INET6) {
                            ...
                            const unsigned int sipNetmaskBits = Utils::ntoh(
                                    (uint16_t) reinterpret_cast<const struct sockaddr_in6 *>(&(*sip))->sin6_port);
                            if ((sipNetmaskBits == 88) && (my6[0] == 0xfd) && (my6[9] == 0x99) &&
                                ...
                                const uint32_t nwid32 = (uint32_t) (
                                        (network->id() ^ (network->id() >> 32)) & 0xffffffff);
                                if ((my6[0] == 0xfc) &&
                    ...
                    if ((v6EmbeddedAddress) && (v6EmbeddedAddress != RR->identity.address())) {
                        ...
                        std::thread([=]() {
                            ...
                            RR->node->putFrame(tPtr, network->id(), network->userPtr(), peerMac,
                                               from, ZT_ETHERTYPE_IPV6, 0, adv, 72);
            ...
            if (!network->filterOutgoingPacket(tPtr, false, RR->identity.address(), Address(), from,
                                               to, (const uint8_t *) data, len, etherType, vlanId,
                                               qosBucket)) {
                RR->t->outgoingNetworkFrameDropped(tPtr, network, from, to, etherType, vlanId, len,
                                                   "filter blocked");
                return;
            ...
            std::thread([=]() {
                ...
                RR->node->putFrame(tPtr, network->id(), network->userPtr(), from, to, etherType,
                                   vlanId, data, len);
            ...
            if (!network->filterOutgoingPacket(tPtr, false, RR->identity.address(), toZT, from, to,
                                               (const uint8_t *) data, len, etherType, vlanId,
                                               qosBucket)) {
                RR->t->outgoingNetworkFrameDropped(tPtr, network, from, to, etherType, vlanId, len,
                                                   "filter blocked");
                return;
            ...
            if ((bridges[0]) && (bridges[0] != RR->identity.address()) &&
                (network->config().permitsBridging(bridges[0]))) {
                ...
                if (activeBridges.size() <= ZT_MAX_BRIDGE_SPAM) {
                    ...
                    while (numBridges < ZT_MAX_BRIDGE_SPAM) {
                        ...
                        if (((unsigned long) RR->node->prng() %
                             (unsigned long) activeBridges.size()) == 0) {
                            bridges[numBridges++] = *ab;
                            ++ab;
                        } else {
                            ++ab;
            ...
            for (unsigned int b = 0; b < numBridges; ++b) {
                if (network->filterOutgoingPacket(tPtr, true, RR->identity.address(), bridges[b],
                                                  from, to, (const uint8_t *) data, len, etherType,
                                                  vlanId, qosBucket)) {
                    ...
                    RR->t->outgoingNetworkFrameDropped(tPtr, network, from, to, etherType, vlanId,
                                                       len, "filter blocked (bridge replication)");
                }
            }
        }
    }
    ...
    void
    Switch::aqm_enqueue(void *tPtr, const SharedPtr<Network> &network, Packet &packet, bool encrypt,
                        int qosBucket, int32_t flowId) {
        ...
        for (size_t i = 0; i < ZT_AQM_NUM_BUCKETS; i++) {
            if (i <
                nqcb->oldQueues.size()) { // search old queues first (I think this is best since old would imply most recent usage of the queue)
                if (nqcb->oldQueues[i]->id == qosBucket) {
                    selectedQueue = nqcb->oldQueues[i];
                }
            }
            if (i <
                nqcb->newQueues.size()) { // search new queues (this would imply not often-used queues)
    ...
    void Switch::aqm_dequeue(void *tPtr) {
        ...
        for (std::map<uint64_t, NetworkQoSControlBlock *>::iterator nqcb(
                _netQueueControlBlock.begin()); nqcb != _netQueueControlBlock.end();) {
    ...
    void Switch::doAnythingWaitingForPeer(void *tPtr, const SharedPtr<Peer> &peer) {
        ...
        for (unsigned int ptr = 0; ptr < ZT_RX_QUEUE_SIZE; ++ptr) {
            ...
            if ((rq->timestamp) && (rq->complete)) {
                if ((rq->frag0.tryDecode(RR, tPtr, rq->flowId)) ||
                    ((now - rq->timestamp) > ZT_RECEIVE_QUEUE_TIMEOUT)) {
                    rq->timestamp = 0;
                }
            }
        }
    ...
    bool
    Switch::_shouldUnite(const int64_t now, const Address &source, const Address &destination) {
        Mutex::Lock _l(_lastUniteAttempt_m);
            ...
            if ((peer->bondingPolicy() == ZT_BOND_POLICY_BROADCAST)
                ...
                for (int i = 0; i < ZT_MAX_PEER_NETWORK_PATHS; ++i) {
                    if (peer->_paths[i].p && peer->_paths[i].p->alive(now)) {
                        ...
                        _sendViaSpecificPath(tPtr, peer, peer->_paths[i].p, userSpecifiedMtu, now,
                                             packet, encrypt, flowId);
                    }
                }
                return true;
            } else {
                ...
                if (!viaPath) {
                    peer->tryMemorizedPath(tPtr,
                                           now); // periodically attempt memorized or statically defined paths, if any are known
                    const SharedPtr<Peer> relay(RR->topology->getUpstreamPeer());
                ...
                if (viaPath) {
                    ...
                    _sendViaSpecificPath(tPtr, peer, viaPath, userSpecifiedMtu, now, packet,
                                         encrypt, flowId);
                    return true;
                }
            }
    ...
    void Switch::_sendViaSpecificPath(void *tPtr, SharedPtr<Peer> peer, SharedPtr<Path> viaPath,
                                      uint16_t userSpecifiedMtu, int64_t now, Packet &packet,
        ...
        peer->recordOutgoingPacket(viaPath, packet.packetId(), packet.payloadLength(),
                                   packet.verb(), flowId, now);
        ...
        if (viaPath->send(RR, tPtr, packet.data(), chunkSize, now)) {
            if (chunkSize < packet.size()) {
                ...
                for (unsigned int fno = 1; fno < totalFragments; ++fno) {
                    chunkSize = std::min(remaining,
                                         (unsigned int) (mtu - ZT_PROTO_MIN_FRAGMENT_LENGTH));
