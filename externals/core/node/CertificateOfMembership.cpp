...
namespace ZeroTier {
    ...
    CertificateOfMembership::CertificateOfMembership(uint64_t timestamp, uint64_t timestampMaxDelta,
                                                     uint64_t nwid, const Identity &issuedTo) {
    ...
    bool CertificateOfMembership::agreesWith(const CertificateOfMembership &other,
                                             const Identity &otherIdentity) const {
    ...
    int CertificateOfMembership::verify(const RuntimeEnvironment *RR, void *tPtr) const {
        if ((!_signedBy) || (_signedBy != Network::controllerFor(networkId())) ||
