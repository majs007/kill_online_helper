 ...
 */
    ...
    public:
         ...
         */
        inline const C25519::Public &
        updatesMustBeSignedBy() const { return _updatesMustBeSignedBy; }
        ...
        inline bool shouldBeReplacedBy(const World &update) {
            ...
            if ((_id == update._id) && (_ts < update._ts) && (_type == update._type)) {
                ...
                return C25519::verify(_updatesMustBeSignedBy, tmp.data(), tmp.size(),
                                      update._signature);
            }
            return false;
        }
        ...
        inline void serialize(Buffer<C> &b, bool forSign = false) const {
            ...
            for (std::vector<Root>::const_iterator r(_roots.begin()); r != _roots.end(); ++r) {
                ...
                for (std::vector<InetAddress>::const_iterator ep(r->stableEndpoints.begin());
                     ep != r->stableEndpoints.end(); ++ep) {
                    ep->serialize(b);
                }
            }
            if (_type == TYPE_MOON) {
        ...
        inline unsigned int deserialize(const Buffer<C> &b, unsigned int startAt = 0) {
            ...
            p += 8;
            memcpy(_updatesMustBeSignedBy.data, b.field(p, ZT_C25519_PUBLIC_KEY_LEN),
        ...
        inline bool operator==(const World &w) const {
            return ((_id == w._id) && (_ts == w._ts) && (memcmp(_updatesMustBeSignedBy.data,
                                                                w._updatesMustBeSignedBy.data,
                                                                ZT_C25519_PUBLIC_KEY_LEN) == 0) &&
                    (memcmp(_signature.data, w._signature.data, ZT_C25519_SIGNATURE_LEN) == 0) &&
         ...
         */
        static inline World make(World::Type t, uint64_t id, uint64_t ts, const C25519::Public &sk,
                                 const std::vector<World::Root> &roots,
                                 const C25519::Pair &signWith) {
            World w;
            w._id = id;
            w._ts = ts;
            w._type = t;
