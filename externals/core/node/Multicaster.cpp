...
namespace ZeroTier {
    ...
    void Multicaster::addMultiple(void *tPtr, int64_t now, uint64_t nwid, const MulticastGroup &mg,
                                  const void *addresses, unsigned int count,
                                  unsigned int totalKnown) {
    ...
    void Multicaster::remove(uint64_t nwid, const MulticastGroup &mg, const Address &member) {
        ...
        if (s) {
            for (std::vector<MulticastGroupMember>::iterator m(s->members.begin());
                 m != s->members.end(); ++m) {
                if (m->address == member) {
                    s->members.erase(m);
                    break;
    ...
    unsigned int
    Multicaster::gather(const Address &queryingPeer, uint64_t nwid, const MulticastGroup &mg,
                        Buffer<ZT_PROTO_MAX_PACKET_LENGTH> &appendTo, unsigned int limit) const {
        unsigned char *p;
        ...
        if ((s) && (!s->members.empty())) {
            ...
            k = 0;
            while ((added < limit) && (k < s->members.size()) &&
                   ((appendTo.size() + ZT_ADDRESS_LENGTH) <= ZT_PROTO_MAX_PACKET_LENGTH)) {
                ...
                if (queryingPeer.toInt() !=
                    a) { // do not return the peer that is making the request as a result
    ...
    std::vector<Address>
    Multicaster::getMembers(uint64_t nwid, const MulticastGroup &mg, unsigned int limit) const {
        ...
        for (std::vector<MulticastGroupMember>::const_reverse_iterator m(s->members.rbegin());
             m != s->members.rend(); ++m) {
    ...
    void Multicaster::send(
        ...
        if (!network->config().isActiveBridge(RR->identity.address())) {
            ...
            const unsigned int multicastReplicatorCount = network->config().multicastReplicators(
                    multicastReplicators);
            if (multicastReplicatorCount) {
                if (std::find(multicastReplicators, multicastReplicators + multicastReplicatorCount,
                              RR->identity.address()) ==
                    (multicastReplicators + multicastReplicatorCount)) {
                    ...
                    for (unsigned int i = 0; i < multicastReplicatorCount; ++i) {
                        const SharedPtr<Peer> p(
                                RR->topology->getPeerNoCache(multicastReplicators[i]));
                        if ((p) && (p->isAlive(now))) {
                    ...
                    if (bestMulticastReplicator) {
                        Packet outp(bestMulticastReplicator->address(), RR->identity.address(),
                        ...
                        outp.armor(bestMulticastReplicator->key(), true,
                                   bestMulticastReplicator->aesKeysIfSupported());
            ...
            if (gs.members.size() >= limit) {
                ...
                for (unsigned int i = 0; i < activeBridgeCount; ++i) {
                    if ((activeBridges[i] != RR->identity.address()) &&
                        (activeBridges[i] != origin)) {
                        out.sendOnly(RR, tPtr,
                                     activeBridges[i]); // optimization: don't use dedup log if it's a one-pass send
                        if (++count >= limit) {
                            break;
                        }
                    }
                ...
                while ((count < limit) && (idx < gs.members.size())) {
                    ...
                    if ((std::find(activeBridges, activeBridges + activeBridgeCount, ma) ==
                         (activeBridges + activeBridgeCount)) && (ma != origin)) {
                        out.sendOnly(RR, tPtr,
                                     ma); // optimization: don't use dedup log if it's a one-pass send
                        ++count;
                    }
                }
            } else {
                ...
                if ((gs.members.empty()) || ((now - gs.lastExplicitGather) >=
                                             (ZT_MULTICAST_EXPLICIT_GATHER_DELAY * timerScale))) {
                    ...
                    for (std::vector<Address>::const_iterator a(anchors.begin());
                         a != anchors.end(); ++a) {
                    ...
                    for (unsigned int k = 0; k < numExplicitGatherPeers; ++k) {
                        const CertificateOfMembership *com = (network) ? ((network->config().com)
                                                                          ? &(network->config().com)
                                                                          : (const CertificateOfMembership *) 0)
                                                                       : (const CertificateOfMembership *) 0;
                        Packet outp(explicitGatherPeers[k], RR->identity.address(),
                                    Packet::VERB_MULTICAST_GATHER);
    ...
    void Multicaster::clean(int64_t now) {
        ...
        while (mm.next(k, s)) {
            for (std::list<OutboundMulticast>::iterator tx(s->txQueue.begin());
                 tx != s->txQueue.end();) {
    ...
    void Multicaster::_add(void *tPtr, int64_t now, uint64_t nwid, const MulticastGroup &mg,
                           MulticastGroupStatus &gs, const Address &member) {
        ...
        std::vector<MulticastGroupMember>::iterator m(
                std::lower_bound(gs.members.begin(), gs.members.end(), member));
        if (m != gs.members.end()) {
            if (m->address == member) {
                m->timestamp = now;
                return;
        ...
        for (std::list<OutboundMulticast>::iterator tx(gs.txQueue.begin());
             tx != gs.txQueue.end();) {
            if (tx->atLimit()) {
                gs.txQueue.erase(tx++);
            } else {
