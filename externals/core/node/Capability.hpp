 ...
 */
    ...
    public:
        static inline Credential::Type
        credentialType() { return Credential::CREDENTIAL_TYPE_CAPABILITY; }
         ...
         */
        Capability(uint32_t id, uint64_t nwid, int64_t ts, unsigned int mccl,
                   const ZT_VirtualNetworkRule *rules, unsigned int ruleCount) :
                _nwid(nwid),
                _ts(ts),
                _id(id),
                _maxCustodyChainLength(
                        (mccl > 0) ? ((mccl < ZT_MAX_CAPABILITY_CUSTODY_CHAIN_LENGTH) ? mccl
                                                                                      : (unsigned int) ZT_MAX_CAPABILITY_CUSTODY_CHAIN_LENGTH)
                                   : 1),
                _ruleCount((ruleCount < ZT_MAX_CAPABILITY_RULES) ? ruleCount
        ...
        inline bool sign(const Identity &from, const Address &to) {
            try {
                for (unsigned int i = 0; ((i < _maxCustodyChainLength) &&
                                          (i < ZT_MAX_CAPABILITY_CUSTODY_CHAIN_LENGTH)); ++i) {
                    if (!(_custody[i].to)) {
        ...
        static inline void
        serializeRules(Buffer<C> &b, const ZT_VirtualNetworkRule *rules, unsigned int ruleCount) {
        ...
        template<unsigned int C>
        static inline void
        deserializeRules(const Buffer<C> &b, unsigned int &p, ZT_VirtualNetworkRule *rules,
                         unsigned int &ruleCount, const unsigned int maxRuleCount) {
            while ((ruleCount < maxRuleCount) && (p < b.size())) {
                ...
                switch ((ZT_VirtualNetworkRuleType) (rules[ruleCount].t & 0x3f)) {
                    ...
                    case ZT_NETWORK_RULE_MATCH_SOURCE_ZEROTIER_ADDRESS:
                    case ZT_NETWORK_RULE_MATCH_DEST_ZEROTIER_ADDRESS:
                        rules[ruleCount].v.zt = Address(b.field(p, ZT_ADDRESS_LENGTH),
                                                        ZT_ADDRESS_LENGTH).toInt();
                        break;
                    case ZT_NETWORK_RULE_MATCH_VLAN_ID:
                    ...
                    case ZT_NETWORK_RULE_MATCH_INTEGER_RANGE:
                        ...
                        rules[ruleCount].v.intRange.end = (uint32_t) (
                                b.template at<uint64_t>(p + 8) - rules[ruleCount].v.intRange.start);
        ...
        inline void serialize(Buffer<C> &b, const bool forSign = false) const {
            ...
            if (!forSign) {
                for (unsigned int i = 0;; ++i) {
                    if ((i < _maxCustodyChainLength) &&
                        (i < ZT_MAX_CAPABILITY_CUSTODY_CHAIN_LENGTH) && (_custody[i].to)) {
                        ...
                        b.append((unsigned char) 0,
                                 ZT_ADDRESS_LENGTH); // zero 'to' terminates chain
                        break;
                    }
                }
            }
        ...
        inline unsigned int deserialize(const Buffer<C> &b, unsigned int startAt = 0) {
            *this = Capability();
            ...
            if ((_maxCustodyChainLength < 1) ||
                (_maxCustodyChainLength > ZT_MAX_CAPABILITY_CUSTODY_CHAIN_LENGTH)) {
                throw ZT_EXCEPTION_INVALID_SERIALIZED_DATA_OVERFLOW;
            }
            ...
            for (unsigned int i = 0;; ++i) {
                ...
                if ((i >= _maxCustodyChainLength) ||
                    (i >= ZT_MAX_CAPABILITY_CUSTODY_CHAIN_LENGTH)) {
                    throw ZT_EXCEPTION_INVALID_SERIALIZED_DATA_OVERFLOW;
                }
                _custody[i].to = to;
                ...
                if (b[p++] == 1) {
                    ...
                    p += 2;
                    memcpy(_custody[i].signature.data, b.field(p, ZT_C25519_SIGNATURE_LEN),
