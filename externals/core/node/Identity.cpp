...
// A memory-hard composition of SHA-512 and Salsa20 for hashcash hashing
    static inline void
    _computeMemoryHardHash(const void *publicKey, unsigned int publicKeyBytes, void *digest,
                           void *genmem) {
        ...
        for (unsigned long i = 0; i < (ZT_IDENTITY_GEN_MEMORY / sizeof(uint64_t));) {
            unsigned long idx1 = (unsigned long) (Utils::ntoh(((uint64_t *) genmem)[i++]) %
                                                  (64 / sizeof(uint64_t)));
            unsigned long idx2 = (unsigned long) (Utils::ntoh(((uint64_t *) genmem)[i++]) %
                                                  (ZT_IDENTITY_GEN_MEMORY / sizeof(uint64_t)));
...
// threshold value.
    ...
    char *
    Identity::toString(bool includePrivate, char buf[ZT_IDENTITY_STRING_BUFFER_LENGTH]) const {
        char *p = buf;
    ...
    bool Identity::fromString(const char *str) {
        ...
        int fno = 0;
        char *saveptr = (char *) 0;
        for (char *f = Utils::stok(tmp, ":", &saveptr); (f); f = Utils::stok((char *) 0, ":",
                                                                             &saveptr)) {
            switch (fno++) {
                case 0:
                ...
                case 1:
                    ...
                    break;
                case 2:
                    if (Utils::unhex(f, _publicKey.data, ZT_C25519_PUBLIC_KEY_LEN) !=
                ...
                case 3:
                    _privateKey = new C25519::Private();
                    if (Utils::unhex(f, _privateKey->data, ZT_C25519_PRIVATE_KEY_LEN) !=
                        ZT_C25519_PRIVATE_KEY_LEN) {
                        _address.zero();
                        return false;
                    }
                    break;
