...
namespace ZeroTier {
    ...
    void Membership::pushCredentials(const RuntimeEnvironment *RR, void *tPtr, const int64_t now,
        ...
        while ((capPtr < sendCapCount) || (tagPtr < sendTagCount) || (cooPtr < sendCooCount) ||
               (sendCom)) {
            ...
            while ((capPtr < sendCapCount) &&
                   ((outp.size() + sizeof(Capability) + 16) < ZT_PROTO_MAX_PACKET_LENGTH)) {
            ...
            while ((tagPtr < sendTagCount) &&
                   ((outp.size() + sizeof(Tag) + 16) < ZT_PROTO_MAX_PACKET_LENGTH)) {
            ...
            while ((cooPtr < sendCooCount) && ((outp.size() + sizeof(CertificateOfOwnership) + 16) <
    ...
    Membership::addCredential(const RuntimeEnvironment *RR, void *tPtr, const NetworkConfig &nconf,
...
// Template out addCredential() for many cred types to avoid copypasta
    template<typename C>
    static Membership::AddCredentialResult _addCredImpl(Hashtable<uint32_t, C> &remoteCreds,
                                                        const Hashtable<uint64_t, int64_t> &revocations,
                                                        const RuntimeEnvironment *RR, void *tPtr,
                                                        const NetworkConfig &nconf, const C &cred) {
        ...
        const int64_t *const rt = revocations.get(
                Membership::credentialKey(C::credentialType(), cred.id()));
    ...
    Membership::addCredential(const RuntimeEnvironment *RR, void *tPtr, const NetworkConfig &nconf,
        ...
        return _addCredImpl<CertificateOfOwnership>(_remoteCoos, _revocations, RR, tPtr, nconf,
                                                    coo);
    }
