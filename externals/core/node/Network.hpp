 ...
 */
    ...
    public:
         ...
         */
        uint64_t handleConfigChunk(void *tPtr, const uint64_t packetId, const Address &source,
         ...
         */
        void
        setAuthenticationRequired(void *tPtr, const char *issuerURL, const char *centralEndpoint,
                                  const char *clientID, const char *ssoProvider, const char *nonce,
                                  const char *state);
        ...
        addCredential(void *tPtr, const CertificateOfMembership &com);
         ...
         */
        Membership::AddCredentialResult
        addCredential(void *tPtr, const Address &sentFrom, const Revocation &rev);

        /**
         * Validate a credential and learn it if it passes certificate and other checks
         */
        inline Membership::AddCredentialResult
        addCredential(void *tPtr, const CertificateOfOwnership &coo) {
        ...
        inline void peerRequestedCredentials(void *tPtr, const Address &to, const int64_t now) {
            ...
            if ((lastPushed < _lastConfigUpdate) ||
                ((now - lastPushed) > ZT_PEER_CREDENTIALS_REQUEST_RATE_LIMIT)) {
        ...
        inline void pushCredentialsIfNeeded(void *tPtr, const Address &to, const int64_t now) {
            ...
            const int64_t lastPushed = m.lastPushedCredentials();
            if ((lastPushed < _lastConfigUpdate) ||
                ((now - lastPushed) > ZT_PEER_ACTIVITY_TIMEOUT)) {
                m.pushCredentials(RR, tPtr, now, to, _config);
            }
        }
    ...
    private:
        ...
        void _announceMulticastGroupsTo(void *tPtr, const Address &peer,
