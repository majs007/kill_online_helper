...
namespace ZeroTier {
        ...
        try {
            ...
            if (c == ZT_PROTO_CIPHER_SUITE__NO_CRYPTO_TRUSTED_PATH) {
                ...
                if (RR->topology->shouldInboundPathBeTrusted(_path->address(), tpid)) {
                    ...
                    RR->t->incomingPacketMessageAuthenticationFailure(tPtr, _path, packetId(),
                                                                      sourceAddress, hops(),
                                                                      "path not trusted");
                    return true;
                }
            } else if ((c == ZT_PROTO_CIPHER_SUITE__C25519_POLY1305_NONE) &&
            ...
            if (peer) {
                ...
                if (!uncompress()) {
                    RR->t->incomingPacketInvalid(tPtr, _path, packetId(), sourceAddress, hops(),
                                                 Packet::VERB_NOP, "LZ4 decompression failed");
                    return true;
                }
                ...
                switch (v) {
                    ...
                    default: // ignore unknown verbs, but if they pass auth check they are "received"
                        Metrics::pkt_nop_in++;
                        peer->received(tPtr, _path, hops(), packetId(), payloadLength(), v, 0,
    ...
    bool IncomingPacket::_doERROR(const RuntimeEnvironment *RR, void *tPtr,
        ...
        switch (errorCode) {
            ...
            case Packet::ERROR_OBJ_NOT_FOUND:
                ...
                if (inReVerb == Packet::VERB_NETWORK_CONFIG_REQUEST) {
                    const SharedPtr<Network> network(
                            RR->node->network(at<uint64_t>(ZT_PROTO_VERB_ERROR_IDX_PAYLOAD)));
            ...
            case Packet::ERROR_NETWORK_AUTHENTICATION_REQUIRED: {
                ...
                if ((network) && (network->controller() == peer->address())) {
                    ...
                    if (s > 2) {
                        const uint16_t errorDataSize = at<uint16_t>(
                                ZT_PROTO_VERB_ERROR_IDX_PAYLOAD + 8);
                        s -= 2;
                        if (s >= (int) errorDataSize) {
                            Dictionary<8192> authInfo(((const char *) this->data()) +
                                                      (ZT_PROTO_VERB_ERROR_IDX_PAYLOAD + 10),
                                                      errorDataSize);
                            ...
                            if (authVer == 0) {
                                ...
                                if (authInfo.get(ZT_AUTHINFO_DICT_KEY_AUTHENTICATION_URL,
                                                 authenticationURL, sizeof(authenticationURL)) >
                                    0) {
                                    authenticationURL[sizeof(authenticationURL) -
                                                      1] = 0; // ensure always zero terminated
                                ...
                                if (authInfo.get(ZT_AUTHINFO_DICT_KEY_ISSUER_URL, issuerURL,
                                                 sizeof(issuerURL)) > 0) {
                                    issuerURL[sizeof(issuerURL) - 1] = 0;
                                }
                                if (authInfo.get(ZT_AUTHINFO_DICT_KEY_CENTRAL_ENDPOINT_URL,
                                                 centralAuthURL, sizeof(centralAuthURL)) > 0) {
                                    centralAuthURL[sizeof(centralAuthURL) - 1] = 0;
                                }
                                if (authInfo.get(ZT_AUTHINFO_DICT_KEY_NONCE, ssoNonce,
                                                 sizeof(ssoNonce)) > 0) {
                                    ssoNonce[sizeof(ssoNonce) - 1] = 0;
                                }
                                if (authInfo.get(ZT_AUTHINFO_DICT_KEY_STATE, ssoState,
                                                 sizeof(ssoState)) > 0) {
                                    ssoState[sizeof(ssoState) - 1] = 0;
                                }
                                if (authInfo.get(ZT_AUTHINFO_DICT_KEY_CLIENT_ID, ssoClientID,
                                                 sizeof(ssoClientID)) > 0) {
                                    ssoClientID[sizeof(ssoClientID) - 1] = 0;
                                }
                                if (authInfo.get(ZT_AUTHINFO_DICT_KEY_SSO_PROVIDER, ssoProvider,
                                                 sizeof(ssoProvider)) > 0) {
                                    ssoProvider[sizeof(ssoProvider) - 1] = 0;
                                } else {
                                    strncpy(ssoProvider, "default", sizeof(ssoProvider));
                                }

                                network->setAuthenticationRequired(tPtr, issuerURL, centralAuthURL,
                                                                   ssoClientID, ssoProvider,
                                                                   ssoNonce, ssoState);
                            }
                        }
                    } else {
        ...
        peer->received(tPtr, _path, hops(), packetId(), payloadLength(), Packet::VERB_ERROR,
                       inRePacketId, inReVerb, false, networkId, ZT_QOS_NO_FLOW);

        return true;
    }

    bool
    IncomingPacket::_doACK(const RuntimeEnvironment *RR, void *tPtr, const SharedPtr<Peer> &peer) {
        /*
    ...
    bool IncomingPacket::_doQOS_MEASUREMENT(const RuntimeEnvironment *RR, void *tPtr,
                                            const SharedPtr<Peer> &peer) {
        Metrics::pkt_qos_in++;
    ...
    bool IncomingPacket::_doHELLO(const RuntimeEnvironment *RR, void *tPtr,
                                  const bool alreadyAuthenticated) {
        ...
        Identity id;
        unsigned int ptr = ZT_PROTO_VERB_HELLO_IDX_IDENTITY +
                           id.deserialize(*this, ZT_PROTO_VERB_HELLO_IDX_IDENTITY);
            ...
            if (!alreadyAuthenticated) {
                if (peer->identity() != id) {
                    ...
                    if (RR->identity.agree(id, key)) {
                        if (dearmor(key,
                                    peer->aesKeysIfSupported())) { // ensure packet is authentic, otherwise drop
                            RR->t->incomingPacketDroppedHELLO(tPtr, _path, pid, fromAddress,
                                                              "address collision");
                            ...
                            RR->t->incomingPacketMessageAuthenticationFailure(tPtr, _path, pid,
                                                                              fromAddress, hops(),
                                                                              "invalid MAC");
                        }
                    } else {
        ...
        if (ptr < size()) {
            ...
            if ((externalSurfaceAddress) && (hops() == 0)) {
                RR->sa->iam(tPtr, id.address(), _path->localSocket(), _path->address(),
                            externalSurfaceAddress, RR->topology->isUpstream(id), now);
            }
        }
                ...
                for (unsigned int i = 0; i < numMoons; ++i) {
                    if ((World::Type) (*this)[ptr++] == World::TYPE_MOON) {
                        moonIdsAndTimestamps.push_back(
                                std::pair<uint64_t, uint64_t>(at<uint64_t>(ptr),
                                                              at<uint64_t>(ptr + 8)));
                    }
                    ptr += 16;
                }
        ...
        if (!moonIdsAndTimestamps.empty()) {
            ...
            for (std::vector<World>::const_iterator m(moons.begin()); m != moons.end(); ++m) {
                for (std::vector<std::pair<uint64_t, uint64_t> >::const_iterator i(
                        moonIdsAndTimestamps.begin()); i != moonIdsAndTimestamps.end(); ++i) {
                    if (i->first == m->id()) {
        ...
        peer->recordOutgoingPacket(_path, outp.packetId(), outp.payloadLength(), outp.verb(),
                                   ZT_QOS_NO_FLOW, now);
        Metrics::pkt_ok_out++;
        ...
        peer->received(tPtr, _path, hops(), pid, payloadLength(), Packet::VERB_HELLO, 0,
                       Packet::VERB_NOP, false, 0, ZT_QOS_NO_FLOW);
    ...
    bool
    IncomingPacket::_doOK(const RuntimeEnvironment *RR, void *tPtr, const SharedPtr<Peer> &peer) {
        Metrics::pkt_ok_in++;
        ...
        switch (inReVerb) {
            ...
            case Packet::VERB_HELLO: {
                const uint64_t latency =
                        RR->node->now() - at<uint64_t>(ZT_PROTO_VERB_HELLO__OK__IDX_TIMESTAMP);
                ...
                if ((externalSurfaceAddress) && (hops() == 0)) {
                    RR->sa->iam(tPtr, peer->address(), _path->localSocket(), _path->address(),
                                externalSurfaceAddress, RR->topology->isUpstream(peer->identity()),
                                RR->node->now());
                }
            }
                break;
            ...
            case Packet::VERB_WHOIS:
                if (RR->topology->isUpstream(peer->identity())) {
                    ...
                    RR->sw->doAnythingWaitingForPeer(tPtr, RR->topology->addPeer(tPtr,
                                                                                 SharedPtr<Peer>(
                                                                                         new Peer(
                                                                                                 RR,
                                                                                                 RR->identity,
            ...
            case Packet::VERB_NETWORK_CONFIG_REQUEST: {
                ...
                const SharedPtr<Network> network(RR->node->network(networkId));
                if (network) {
                    network->handleConfigChunk(tPtr, packetId(), source(), *this,
            ...
            case Packet::VERB_MULTICAST_GATHER: {
                ...
                if (network) {
                    const MulticastGroup mg(
                            MAC(field(ZT_PROTO_VERB_MULTICAST_GATHER__OK__IDX_MAC, 6), 6),
                            at<uint32_t>(ZT_PROTO_VERB_MULTICAST_GATHER__OK__IDX_ADI));
                    const unsigned int count = at<uint16_t>(
                            ZT_PROTO_VERB_MULTICAST_GATHER__OK__IDX_GATHER_RESULTS + 4);
                    RR->mc->addMultiple(tPtr, RR->node->now(), networkId, mg,
                                        field(ZT_PROTO_VERB_MULTICAST_GATHER__OK__IDX_GATHER_RESULTS +
                                              6, count * 5), count, at<uint32_t>(
                                    ZT_PROTO_VERB_MULTICAST_GATHER__OK__IDX_GATHER_RESULTS));
                }
            }
                break;
            ...
            case Packet::VERB_MULTICAST_FRAME: {
                ...
                const MulticastGroup mg(
                        MAC(field(ZT_PROTO_VERB_MULTICAST_FRAME__OK__IDX_MAC, 6), 6),
                ...
                if (network) {
                    ...
                    if ((flags & 0x01) != 0) { // deprecated but still used by older peers
                        ...
                        offset += com.deserialize(*this,
                                                  ZT_PROTO_VERB_MULTICAST_FRAME__OK__IDX_COM_AND_GATHER_RESULTS);
                        if (com) {
                    ...
                    if ((flags & 0x02) != 0) {
                        ...
                        offset += 2;
                        RR->mc->addMultiple(tPtr, RR->node->now(), networkId, mg,
                                            field(offset, count * 5), count, totalKnown);
                    }
                }
            }
                break;
        ...
        peer->received(tPtr, _path, hops(), packetId(), payloadLength(), Packet::VERB_OK,
                       inRePacketId, inReVerb, false, networkId, ZT_QOS_NO_FLOW);

        return true;
    }

    bool IncomingPacket::_doWHOIS(const RuntimeEnvironment *RR, void *tPtr,
                                  const SharedPtr<Peer> &peer) {
        if ((!RR->topology->amUpstream()) &&
            (!peer->rateGateInboundWhoisRequest(RR->node->now()))) {
            return true;
        }
        ...
        peer->received(tPtr, _path, hops(), packetId(), payloadLength(), Packet::VERB_WHOIS, 0,
                       Packet::VERB_NOP, false, 0, ZT_QOS_NO_FLOW);

        return true;
    }

    bool IncomingPacket::_doRENDEZVOUS(const RuntimeEnvironment *RR, void *tPtr,
                                       const SharedPtr<Peer> &peer) {
        Metrics::pkt_rendezvous_in++;
        if (RR->topology->isUpstream(peer->identity())) {
            const Address with(field(ZT_PROTO_VERB_RENDEZVOUS_IDX_ZTADDRESS, ZT_ADDRESS_LENGTH),
                               ZT_ADDRESS_LENGTH);
            ...
            if (rendezvousWith) {
                ...
                if ((port > 0) && ((addrlen == 4) || (addrlen == 16))) {
                    InetAddress atAddr(field(ZT_PROTO_VERB_RENDEZVOUS_IDX_ADDRESS, addrlen),
                                       addrlen, port);
                    if (RR->node->shouldUsePathForZeroTierTraffic(tPtr, with, _path->localSocket(),
                                                                  atAddr)) {
                        ...
                        rendezvousWith->attemptToContactAt(tPtr, _path->localSocket(), atAddr,
                                                           RR->node->now(), false);
                    }
                }
            }
        ...
        peer->received(tPtr, _path, hops(), packetId(), payloadLength(), Packet::VERB_RENDEZVOUS, 0,
                       Packet::VERB_NOP, false, 0, ZT_QOS_NO_FLOW);
...
// Returns true if packet appears valid; pos and proto will be set
    static bool _ipv6GetPayload(const uint8_t *frameData, unsigned int frameLen, unsigned int &pos,
                                unsigned int &proto) {
        if (frameLen < 40) {
            return false;
        }
    ...
    bool
    IncomingPacket::_doFRAME(const RuntimeEnvironment *RR, void *tPtr, const SharedPtr<Peer> &peer,
                             int32_t flowId) {
            ...
            if (size() > ZT_PROTO_VERB_EXT_FRAME_IDX_PAYLOAD) {
                ...
                if (etherType == ZT_ETHERTYPE_IPV4 && (frameLen >= 20)) {
                    ...
                    const unsigned int headerLen =
                            4 * (reinterpret_cast<const uint8_t *>(frameData)[0] & 0xf);
                    switch (proto) {
                        case 0x01: // ICMP
                            //flowId = 0x01;
                            break;
                        ...
                        case 0x88: // UDPLite
                            if (frameLen > (headerLen + 4)) {
                                ...
                                srcPort =
                                        (reinterpret_cast<const uint8_t *>(frameData)[pos++]) << 8;
        ...
        if (network) {
            if (network->gate(tPtr, peer)) {
                ...
                if (size() > ZT_PROTO_VERB_FRAME_IDX_PAYLOAD) {
                    ...
                    const unsigned int frameLen = size() - ZT_PROTO_VERB_FRAME_IDX_PAYLOAD;
                    const uint8_t *const frameData = reinterpret_cast<const uint8_t *>(data()) +
                                                     ZT_PROTO_VERB_FRAME_IDX_PAYLOAD;
                    if (network->filterIncomingPacket(tPtr, peer, RR->identity.address(), sourceMac,
                                                      network->mac(), frameData, frameLen,
                                                      etherType, 0) > 0) {
                        RR->node->putFrame(tPtr, nwid, network->userPtr(), sourceMac,
                                           network->mac(), etherType, 0, (const void *) frameData,
                                           frameLen);
                    }
                }
            } else {
        ...
        peer->received(tPtr, _path, hops(), packetId(), payloadLength(), Packet::VERB_FRAME, 0,
                       Packet::VERB_NOP, trustEstablished, nwid, _flowId);

        return true;
    }

    bool IncomingPacket::_doEXT_FRAME(const RuntimeEnvironment *RR, void *tPtr,
                                      const SharedPtr<Peer> &peer, int32_t flowId) {
        ...
        if (network) {
            ...
            if ((flags & 0x01) !=
                0) { // inline COM with EXT_FRAME is deprecated but still used with old peers
                CertificateOfMembership com;
            ...
            if (!network->gate(tPtr, peer)) {
                RR->t->incomingNetworkAccessDenied(tPtr, network, _path, packetId(), size(),
                                                   peer->address(), Packet::VERB_EXT_FRAME, true);
                _sendErrorNeedCredentials(RR, tPtr, peer, nwid);
                return false;
            }

            if (size() > ZT_PROTO_VERB_EXT_FRAME_IDX_PAYLOAD) {
                const unsigned int etherType = at<uint16_t>(
                        comLen + ZT_PROTO_VERB_EXT_FRAME_IDX_ETHERTYPE);
                const MAC to(field(comLen + ZT_PROTO_VERB_EXT_FRAME_IDX_TO,
                                   ZT_PROTO_VERB_EXT_FRAME_LEN_TO), ZT_PROTO_VERB_EXT_FRAME_LEN_TO);
                const MAC from(field(comLen + ZT_PROTO_VERB_EXT_FRAME_IDX_FROM,
                                     ZT_PROTO_VERB_EXT_FRAME_LEN_FROM),
                               ZT_PROTO_VERB_EXT_FRAME_LEN_FROM);
                const unsigned int frameLen =
                        size() - (comLen + ZT_PROTO_VERB_EXT_FRAME_IDX_PAYLOAD);
                const uint8_t *const frameData = (const uint8_t *) field(
                        comLen + ZT_PROTO_VERB_EXT_FRAME_IDX_PAYLOAD, frameLen);

                if ((!from) || (from == network->mac())) {
                    peer->received(tPtr, _path, hops(), packetId(), payloadLength(),
                                   Packet::VERB_EXT_FRAME, 0, Packet::VERB_NOP, true, nwid,
                                   flowId); // trustEstablished because COM is okay
                    return true;
                }

                switch (network->filterIncomingPacket(tPtr, peer, RR->identity.address(), from, to,
                    ...
                    case 1:
                        if (from != MAC(peer->address(), nwid)) {
                            if (network->config().permitsBridging(peer->address())) {
                                ...
                                RR->t->incomingNetworkFrameDropped(tPtr, network, _path, packetId(),
                                                                   size(), peer->address(),
                                                                   Packet::VERB_EXT_FRAME, from, to,
                                                                   "bridging not allowed (remote)");
                                peer->received(tPtr, _path, hops(), packetId(), payloadLength(),
                                               Packet::VERB_EXT_FRAME, 0, Packet::VERB_NOP, true,
                                               nwid,
                                               flowId); // trustEstablished because COM is okay
                                return true;
                            }
                            ...
                            if (to.isMulticast()) {
                                if (network->config().multicastLimit == 0) {
                                    RR->t->incomingNetworkFrameDropped(tPtr, network, _path,
                                                                       packetId(), size(),
                                                                       peer->address(),
                                                                       Packet::VERB_EXT_FRAME, from,
                                                                       to, "multicast disabled");
                                    peer->received(tPtr, _path, hops(), packetId(), payloadLength(),
                                                   Packet::VERB_EXT_FRAME, 0, Packet::VERB_NOP,
                                                   true, nwid,
                                                   flowId); // trustEstablished because COM is okay
                                    return true;
                                }
                    ...
                    case 2:
                        RR->node->putFrame(tPtr, nwid, network->userPtr(), from, to, etherType, 0,
                                           (const void *) frameData, frameLen);
                        break;
                }
            }
            ...
            if ((flags & 0x10) != 0) { // ACK requested
                ...
                const int64_t now = RR->node->now();
                outp.armor(peer->key(), true, peer->aesKeysIfSupported());
                peer->recordOutgoingPacket(_path, outp.packetId(), outp.payloadLength(),
                                           outp.verb(), ZT_QOS_NO_FLOW, now);
                Metrics::pkt_ok_out++;
                _path->send(RR, tPtr, outp.data(), outp.size(), RR->node->now());
            }

            peer->received(tPtr, _path, hops(), packetId(), payloadLength(), Packet::VERB_EXT_FRAME,
                           0, Packet::VERB_NOP, true, nwid, flowId);
        } else {
            peer->received(tPtr, _path, hops(), packetId(), payloadLength(), Packet::VERB_EXT_FRAME,
                           0, Packet::VERB_NOP, false, nwid, flowId);
        }
    ...
    bool
    IncomingPacket::_doECHO(const RuntimeEnvironment *RR, void *tPtr, const SharedPtr<Peer> &peer) {
        Metrics::pkt_echo_in++;
        ...
        peer->received(tPtr, _path, hops(), pid, payloadLength(), Packet::VERB_ECHO, 0,
                       Packet::VERB_NOP, false, 0, ZT_QOS_NO_FLOW);

        return true;
    }

    bool IncomingPacket::_doMULTICAST_LIKE(const RuntimeEnvironment *RR, void *tPtr,
                                           const SharedPtr<Peer> &peer) {
            ...
            if (nwid != lastNwid) {
                ...
                if (!authorized) {
                    authorized = ((RR->topology->amUpstream()) ||
                                  (RR->node->localControllerHasAuthorized(now, nwid,
                                                                          peer->address())));
                }
            }
            if (authorized) {
                RR->mc->add(tPtr, now, nwid,
                            MulticastGroup(MAC(field(ptr + 8, 6), 6), at<uint32_t>(ptr + 14)),
                            peer->address());
            }
        }

        peer->received(tPtr, _path, hops(), packetId(), payloadLength(),
                       Packet::VERB_MULTICAST_LIKE, 0, Packet::VERB_NOP, false, 0, ZT_QOS_NO_FLOW);
        return true;
    }

    bool IncomingPacket::_doNETWORK_CREDENTIALS(const RuntimeEnvironment *RR, void *tPtr,
                                                const SharedPtr<Peer> &peer) {
        ...
        peer->received(tPtr, _path, hops(), packetId(), payloadLength(),
                       Packet::VERB_NETWORK_CREDENTIALS, 0, Packet::VERB_NOP, trustEstablished,
                       (network) ? network->id() : 0, ZT_QOS_NO_FLOW);

        return true;
    }

    bool IncomingPacket::_doNETWORK_CONFIG_REQUEST(const RuntimeEnvironment *RR, void *tPtr,
                                                   const SharedPtr<Peer> &peer) {
        ...
        if (RR->localNetworkController) {
            const unsigned int metaDataLength = (ZT_PROTO_VERB_NETWORK_CONFIG_REQUEST_IDX_DICT_LEN <=
                                                 size()) ? at<uint16_t>(
                    ZT_PROTO_VERB_NETWORK_CONFIG_REQUEST_IDX_DICT_LEN) : 0;
            const char *metaDataBytes = (metaDataLength != 0) ? (const char *) field(
                    ZT_PROTO_VERB_NETWORK_CONFIG_REQUEST_IDX_DICT, metaDataLength)
                                                              : (const char *) 0;
            const Dictionary<ZT_NETWORKCONFIG_METADATA_DICT_CAPACITY> metaData(metaDataBytes,
                                                                               metaDataLength);
            RR->localNetworkController->request(nwid,
                                                (hopCount > 0) ? InetAddress() : _path->address(),
                                                requestPacketId, peer->identity(), metaData);
        } else {
        ...
        peer->received(tPtr, _path, hopCount, requestPacketId, payloadLength(),
                       Packet::VERB_NETWORK_CONFIG_REQUEST, 0, Packet::VERB_NOP, false, nwid,
                       ZT_QOS_NO_FLOW);

        return true;
    }

    bool IncomingPacket::_doNETWORK_CONFIG(const RuntimeEnvironment *RR, void *tPtr,
                                           const SharedPtr<Peer> &peer) {
        Metrics::pkt_network_config_in++;
        const SharedPtr<Network> network(RR->node->network(at<uint64_t>(ZT_PACKET_IDX_PAYLOAD)));
        if (network) {
            const uint64_t configUpdateId = network->handleConfigChunk(tPtr, packetId(), source(),
                                                                       *this,
        ...
        peer->received(tPtr, _path, hops(), packetId(), payloadLength(),
                       Packet::VERB_NETWORK_CONFIG, 0, Packet::VERB_NOP, false,
                       (network) ? network->id() : 0, ZT_QOS_NO_FLOW);

        return true;
    }

    bool IncomingPacket::_doMULTICAST_GATHER(const RuntimeEnvironment *RR, void *tPtr,
                                             const SharedPtr<Peer> &peer) {
        ...
        const unsigned int gatherLimit = at<uint32_t>(
                ZT_PROTO_VERB_MULTICAST_GATHER_IDX_GATHER_LIMIT);
        ...
        const int64_t now = RR->node->now();
        if ((gatherLimit > 0) && ((trustEstablished) || (RR->topology->amUpstream()) ||
                                  (RR->node->localControllerHasAuthorized(now, nwid,
                                                                          peer->address())))) {
            Packet outp(peer->address(), RR->identity.address(), Packet::VERB_OK);
            ...
            const unsigned int gatheredLocally = RR->mc->gather(peer->address(), nwid, mg, outp,
                                                                gatherLimit);
        ...
        peer->received(tPtr, _path, hops(), packetId(), payloadLength(),
                       Packet::VERB_MULTICAST_GATHER, 0, Packet::VERB_NOP, trustEstablished, nwid,
                       ZT_QOS_NO_FLOW);

        return true;
    }

    bool IncomingPacket::_doMULTICAST_FRAME(const RuntimeEnvironment *RR, void *tPtr,
                                            const SharedPtr<Peer> &peer) {
        ...
        if (network) {
            ...
            const MulticastGroup to(MAC(field(offset
            +ZT_PROTO_VERB_MULTICAST_FRAME_IDX_DEST_MAC, 6),6),at<uint32_t>(
                    offset + ZT_PROTO_VERB_MULTICAST_FRAME_IDX_DEST_ADI));
            const unsigned int etherType = at<uint16_t>(
                    offset + ZT_PROTO_VERB_MULTICAST_FRAME_IDX_ETHERTYPE);
            const unsigned int frameLen =
                    size() - (offset + ZT_PROTO_VERB_MULTICAST_FRAME_IDX_FRAME);

            if (network->config().multicastLimit == 0) {
                RR->t->incomingNetworkFrameDropped(tPtr, network, _path, packetId(), size(),
                                                   peer->address(), Packet::VERB_MULTICAST_FRAME,
                                                   from, to.mac(), "multicast disabled");
                peer->received(tPtr, _path, hops(), packetId(), payloadLength(),
                               Packet::VERB_MULTICAST_FRAME, 0, Packet::VERB_NOP, false, nwid,
                               ZT_QOS_NO_FLOW);
                return true;
            }
            ...
            if ((frameLen > 0) && (frameLen <= ZT_MAX_MTU)) {
                if (!to.mac().isMulticast()) {
                    RR->t->incomingPacketInvalid(tPtr, _path, packetId(), source(), hops(),
                                                 Packet::VERB_MULTICAST_FRAME,
                                                 "destination not multicast");
                    peer->received(tPtr, _path, hops(), packetId(), payloadLength(),
                                   Packet::VERB_MULTICAST_FRAME, 0, Packet::VERB_NOP, true, nwid,
                ...
                const uint8_t *const frameData = (const uint8_t *) field(
                        offset + ZT_PROTO_VERB_MULTICAST_FRAME_IDX_FRAME, frameLen);

                if ((flags & 0x08) &&
                    (network->config().isMulticastReplicator(RR->identity.address()))) {
                    RR->mc->send(tPtr, RR->node->now(), network, peer->address(), to, from,
                                 etherType, frameData, frameLen);
                }
                ...
                if (from != MAC(peer->address(), nwid)) {
                    if (network->config().permitsBridging(peer->address())) {
                        ...
                        RR->t->incomingNetworkFrameDropped(tPtr, network, _path, packetId(), size(),
                                                           peer->address(),
                                                           Packet::VERB_MULTICAST_FRAME, from,
                                                           to.mac(),
                                                           "bridging not allowed (remote)");
                        peer->received(tPtr, _path, hops(), packetId(), payloadLength(),
                                       Packet::VERB_MULTICAST_FRAME, 0, Packet::VERB_NOP, true,
                                       nwid,
                                       ZT_QOS_NO_FLOW); // trustEstablished because COM is okay
                        return true;
                    }
                ...
                if (network->filterIncomingPacket(tPtr, peer, RR->identity.address(), from,
                                                  to.mac(), frameData, frameLen, etherType, 0) >
                    0) {
                    RR->node->putFrame(tPtr, nwid, network->userPtr(), from, to.mac(), etherType, 0,
                                       (const void *) frameData, frameLen);
                }
            }
    ...
    bool IncomingPacket::_doPUSH_DIRECT_PATHS(const RuntimeEnvironment *RR, void *tPtr,
                                              const SharedPtr<Peer> &peer) {
        ...
        if (!peer->rateGatePushDirectPaths(now)) {
            peer->received(tPtr, _path, hops(), packetId(), payloadLength(),
                           Packet::VERB_PUSH_DIRECT_PATHS, 0, Packet::VERB_NOP, false, 0,
                           ZT_QOS_NO_FLOW);
            return true;
        }
        ...
        while (count--) { // if ptr overflows Buffer will throw
            ...
            switch (addrType) {
                case 4: {
                    ...
                    if (
                            ((flags & ZT_PUSH_DIRECT_PATHS_FLAG_FORGET_PATH) == 0) &&
                            // not being told to forget
                            (!(((flags & ZT_PUSH_DIRECT_PATHS_FLAG_CLUSTER_REDIRECT) == 0) &&
                               (peer->hasActivePathTo(now, a)))) && // not already known
                            (RR->node->shouldUsePathForZeroTierTraffic(tPtr, peer->address(),
                                                                       _path->localSocket(),
                                                                       a))) // should use path
                    {
                        if ((flags & ZT_PUSH_DIRECT_PATHS_FLAG_CLUSTER_REDIRECT) != 0) {
                            peer->clusterRedirect(tPtr, _path, a, now);
                        } else if (++countPerScope[(int) a.ipScope()][0] <=
                ...
                case 6: {
                                                                       ...
                                                                       a))) // should use path
                    {
                        if ((flags & ZT_PUSH_DIRECT_PATHS_FLAG_CLUSTER_REDIRECT) != 0) {
                            peer->clusterRedirect(tPtr, _path, a, now);
                        } else if (++countPerScope[(int) a.ipScope()][1] <=
                                   ZT_PUSH_DIRECT_PATHS_MAX_PER_SCOPE_AND_FAMILY) {
                            peer->attemptToContactAt(tPtr, InetAddress(), a, now, false);
                        }
                    }
    ...
    bool IncomingPacket::_doUSER_MESSAGE(const RuntimeEnvironment *RR, void *tPtr,
                                         const SharedPtr<Peer> &peer) {
        ...
        if (likely(size() >= (ZT_PACKET_IDX_PAYLOAD + 8))) {
            ...
            um.data = reinterpret_cast<const void *>(reinterpret_cast<const uint8_t *>(data()) +
                                                     ZT_PACKET_IDX_PAYLOAD + 8);
            um.length = size() - (ZT_PACKET_IDX_PAYLOAD + 8);
            RR->node->postEvent(tPtr, ZT_EVENT_USER_MESSAGE, reinterpret_cast<const void *>(&um));
        }

        peer->received(tPtr, _path, hops(), packetId(), payloadLength(), Packet::VERB_USER_MESSAGE,
                       0, Packet::VERB_NOP, false, 0, ZT_QOS_NO_FLOW);

        return true;
    }

    bool IncomingPacket::_doREMOTE_TRACE(const RuntimeEnvironment *RR, void *tPtr,
                                         const SharedPtr<Peer> &peer) {
        ...
        peer->received(tPtr, _path, hops(), packetId(), payloadLength(), Packet::VERB_REMOTE_TRACE,
                       0, Packet::VERB_NOP, false, 0, ZT_QOS_NO_FLOW);

        return true;
    }

    bool IncomingPacket::_doPATH_NEGOTIATION_REQUEST(const RuntimeEnvironment *RR, void *tPtr,
                                                     const SharedPtr<Peer> &peer) {
    ...
    void IncomingPacket::_sendErrorNeedCredentials(const RuntimeEnvironment *RR, void *tPtr,
                                                   const SharedPtr<Peer> &peer,
                                                   const uint64_t nwid) {
