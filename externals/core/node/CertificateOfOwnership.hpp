 ...
 */
    ...
    public:
        ...
        CertificateOfOwnership(const uint64_t nwid, const int64_t ts, const Address &issuedTo,
                               const uint32_t id) {
            memset(reinterpret_cast<void *>(this), 0, sizeof(CertificateOfOwnership));
            _networkId = nwid;
            _ts = ts;
        ...
        inline bool owns(const InetAddress &ip) const {
            if (ip.ss_family == AF_INET) {
                return this->_owns(THING_IPV4_ADDRESS,
                                   &(reinterpret_cast<const struct sockaddr_in *>(&ip)->sin_addr.s_addr),
                                   4);
            }
            if (ip.ss_family == AF_INET6) {
                return this->_owns(THING_IPV6_ADDRESS,
        ...
        inline void addThing(const InetAddress &ip) {
            ...
            if (ip.ss_family == AF_INET) {
                ...
                memcpy(_thingValues[_thingCount],
                       &(reinterpret_cast<const struct sockaddr_in *>(&ip)->sin_addr.s_addr), 4);
                ++_thingCount;
            } else if (ip.ss_family == AF_INET6) {
                _thingTypes[_thingCount] = THING_IPV6_ADDRESS;
                memcpy(_thingValues[_thingCount],
                       reinterpret_cast<const struct sockaddr_in6 *>(&ip)->sin6_addr.s6_addr, 16);
                ++_thingCount;
            }
        }
            ...
            *this = CertificateOfOwnership();
            ...
            for (unsigned int i = 0, j = _thingCount; i < j; ++i) {
                if (i < ZT_CERTIFICATEOFOWNERSHIP_MAX_THINGS) {
                    ...
                    memcpy(_thingValues[i],
                           b.field(p, ZT_CERTIFICATEOFOWNERSHIP_MAX_THING_VALUE_SIZE),
                           ZT_CERTIFICATEOFOWNERSHIP_MAX_THING_VALUE_SIZE);
            ...
            if (b[p++] == 1) {
                ...
                p += 2;
                memcpy(_signature.data, b.field(p, ZT_C25519_SIGNATURE_LEN),
