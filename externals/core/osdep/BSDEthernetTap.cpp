...
namespace ZeroTier {
    ...
    BSDEthernetTap::BSDEthernetTap(
            ...
            void (*handler)(void *, void *, uint64_t, const MAC &, const MAC &, unsigned int,
        ...
        OSUtils::ztsnprintf(ethaddr, sizeof(ethaddr), "%.2x:%.2x:%.2x:%.2x:%.2x:%.2x", (int) mac[0],
        ...
        if (cpid == 0) {
...
#endif
            ::execl("/sbin/ifconfig", "/sbin/ifconfig", _dev.c_str(), "lladdr", ethaddr, "mtu",
                    mtustr, "metric", metstr, "up", (const char *) 0);
            ::_exit(-1);
        } else if (cpid > 0) {
            int exitcode = -1;
            ::waitpid(cpid, &exitcode, 0);
            if (exitcode) {
                ...
                throw std::runtime_error(
                        "ifconfig failure setting link-layer address and activating tap interface");
            }
        }
...
#endif
            execl("/sbin/ifconfig", "/sbin/ifconfig", _dev.c_str(), "inet", ip.toIpString(ipbuf),
                  "-alias", (const char *) 0);
            _exit(-1);
        } else if (cpid > 0) {
            int exitcode = -1;
...
#endif
            ::execl("/sbin/ifconfig", "/sbin/ifconfig", _dev.c_str(), ip.isV4() ? "inet" : "inet6",
                    ip.toString(tmp), "alias", (const char *) 0);
            ::_exit(-1);
        } else if (cpid > 0) {
            int exitcode = -1;
            ::waitpid(cpid, &exitcode, 0);
            ...
            if ((!strcmp(p->ifa_name, _dev.c_str())) && (p->ifa_addr) && (p->ifa_netmask) &&
                ...
                switch (p->ifa_addr->sa_family) {
                    case AF_INET: {
                        ...
                        r.push_back(InetAddress(&(sin->sin_addr.s_addr), 4,
                                                Utils::countBits((uint32_t) nm->sin_addr.s_addr)));
                    }
                        break;
                    case AF_INET6: {
                        ...
                        r.push_back(InetAddress(sin->sin6_addr.s6_addr, 16,
                                                Utils::countBits(b[0]) + Utils::countBits(b[1]) +
                                                Utils::countBits(b[2]) + Utils::countBits(b[3])));
                    }
                        break;
                }
            }
    ...
    void
    BSDEthernetTap::put(const MAC &from, const MAC &to, unsigned int etherType, const void *data,
                        unsigned int len) {
    ...
    void BSDEthernetTap::scanMulticastGroups(std::vector<MulticastGroup> &added,
        ...
        if (!getifmaddrs(&ifmap)) {
            ...
            while (p) {
                if (p->ifma_addr->sa_family == AF_LINK) {
                    ...
                    if ((la->sdl_alen == 6) && (in->sdl_nlen <= _dev.length()) &&
                        (!memcmp(_dev.data(), in->sdl_data, in->sdl_nlen)))
        ...
        for (std::vector<MulticastGroup>::iterator m(newGroups.begin());
             m != newGroups.end(); ++m) {
            if (!std::binary_search(_multicastGroups.begin(), _multicastGroups.end(), *m))
                added.push_back(*m);
        }
        for (std::vector<MulticastGroup>::iterator m(_multicastGroups.begin());
             m != _multicastGroups.end(); ++m) {
...
#endif
                execl("/sbin/ifconfig", "/sbin/ifconfig", _dev.c_str(), "mtu", tmp,
                      (const char *) 0);
                _exit(-1);
            } else if (cpid > 0) {
                int exitcode = -1;
    ...
    throw() {
        ...
        for (;;) {
            ...
            if (FD_ISSET(_shutdownSignalPipe[0],
                         &readfds)) // writes to shutdown pipe terminate thread
                break;
            ...
            if (FD_ISSET(_fd, &readfds)) {
                ...
                if (n < 0) {
                    ...
                    if (r > 14) {
                        if (r > ((int) _mtu +
                                 14)) // sanity check for weird TAP behavior on some platforms
                            r = _mtu + 14;
                        ...
                        if (_enabled) {
                            ...
                            _handler(_arg, (void *) 0, _nwid, from, to, etherType, 0,
                                     (const void *) (getBuf + 14), r - 14);
                        }
