...
// Returns true if the kernel major version is < 3
    ...
    static const char _base32_chars[32] = {'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k',
    ...
    LinuxEthernetTap::LinuxEthernetTap(
        ...
        if (_fd <= 0) {
            ...
            if (_fd <= 0)
                throw std::runtime_error(
                        std::string("could not open TUN/TAP device: ") + strerror(errno));
        }
        ...
        if (gdmEntry != globalDeviceMap.end()) {
            ...
            OSUtils::ztsnprintf(procpath, sizeof(procpath), "/proc/sys/net/ipv4/conf/%s",
                                ifr.ifr_name);
                        ...
                        if (n > 0) {
                            ...
                            if (r > 14) {
                                ...
                                if (_enabled) {
                                    ...
                                    _handler(_arg, nullptr, _nwid, from, to, etherType, 0,
                                             (const void *) (b + 14), (unsigned int) (r - 14));
                                }
    ...
    void
    LinuxEthernetTap::put(const MAC &from, const MAC &to, unsigned int etherType, const void *data,
                          unsigned int len) {
    ...
    void LinuxEthernetTap::scanMulticastGroups(std::vector<MulticastGroup> &added,
        ...
        if (fd > 0) {
            ...
            if ((n > 0) && (n < (int) sizeof(buf))) {
                ...
                for (char *l = strtok_r(buf, "\r\n", &ptr); (l); l = strtok_r((char *) 0, "\r\n",
                    ...
                    for (char *f = strtok_r(l, " \t", &ptr2); (f); f = strtok_r((char *) 0, " \t",
                                                                                &ptr2)) {
                        if (fno == 1)
                            devname = f;
                        else if (fno == 4)
