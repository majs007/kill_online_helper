...
namespace ZeroTier {
    ...
    uint32_t Arp::processIncomingArp(const void *arp, unsigned int len, void *response,
                                     unsigned int &responseLen, MAC &responseDest) {
        ...
        if (len >= 28) {
            if (!memcmp(arp, ARP_REQUEST_HEADER, 8)) {
                ...
                if ((targetEntry) && (targetEntry->local)) {
                    ...
                    memcpy(reinterpret_cast<uint8_t *>(response) + 14,
                           reinterpret_cast<const uint8_t *>(arp) + 24, 4);
                    memcpy(reinterpret_cast<uint8_t *>(response) + 18,
                           reinterpret_cast<const uint8_t *>(arp) + 8, 10);
                    responseLen = 28;
                ...
                if ((queryEntry) && (!queryEntry->local) &&
                    ((now - queryEntry->lastQuerySent) <= ZT_ARP_QUERY_MAX_TTL)) {
    ...
    MAC Arp::query(const MAC &localMac, uint32_t localIp, uint32_t targetIp, void *query,
                   unsigned int &queryLen, MAC &queryDest) {
        const uint64_t now = OSUtils::now();
        ...
        if (((e.mac) && ((now - e.lastResponseReceived) >= (ZT_ARP_EXPIRE / 3))) ||
            ...
            memcpy(q, &targetIp,
                   4); // target IP address for resolution (IP already in big-endian byte order)
            queryLen = 28;
            if (e.mac)
