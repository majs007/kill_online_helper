...
RemoveRedirectRange(struct UPNPUrls *urls,
    ...
    r = UPNP_DeletePortMappingRange(urls->controlURL, data->first.servicetype, ePortStart, ePortEnd,
                                    proto, manage);
    if (r != UPNPCOMMAND_SUCCESS) {
...
static void GetFirewallStatus(struct UPNPUrls *urls, struct IGDdatas *data) {
    ...
    UPNP_GetFirewallStatus(urls->controlURL_6FC, data->IPv6FC.servicetype, &firewallEnabled,
...
static void SetPinholeAndTest(struct UPNPUrls *urls, struct IGDdatas *data,
    ...
    r = UPNP_AddPinhole(urls->controlURL_6FC, data->IPv6FC.servicetype, remoteaddr, eport, intaddr,
                        iport, proto, lease_time, uniqueID);
    if (r != UPNPCOMMAND_SUCCESS)
...
static void GetPinholeAndUpdate(struct UPNPUrls *urls, struct IGDdatas *data,
    ...
    r = UPNP_CheckPinholeWorking(urls->controlURL_6FC, data->IPv6FC.servicetype, uniqueID,
                                 &isWorking);
    printf("CheckPinholeWorking: Pinhole ID = %s / IsWorking = %s\n", uniqueID,
           (isWorking) ? "Yes" : "No");
    if (r != UPNPCOMMAND_SUCCESS)
        printf("CheckPinholeWorking() failed with code %d (%s)\n", r, strupnperror(r));
    if (isWorking || r == 709) {
        r = UPNP_UpdatePinhole(urls->controlURL_6FC, data->IPv6FC.servicetype, uniqueID,
                               lease_time);
...
static void GetPinholeOutboundTimeout(struct UPNPUrls *urls, struct IGDdatas *data,
    ...
    r = UPNP_GetOutboundPinholeTimeout(urls->controlURL_6FC, data->IPv6FC.servicetype, remoteaddr,
...
GetPinholePackets(struct UPNPUrls *urls,
    ...
    r = UPNP_GetPinholePackets(urls->controlURL_6FC, data->IPv6FC.servicetype, uniqueID,
                               &pinholePackets);
    if (r != UPNPCOMMAND_SUCCESS)
        ...
        if ((rootdescurl && UPNP_GetIGDFromUrl(rootdescurl, &urls, &data, lanaddr, sizeof(lanaddr)))
            ...
            switch (command) {
                ...
                case 'N':
                    ...
                    if (RemoveRedirectRange(&urls, &data, commandargv[0], commandargv[1],
                                            commandargv[2],
