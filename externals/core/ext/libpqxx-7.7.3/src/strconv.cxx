...
namespace pqxx::internal {
    ...
    void PQXX_COLD
    throw_null_conversion(std::string
    const &type) {
    throw conversion_error {
    "Attempt to convert null to " + type + "."
};
}
...
namespace {
#if !defined(PQXX_HAVE_CHARCONV_INT)
    [[noreturn, maybe_unused]] void PQXX_COLD
 ...
 */
    template<typename T>
    class dumb_stringstream : public std::stringstream {
    public:
    ...
    template<typename F>
    inline bool PQXX_COLD
    from_dumb_stringstream(
            dumb_stringstream<F>
    &s,
    F &result, std::string_view
    text) {
    s.
    str(std::string{text}
    );
...
template<typename T>
inline T PQXX_COLD
from_string_awful_float(std::string_view
text)
{
if (
std::empty(text)
...
if (not ok)
throw pqxx::conversion_error{
"Could not convert string to numeric value: '" + std::string{
text} +
"'."};
...
/// Floating-point to_buf implemented in terms of to_string.
    ...
    to_dumb_stringstream(dumb_stringstream<F>
    &s,
    F value
    ) {
    s.str("");
