...
pqxx::connection::backendpid() const & noexcept {
...
pqxx::connection::protocol_version() const noexcept {
...
pqxx::connection::server_version() const noexcept {
...
void PQXX_COLD
pqxx::connection::add_receiver(pqxx::notification_receiver
*n)
{
if (n == nullptr)
throw argument_error{
"Null receiver registered"};
...
void PQXX_COLD
pqxx::connection::remove_receiver(pqxx::notification_receiver
*T) noexcept
{
if (T == nullptr)
return;
...
void PQXX_COLD
pqxx::connection::set_verbosity(error_verbosity
verbosity) &noexcept
{
PQsetErrorVerbosity(m_conn,
static_cast
...
int pqxx::connection::get_notifs() {
    ...
    for (auto N{get_notif(m_conn)}; N.get(); N = get_notif(m_conn)) {
        ...
        if (Hit.second != Hit.first) {
            ...
            for (auto i{Hit.first}; i != Hit.second; ++i)
                try {
...
pqxx::connection::username() const {
...
pqxx::connection::hostname() const {
...
void PQXX_COLD
pqxx::connection::register_errorhandler(errorhandler
*handler)
{
...
if (
std::empty(m_errorhandlers)
)
PQsetNoticeProcessor(m_conn, pqxx_notice_processor,
this);
m_errorhandlers.
push_back(handler);
}
...
void PQXX_COLD
pqxx::connection::unregister_errorhandler(errorhandler
*handler) noexcept
{
...
PQsetNoticeProcessor(m_conn, inert_notice_processor,
nullptr);
}
...
pqxx::connection::unesc_raw(char const text[]) const {
...
std::string PQXX_COLD
pqxx::connection::quote(binarystring
const &b) const
{
return
quote(b
...
pqxx::connection::set_client_encoding(char const encoding[]) & {
