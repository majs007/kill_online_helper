 ...
 */
    ...
    template<typename T> PQXX_LIBEXPORT extern std::string
    to_string_float(T);
...
/// The built-in arithmetic types do not have inherent null values.
    ...
    template<>
    struct string_traits<short> : internal::integral_traits<short> {
    };
    ...
    template<>
    struct string_traits<int> : internal::integral_traits<int> {
    };
    template<> inline constexpr bool is_unquoted_safe<int>{true};
    template<>
    struct string_traits<unsigned> : internal::integral_traits<unsigned> {
    };
    template<> inline constexpr bool is_unquoted_safe<unsigned>{true};
    template<>
    struct string_traits<long> : internal::integral_traits<long> {
    };
    ...
    template<>
    struct string_traits<float> : internal::float_traits<float> {
    };
    template<> inline constexpr bool is_unquoted_safe<float>{true};
    template<>
    struct string_traits<double> : internal::float_traits<double> {
    };
 ...
 */
        ...
        static constexpr bool is_null(std::variant<T...> const &value) noexcept {
            return std::visit(
                    [](auto const &i) noexcept {
                        return nullness<strip_t < decltype(i)>>
                        ::is_null(i);
                    },
                    value);
        }
        ...
        into_buf(char *begin, char *end, std::variant<T...> const &value) {
            return std::visit(
                    [begin, end](auto const &i) {
                        return string_traits<strip_t < decltype(i)>>
                        ::into_buf(begin, end, i);
                    },
                    value);
        }
    ...
    template<typename T>
    inline T from_string(std::stringstream const &text) {
...
/// String traits for non-const C-style string ("pointer to char").
    ...
    struct nullness<char[N]> : no_null<char[N]> {
    };
 ...
 */
    ...
    template<>
    struct nullness<std::string> : no_null<std::string> {
    };
 ...
 */
    template<>
    struct nullness<std::string_view> : no_null<std::string_view> {
    };
...
/// String traits for `string_view`.
    ...
    template<>
    struct nullness<zview> : no_null<zview> {
    };
...
/// String traits for `zview`.
    ...
    template<>
    struct nullness<std::stringstream> : no_null<std::stringstream> {
    };
    ...
    template<>
    struct string_traits<std::basic_string<std::byte>> {
        static std::size_t
    ...
    template<>
    struct string_traits<std::basic_string_view<std::byte>> {
        static std::size_t
...
namespace pqxx {
    ...
    template<>
    inline std::string to_string(std::stringstream const &value) {
        return value.str();
    }


    template<typename T>
    inline void into_string(T const &value, std::string &out) {
        if (is_null(value))
            throw conversion_error{
                    "Attempt to convert null " + type_name < T > +" to a string."};
