 ...
 */
    class PQXX_LIBEXPORT PQXX_NOVTABLE
    ...
    transaction_base &operator=(transaction_base && )
    = delete;
    ...
    [[nodiscard]] auto esc(ARGS &&...args) const {
     ...
     */
    template<typename... ARGS>
    [[nodiscard]] auto esc_raw(ARGS &&...args) const {
...
#include "pqxx/internal/ignore-deprecated-post.hxx"
    ...
    template<typename T>
    [[nodiscard]] std::string quote(T const &t) const {
        return conn().quote(t);
    }
...
/// Create a transaction (to be called by implementation classes only).
explicit transaction_base(connection
&c);
