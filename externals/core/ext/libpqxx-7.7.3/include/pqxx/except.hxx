 ...
 * ```
 */
struct PQXX_LIBEXPORT broken_connection
: failure
{
broken_connection();

explicit broken_connection(std::string
const &);
};


/// The caller attempted to set a variable to null, which is not allowed.
struct PQXX_LIBEXPORT variable_set_to_null
: failure
{
variable_set_to_null();

explicit variable_set_to_null(std::string
const &);
};
...
[[nodiscard]] PQXX_PURE std::string
const &
 ...
 */
struct PQXX_LIBEXPORT in_doubt_error
: failure
{
explicit in_doubt_error(std::string
const &);
};


/// The backend saw itself forced to roll back the ongoing transaction.
struct PQXX_LIBEXPORT transaction_rollback
: sql_error
{
 ...
 * perform it again.
 */
struct PQXX_LIBEXPORT serialization_failure
...
struct PQXX_LIBEXPORT statement_completion_unknown
: transaction_rollback
{
...
struct PQXX_LIBEXPORT deadlock_detected
: transaction_rollback
{
...
struct PQXX_LIBEXPORT internal_error
: std::logic_error
{
explicit internal_error(std::string
const &);
};


/// Error in usage of libpqxx library, similar to std::logic_error
struct PQXX_LIBEXPORT usage_error
: std::logic_error
{
explicit usage_error(std::string
const &);
};


/// Invalid argument passed to libpqxx, similar to std::invalid_argument
struct PQXX_LIBEXPORT argument_error
: std::invalid_argument
{
explicit argument_error(std::string
const &);
};


/// Value conversion failed, e.g. when converting "Hello" to int.
struct PQXX_LIBEXPORT conversion_error
: std::domain_error
{
explicit conversion_error(std::string
const &);
};


/// Could not convert value to string: not enough buffer space.
struct PQXX_LIBEXPORT conversion_overrun
: conversion_error
{
explicit conversion_overrun(std::string
const &);
};


/// Something is out of range, similar to std::out_of_range
struct PQXX_LIBEXPORT range_error
: std::out_of_range
{
explicit range_error(std::string
const &);
};


/// Query returned an unexpected number of rows.
struct PQXX_LIBEXPORT unexpected_rows
: public range_error
{
explicit unexpected_rows(std::string
const &msg) : range_error{
msg} {
}
};
...
struct PQXX_LIBEXPORT feature_not_supported
: sql_error
{
...
struct PQXX_LIBEXPORT data_exception
: sql_error
{
...
struct PQXX_LIBEXPORT integrity_constraint_violation
: sql_error
{
...
struct PQXX_LIBEXPORT restrict_violation
...
struct PQXX_LIBEXPORT not_null_violation
: integrity_constraint_violation
{
...
struct PQXX_LIBEXPORT foreign_key_violation
: integrity_constraint_violation
{
...
struct PQXX_LIBEXPORT unique_violation
: integrity_constraint_violation
{
...
struct PQXX_LIBEXPORT check_violation
: integrity_constraint_violation
{
...
struct PQXX_LIBEXPORT invalid_sql_statement_name
: sql_error
{
...
struct PQXX_LIBEXPORT invalid_cursor_name
: sql_error
{
...
struct PQXX_LIBEXPORT syntax_error
: sql_error
{
...
sql_error{
err, Q, sqlstate}, error_position{
pos}
{
}
};

struct PQXX_LIBEXPORT undefined_column
: syntax_error
{
...
struct PQXX_LIBEXPORT undefined_function
: syntax_error
{
...
struct PQXX_LIBEXPORT undefined_table
: syntax_error
{
...
struct PQXX_LIBEXPORT insufficient_privilege
: sql_error
{
...
struct PQXX_LIBEXPORT insufficient_resources
: sql_error
{
...
struct PQXX_LIBEXPORT disk_full
...
struct PQXX_LIBEXPORT out_of_memory
: insufficient_resources
{
...
struct PQXX_LIBEXPORT too_many_connections
: broken_connection
{
explicit too_many_connections(std::string
const &err) :
broken_connection{
err}
{
 ...
 */
struct PQXX_LIBEXPORT plpgsql_error
: sql_error
{
...
struct PQXX_LIBEXPORT plpgsql_raise
: plpgsql_error
{
...
struct PQXX_LIBEXPORT plpgsql_no_data_found
: plpgsql_error
{
...
struct PQXX_LIBEXPORT plpgsql_too_many_rows
: plpgsql_error
{
...
struct PQXX_LIBEXPORT blob_already_exists
: failure
{
explicit blob_already_exists(std::string
const &);
};
