...
the type of string parameters. However, by now, not all compilers support `std::string_view`. So we
wrote
...
`RedisCluster` connects to all master nodes in the cluster. For each master node, it maintains a
