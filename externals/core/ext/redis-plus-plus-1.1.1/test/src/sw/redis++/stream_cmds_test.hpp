    ...
    namespace redis {
        ...
        namespace test {
            ...
            void StreamCmdsTest<RedisInstance>::_test_group_cmds() {
                ...
                _redis.xpending(key, group, "-", "+", 1, consumer1,
                ...
                items.clear();
                _redis.xclaim(key, group, consumer1, std::chrono::milliseconds(10), id,
                              std::back_inserter(items));
                REDIS_ASSERT(items.size() == 1 && items[0].first == id,
                             "failed to test xclaim: " + std::to_string(items.size()));
