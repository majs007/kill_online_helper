...
namespace sw {
    ...
    namespace redis {
        ...
        namespace test {
            ...
            template<>
            inline Pipeline
            PipelineTransactionTest<RedisCluster>::_pipeline(const StringView &key) {
                return _redis.pipeline(key);
            }

            template<typename RedisInstance>
            Transaction
            PipelineTransactionTest<RedisInstance>::_transaction(const StringView &, bool piped) {
                return _redis.transaction(piped);
            }

            template<>
            inline Transaction
            PipelineTransactionTest<RedisCluster>::_transaction(const StringView &key,
                                                                bool piped) {
            ...
            void PipelineTransactionTest<RedisInstance>::_test_pipeline(const StringView &key,
                ...
                REDIS_ASSERT(reply::parse<bool>(replies.get(0)),
                             "failed to test pipeline with set operation");
