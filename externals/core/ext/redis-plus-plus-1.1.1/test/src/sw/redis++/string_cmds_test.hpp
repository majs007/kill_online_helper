    ...
    namespace redis {
        ...
        namespace test {
            ...
            void StringCmdTest<RedisInstance>::_test_str() {
                ...
                REDIS_ASSERT(_redis.getrange(key, 0, 2) == "",
                             "failed to test getrange on non-existent key");

                _redis.set(key, val);

                REDIS_ASSERT(_redis.getrange(key, 1, 2) == val.substr(1, 2),
                             "failed to test getrange");

                long long new_size = val.size() * 2;
                REDIS_ASSERT(_redis.setrange(key, val.size(), val) == new_size,
                             "failed to test setrange");
            ...
            void StringCmdTest<RedisInstance>::_test_bit() {
                ...
                REDIS_ASSERT(_redis.bitcount(key) == 0,
                             "failed to test bitcount on non-existent key");
            ...
            void StringCmdTest<RedisInstance>::_test_getset() {
                ...
                REDIS_ASSERT(
                        !_redis.set(key, val, std::chrono::milliseconds(0), UpdateType::NOT_EXIST),
                        "failed to test set with NOT_EXIST type");
                REDIS_ASSERT(!_redis.set(non_exist_key, val, std::chrono::milliseconds(0),
                                         UpdateType::EXIST),
