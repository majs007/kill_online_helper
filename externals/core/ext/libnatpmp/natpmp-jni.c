...
Java_fr_free_miniupnp_libnatpmp_NatPmp_init (JNIEnv
*env,
jobject obj, jint
forcegw,
jint forcedgw
...
initnatpmp(p, forcegw, (in_addr_t)
forcedgw);
...
if (fid == NULL) return;
(*env)->
SetObjectField(env, obj, fid, wrapped
);
}

JNIEXPORT void JNICALL
Java_fr_free_miniupnp_libnatpmp_NatPmp_free (JNIEnv
*env,
jobject obj
) {
...
(*env)->
SetObjectField(env, obj, fid, NULL
);
}
...
JNICALL Java_fr_free_miniupnp_libnatpmp_NatPmp_sendPublicAddressRequest(JNIEnv *env, jobject obj) {
...
JNIEXPORT void JNICALL
Java_fr_free_miniupnp_libnatpmp_NatPmp_startup(JNIEnv
* env,
jclass cls
) {
(void)
...
JNICALL Java_fr_free_miniupnp_libnatpmp_NatPmp_sendNewPortMappingRequest(JNIEnv *env, jobject obj,
                                                                         jint protocol,
                                                                         jint privateport,
                                                                         jint publicport,
                                                                         jint lifetime) {
...
JNICALL Java_fr_free_miniupnp_libnatpmp_NatPmp_getNatPmpRequestTimeout(JNIEnv *env, jobject obj) {
    natpmp_t *natpmp = getNatPmp(env, obj);
...
JNICALL Java_fr_free_miniupnp_libnatpmp_NatPmp_readNatPmpResponseOrRetry(JNIEnv *env, jobject obj,
                                                                         jobject response) {
    ...
    if (fid == NULL) return -1;
    (*env)->SetLongField(env, response, fid, ((jlong)
    resp.epoch) *1000);
    ...
    if (resp.type == 0) {
        ...
        if (fid == NULL) return -1;
        (*env)->SetLongField(env, response, fid, ((jlong)
        resp.pnu.newportmapping.lifetime) *1000 * 1000);
    }
    return result;
}
