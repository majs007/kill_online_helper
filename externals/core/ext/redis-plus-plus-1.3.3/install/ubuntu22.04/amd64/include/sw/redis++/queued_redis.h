...
// In this case, the only thing we can do is to destory the QueuedRedis object.
        ...
        public:
            ...
            QueuedRedis &restore(const StringView &key,
                                 const StringView &val,
                                 const std::chrono::milliseconds &ttl = std::chrono::milliseconds{
                                         0},
                                 bool replace = false) {
            ...
            QueuedRedis &
            hsetnx(const StringView &key, const StringView &field, const StringView &val) {
                return command(cmd::hsetnx, key, field, val);
            }

            QueuedRedis &
            hsetnx(const StringView &key, const std::pair<StringView, StringView> &item) {
            ...
            QueuedRedis &eval(const StringView &script,
                              ...
                              Args args_first,
                              Args args_last) {
                return command(cmd::eval<Keys, Args>, script, keys_first, keys_last, args_first,
                               args_last);
            }
            ...
            QueuedRedis &evalsha(const StringView &script,
                                 ...
                                 Args args_first,
                                 Args args_last) {
                return command(cmd::evalsha<Keys, Args>, script, keys_first, keys_last, args_first,
                               args_last);
            }
            ...
            -> typename std::enable_if<!std::is_convertible<Input, StringView>::value,
                ...
                return command(cmd::xreadgroup_range<Input>, group, consumer, first, last, count,
                               noack);
            }
