...
namespace sw {
    ...
    namespace redis {
        ...
        namespace fmt {
            ...
            inline FormattedCommand
            format_cmd(int argc, const char **argv, const std::size_t *argv_len) {
                char *data = nullptr;
...
// CONNECTION commands.
            ...
            inline FormattedCommand
            expire(const StringView &key, const std::chrono::seconds &timeout) {
            ...
            inline FormattedCommand expireat(const StringView &key,
                ...
                return format_cmd("EXPIREAT %b %lld", key.data(), key.size(),
            ...
            inline FormattedCommand pexpireat(const StringView &key,
                ...
                return format_cmd("PEXPIREAT %b %lld", key.data(), key.size(),
                                  tp.time_since_epoch().count());
            }
            ...
            inline FormattedCommand rename(const StringView &key, const StringView &newkey) {
                return format_cmd("RENAME %b %b", key.data(), key.size(), newkey.data(),
                                  newkey.size());
            }

            inline FormattedCommand renamenx(const StringView &key, const StringView &newkey) {
                return format_cmd("RENAMENX %b %b", key.data(), key.size(), newkey.data(),
                                  newkey.size());
            }
...
// STRING commands.
            ...
            inline FormattedCommand
            blpop(const StringView &key, const std::chrono::seconds &timeout) {
                return format_cmd("BLPOP %b %lld", key.data(), key.size(), timeout.count());
            }

            template<typename Input>
            FormattedCommand
            blpop_range(Input first, Input last, const std::chrono::seconds &timeout) {
                assert(first != last);
            ...
            inline FormattedCommand
            brpop(const StringView &key, const std::chrono::seconds &timeout) {
            ...
            template<typename Input>
            FormattedCommand
            brpop_range(Input first, Input last, const std::chrono::seconds &timeout) {
                assert(first != last);
            ...
            inline FormattedCommand
            lrem(const StringView &key, long long count, const StringView &val) {
                return format_cmd("LREM %b %lld %b", key.data(), key.size(), count, val.data(),
                                  val.size());
            }
...
// HASH commands.
            ...
            inline FormattedCommand hexists(const StringView &key, const StringView &field) {
                return format_cmd("HEXISTS %b %b", key.data(), key.size(), field.data(),
                                  field.size());
            }
...
// SET commands.
            ...
            inline FormattedCommand sadd(const StringView &key, const StringView &member) {
                return format_cmd("SADD %b %b", key.data(), key.size(), member.data(),
                                  member.size());
            }
            ...
            inline FormattedCommand sismember(const StringView &key, const StringView &member) {
                return format_cmd("SISMEMBER %b %b", key.data(), key.size(), member.data(),
                                  member.size());
            }
            ...
            inline FormattedCommand srem(const StringView &key, const StringView &member) {
                return format_cmd("SREM %b %b", key.data(), key.size(), member.data(),
                                  member.size());
            }
...
// SORTED SET commands.
            ...
            inline FormattedCommand
            bzpopmax(const StringView &key, const std::chrono::seconds &timeout) {
            ...
            inline FormattedCommand
            bzpopmin(const StringView &key, const std::chrono::seconds &timeout) {
                return format_cmd("BZPOPMIN %b %lld", key.data(), key.size(), timeout.count());
            }

            template<typename Input>
            FormattedCommand
            bzpopmin_range(Input first, Input last, const std::chrono::seconds &timeout) {
                assert(first != last);
            ...
            inline FormattedCommand zrank(const StringView &key, const StringView &member) {
                return format_cmd("ZRANK %b %b", key.data(), key.size(), member.data(),
                                  member.size());
            }

            inline FormattedCommand zrem(const StringView &key, const StringView &member) {
                return format_cmd("ZREM %b %b", key.data(), key.size(), member.data(),
                                  member.size());
            }
            ...
            inline FormattedCommand
            zremrangebyrank(const StringView &key, long long start, long long stop) {
                return format_cmd("ZREMRANGEBYRANK %b %lld %lld", key.data(), key.size(), start,
                                  stop);
            }
            ...
            inline FormattedCommand zrevrank(const StringView &key, const StringView &member) {
                return format_cmd("ZREVRANK %b %b", key.data(), key.size(), member.data(),
                                  member.size());
            }

            inline FormattedCommand zscore(const StringView &key, const StringView &member) {
                return format_cmd("ZSCORE %b %b", key.data(), key.size(), member.data(),
                                  member.size());
            }
