...
namespace sw {
    ...
    namespace redis {
        ...
        AsyncConnectionSPtr SimpleAsyncSentinel::create(const ConnectionOptions &opts,
                                                        ...
                                                        EventLoop *loop) {
            auto connection = std::make_shared<AsyncConnection>(opts, loop,
                                                                AsyncConnectionMode::SENTINEL);
        ...
        AsyncConnectionSPtr AsyncConnectionPool::fetch() {
            ...
            if (_sentinel) {
                ...
                if (role_changed ||
                    _need_reconnect(*connection, connection_lifetime, connection_idle_time)) {
                    try {
                        auto tmp_connection = sentinel.create(opts, shared_from_this(),
                                                              _loop.get());
