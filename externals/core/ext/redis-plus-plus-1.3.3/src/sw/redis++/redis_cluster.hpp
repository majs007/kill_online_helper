...
// KEY commands.
        ...
        inline bool
        RedisCluster::expire(const StringView &key, const std::chrono::seconds &timeout) {
        ...
        inline bool
        RedisCluster::pexpire(const StringView &key, const std::chrono::milliseconds &timeout) {
...
// STRING commands.
        ...
        template<typename Input>
        long long
        RedisCluster::bitop(BitOp op, const StringView &destination, Input first, Input last) {
            range_check("BITOP", first, last);

            auto reply = _command(cmd::bitop_range<Input>, destination, op, destination, first,
                                  last);
...
// LIST commands.
        ...
        inline void RedisCluster::lrange(const StringView &key, long long start, long long stop,
                                         Output output) {
...
// HASH commands.
        ...
        template<typename Input, typename Output>
        inline void
        RedisCluster::hmget(const StringView &key, Input first, Input last, Output output) {
...
// SET commands.
        ...
        template<typename Input>
        long long
        RedisCluster::sunionstore(const StringView &destination, Input first, Input last) {
...
// SORTED SET commands.
        ...
        inline auto
        RedisCluster::bzpopmax(const StringView &key, const std::chrono::seconds &timeout)
        ...
        inline auto
        RedisCluster::bzpopmin(const StringView &key, const std::chrono::seconds &timeout)
        -> Optional <std::tuple<std::string, std::string, double>> {
        ...
        template<typename Output>
        void RedisCluster::zrange(const StringView &key, long long start, long long stop,
                                  Output output) {
        ...
        void
        RedisCluster::zrangebylex(const StringView &key, const Interval &interval, Output output) {
        ...
        template<typename Output>
        void RedisCluster::zrevrange(const StringView &key, long long start, long long stop,
                                     Output output) {
        ...
        template<typename Interval, typename Output>
        void RedisCluster::zrevrangebyscore(const StringView &key, const Interval &interval,
                                            Output output) {
        ...
        void RedisCluster::zrevrangebyscore(const StringView &key,
                                            ...
                                            const LimitOptions &opts,
                                            Output output) {
            auto reply = _score_command<Output>(cmd::zrevrangebyscore<Interval>, key, interval,
                                                opts);
...
// SCRIPTING commands.
        ...
        Result RedisCluster::eval(const StringView &script,
            ...
            auto reply = _command(cmd::eval<Keys, Args>, *keys_first, script, keys_first, keys_last,
...
// Stream commands.
        ...
        std::string RedisCluster::xadd(const StringView &key,
                                       ...
                                       long long count,
                                       bool approx) {
            auto reply = command(cmd::xadd_maxlen_range<Input>, key, id, first, last, count,
                                 approx);
        ...
        void RedisCluster::xpending(const StringView &key,
                                    ...
                                    const StringView &consumer,
                                    Output output) {
            auto reply = command(cmd::xpending_per_consumer, key, group, start, end, count,
                                 consumer);
        ...
        -> typename std::enable_if<!std::is_convertible<Input, StringView>::value>::type {
            ...
            auto reply = command(cmd::xread_block_range<Input>, first, last, timeout.count(),
                                 count);
        ...
        ReplyUPtr
        RedisCluster::_command(Cmd cmd, std::true_type, const StringView &key, Args &&...args) {
        ...
        template<typename Cmd, typename Input, typename ...Args>
        ReplyUPtr
        RedisCluster::_range_command(Cmd cmd, std::true_type, Input input, Args &&...args) {
            return _command(cmd, *input, input, std::forward<Args>(args)...);
        }

        template<typename Cmd, typename Input, typename ...Args>
        ReplyUPtr
        RedisCluster::_range_command(Cmd cmd, std::false_type, Input input, Args &&...args) {
            ...
            for (auto idx = 0; idx < 2; ++idx) {
                try {
                    ...
                    try {
                        ...
                        throw Error(
                                "Slot migrating... ASKING node hasn't been set to IMPORTING state");
                    }
