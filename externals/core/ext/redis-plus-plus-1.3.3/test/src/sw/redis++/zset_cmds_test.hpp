    ...
    namespace redis {
        ...
        namespace test {
            ...
            void ZSetCmdTest<RedisInstance>::_test_zset() {
                ...
                REDIS_ASSERT(_redis.zrem(key, {"m1", "m2", "m3", "m4"}) == 2,
                             "failed to test zrem");
            }
            ...
            void ZSetCmdTest<RedisInstance>::_test_range() {
                ...
                REDIS_ASSERT(_redis.zcount(key, UnboundedInterval < double > {}) ==
                             static_cast<long long int>(s.size()),
                ...
                members.clear();
                _redis.zrangebyscore(key, UnboundedInterval < double > {},
                ...
                limitOpts.count = 2;
                members.clear();
                _redis.zrangebyscore(key, UnboundedInterval < double > {}, limitOpts,
                                     std::back_inserter(members));
                ...
                REDIS_ASSERT(res.size() == 2 && res.at(0) == s.at(s.size() - 2) &&
                             res.at(1) == s.at(s.size() - 1),
                             "failed to test zrangebyscore");
                ...
                _redis.zrevrangebyscore(key,
                                        ...
                                        std::inserter(res, res.end()));
                REDIS_ASSERT(res.size() == 1 && res.at(0) == s.at(0),
                             "failed to test zrevrangebyscore");

                REDIS_ASSERT(_redis.zremrangebyrank(key, 0, 0) == 1,
                             "failed to test zremrangebyrank");

                REDIS_ASSERT(_redis.zremrangebyscore(key,
                                                     BoundedInterval<double>(2, 3,
                                                                             BoundType::LEFT_OPEN)) ==
                             1,
            ...
            void ZSetCmdTest<RedisInstance>::_test_lex() {
                ...
                REDIS_ASSERT(_redis.zremrangebylex(key,
                                                   BoundedInterval<std::string>("m1", "m3",
                                                                                BoundType::OPEN)) ==
                             1,
