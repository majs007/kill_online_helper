    ...
    namespace redis {
        ...
        namespace test {
            ...
            std::string SanityTest<RedisInstance>::_build_uri(const ConnectionOptions &opts) const {
                auto scheme = "tcp://";
                auto uri =
                        opts.host + ":" + std::to_string(opts.port) + "/" + std::to_string(opts.db);
            ...
            void SanityTest<RedisInstance>::_ping(Redis &instance) {
                REDIS_ASSERT(instance.ping() == "PONG",
            ...
            void SanityTest<RedisInstance>::_test_generic_command() {
                ...
                REDIS_ASSERT(reply::parse<long long>(*reply) == 124,
                             "failed to test generic command");
                ...
                REDIS_ASSERT(
                        res.size() == 2 && res[0] && *(res[0]) == "v" && res[1] && *(res[1]) == "v",
                        "failed to test generic command");
                ...
                val = _redis.template command<OptionalString>(get_cmd_str.begin(),
                                                              get_cmd_str.end());
                REDIS_ASSERT(val && *val == "new_value", "failed to test generic command");
            ...
            template<typename RedisInstance>
            void
            SanityTest<RedisInstance>::_test_hash_tag(std::initializer_list<std::string> keys) {
            ...
            template<typename RedisInstance>
            void
            SanityTest<RedisInstance>::_test_pipeline(const StringView &key, Pipeline &pipeline) {
                auto pipe_replies = pipeline.command("set", key, "value").command("get",
                                                                                  key).exec();
            ...
            template<typename RedisInstance>
            void SanityTest<RedisInstance>::_test_transaction(const StringView &key,
            ...
            void SanityTest<RedisInstance>::_test_uri() {
                ...
                opts = ConnectionOptions(
                        "tcp://127.0.0.1:7000/1?keep_alive=true&connect_timeout=300ms&socket_timeout=1s");
                REDIS_ASSERT(opts.user == "default" && opts.password == ""
                             && opts.host == "127.0.0.1" && opts.port == 7000
                             && opts.db == 1 && opts.type == ConnectionType::TCP
                             && opts.keep_alive &&
                ...
                opts = ConnectionOptions(
                        "tcp://pass@127.0.0.1:7000?connect_timeout=300ms&socket_timeout=1s");
                REDIS_ASSERT(opts.user == "default" && opts.password == "pass"
                             && opts.host == "127.0.0.1" && opts.port == 7000
                             && opts.db == 0 && opts.type == ConnectionType::TCP
                             && opts.connect_timeout == std::chrono::milliseconds(300)
                ...
                opts = ConnectionOptions(
                        "tcp://user:pass@127.0.0.1?connect_timeout=300ms&socket_timeout=1s&keep_alive=false");
                REDIS_ASSERT(opts.user == "user" && opts.password == "pass"
                             && opts.host == "127.0.0.1" && opts.port == 6379
                             && opts.db == 0 && opts.type == ConnectionType::TCP
                             && opts.connect_timeout == std::chrono::milliseconds(300)
                ...
                opts = ConnectionOptions(
                        "unix://path/to/unix/domain.sock?keep_alive=false&socket_timeout=100ms");
                REDIS_ASSERT(opts.user == "default" && opts.password == ""
                             && opts.path == "path/to/unix/domain.sock"
                             && opts.db == 0 && opts.type == ConnectionType::UNIX
                             && !opts.keep_alive &&
                ...
                opts = ConnectionOptions(
                        "unix://user:pass@path/to/unix/domain.sock/1?connect_timeout=1s&keep_alive=true&socket_timeout=100ms");
                REDIS_ASSERT(opts.user == "user" && opts.password == "pass"
                             && opts.path == "path/to/unix/domain.sock"
                             && opts.db == 1 && opts.type == ConnectionType::UNIX
