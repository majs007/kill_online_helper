    ...
    namespace redis {
        ...
        namespace test {
            ...
            void SetCmdTest<RedisInstance>::_test_set() {
                ...
                REDIS_ASSERT(_redis.sadd(key, members) == 2,
                             "failed to test sadd with multiple members");
                ...
                REDIS_ASSERT(bool(ele) && res.find(*ele) != res.end(),
                             "failed to test srandmember");
                ...
                REDIS_ASSERT(_redis.srem(key, members) == 2,
                             "failed to test srem with mulitple members");
                REDIS_ASSERT(_redis.srem(key, members) == 0,
                             "failed to test srem with mulitple members");
            }
            ...
            void SetCmdTest<RedisInstance>::_test_multi_set() {
                ...
                REDIS_ASSERT(sinter == std::vector<std::string>({"a"}),
                             "failed to test sinterstore");
