    ...
    namespace redis {
        ...
        namespace test {
            ...
            void HashCmdTest<RedisInstance>::_test_numeric() {
                ...
                REDIS_ASSERT(_redis.hincrbyfloat(key, field, 1.5) == 1.5,
                             "failed to test hincrbyfloat");
            }
            ...
            void HashCmdTest<RedisInstance>::_test_hscan() {
                ...
                auto cursor = 0;
                while (true) {
                    cursor = _redis.hscan(key, cursor, "f*", 2,
                                          std::inserter(item_map, item_map.end()));
                    if (cursor == 0) {
                        break;
                    }
                }
