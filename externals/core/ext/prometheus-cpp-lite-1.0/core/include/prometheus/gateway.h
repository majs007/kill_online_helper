...
namespace prometheus {
    ...
    public:
        ...
        Gateway(const std::string host, const std::string port,
                ...
                : job_uri_(host + ':' + port + std::string("/metrics/job/") + jobname), labels_{} {
    ...
    private:
        ...
        int performHttpRequest(HttpMethod /*method*/, const std::string &uri_str,
            ...
            jdl::URI uri(uri_str);
            jdl::HTTPResponse response = jdl::HTTPClient::request(jdl::HTTPClient::m_post, uri,
                                                                  body);
        ...
        std::future<int> async_push(HttpMethod method) {
            ...
            for (const auto &wcollectable: collectables_) {
                ...
                futures.emplace_back(
                        std::async(std::launch::async, [method, &uri, &body_ptr, this] {
